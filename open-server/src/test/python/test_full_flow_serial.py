#!/usr/bin/env python3
"""
串行编排 E2E 测试 — plan-json-schema v3

拓扑: trigger → script → connector → script → exit
覆盖: 脚本节点 (GraalJS 沙箱) / 连接器节点 / 多节点串行传递数据

用法:
  cd open-server/src/test/python
  python3 test_orch_serial.py
"""
...
from client import (
    CONNECTOR_API_BASE, CONNECTOR_API_HEALTH, MOCK_SERVER_URL
)
import requests
...
class MockServer:
    ...
    def _make_handler(self):
        ...
        class H(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                state["call_count"] += 1
                self._respond(200, {"status": "ok", "call": state["call_count"], "method": "GET"})
...
@pytest.mark.L3
def test_full_flow_serial():
    print("=" * 60)
    print("  串行编排 E2E — plan-json-schema v3")
    ...
    try:
        ...
        def s1():
            ...
            r = os_api("POST", "/connectors", {
                "nameCn": f"串行测试_{_RUN_ID}",
                "nameEn": f"serial-{_RUN_ID}",
                "connectorType": 1
            })
            d = get_data(r)
        ...
        def s5():
            ...
            fid = snow_id()
            r = os_api("POST", "/flows", {
                "nameCn": f"串行编排_{_RUN_ID}",
                "nameEn": f"serial-orch-{_RUN_ID}"
            })
            d = get_data(r)
        ...
        def s7():
            r = os_api("PUT", f"/flows/{fid}/versions/{fvid}", {
                "orchestrationConfig": {
                    "nodes": [
                        ...
                        # ── script_1: 转换触发器的输入 ──
                        {
                            ...
                            "data": {
                                ...
                                "script": (
                                    "function main(ctx) {\n"
                                    "  var name = ctx['" + nid_trigger + "'].input.body.name;\n"
                                    "  var value = ctx['" + nid_trigger + "'].input.body.value || 0;\n"
                                    "  return {\n"
                                    "    message: 'Hello, ' + name + '!',\n"
                                    "    tags: 'serial,orch,v3',\n"
                                    "    doubled: value * 2\n"
                                ...
                                "input": {
                                    "body": {
                                        ...
                                        "properties": {
                                            "message": {
                                                "type": "string",
                                                "value": "${$.node." + nid_script_1 + ".output.message}"
                                            },
                                            "tags": {
                                                "type": "string",
                                                "value": "${$.node." + nid_script_1 + ".output.tags}"
                                            }
                                        }
                                    }
                                }
                                ...
                                "output": {
                                    "body": {
                                        ...
                                        "properties": {
                                            "code": {
                                                "type": "number",
                                                "value": "${$.node." + nid_script_2 + ".output.resultCode}"
                                            },
                                            "message": {
                                                "type": "string",
                                                "value": "${$.node." + nid_script_2 + ".output.resultMsg}"
                                            },
                                            "callCount": {
                                                "type": "number",
                                                "value": "${$.node." + nid_script_2 + ".output.callNumber}"
                                            },
                                            "serverTime": {
                                                "type": "string",
                                                "value": "${$.node." + nid_script_2 + ".output.serverTime}"
                                            },
                                            "echoed": {
                                                "type": "string",
                                                "value": "${$.node." + nid_script_2 + ".output.echoedMessage}"
                                            }
                                        }
                                    }
                                }
                    ...
                    "edges": [
                        {"id": f"e1_{_RUN_ID}", "source": nid_trigger, "target": nid_script_1,
                         "type": "smoothstep", "data": {"businessType": "default"}},
                        {"id": f"e2_{_RUN_ID}", "source": nid_script_1, "target": nid_conn,
                         "type": "smoothstep", "data": {"businessType": "default"}},
                        {"id": f"e3_{_RUN_ID}", "source": nid_conn, "target": nid_script_2,
                         "type": "smoothstep", "data": {"businessType": "default"}},
                        {"id": f"e4_{_RUN_ID}", "source": nid_script_2, "target": nid_exit,
                         "type": "smoothstep", "data": {"businessType": "default"}}
                    ],
                    "flowConfig": {"flowMode": "serial", "rateLimitConfig": {"maxQps": 100}}
                }
            })
        ...
        if not failed:
            if not step("UPDATE 编排(5节点串行)", s7): failed = True
        if not failed:
            print("  Phase 2: 编排完成")
        ...
        def s14():
            ...
            try:
                ...
                os_ok(body.get("code") == 0 or body.get("status") == "success",
                      f"invoke result OK")
                os_ok(body.get("echoed") == "Hello, OpenApp!",
                      f"echoed={body.get('echoed')}")
            except Exception as e:
                os_fail(f"json解析: {e}")
                return False
            return True
        ...
        if not failed:
            if not step("INVOKE (5节点串行)", s14): failed = True
        if not failed:
    ...
    print(f"\n{'='*60}")
    print(f"  串行编排 E2E {'通过' if not failed else '失败'}")
    print(f"{'='*60}")
