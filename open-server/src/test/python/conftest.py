...
def connector():
    """空连接器（无版本），自动清理"""
    cid = _snow_id()
    db(f"INSERT INTO openplatform_v2_cp_connector_t (id, name_cn, name_en, connector_type, app_id, create_by, last_update_by) VALUES ({cid}, 'pytest', 'pytest', 1, {TEST_APP_ID}, 'tester', 'tester')")
    yield cid
...
def flow():
    """空连接流"""
    fid = _snow_id()
    db(f"INSERT INTO openplatform_v2_cp_flow_t (id, name_cn, name_en, lifecycle_status, app_id, create_by, last_update_by) VALUES ({fid}, 'pytest_flow', 'pytest_flow', 1, {TEST_APP_ID}, 'tester', 'tester')")
    yield fid
