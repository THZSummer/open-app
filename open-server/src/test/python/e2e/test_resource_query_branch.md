...
调用方传入查询参数（type、header、query、body），Flow 根据 type 路由到对应的开放平台 OpenAPI，透传参数并发起 HTTP 请求，返回列表数据。
...
curl -X POST 'http://open-server:18080/api/v1/flows/{flowId}/invoke' \
  ...
  -d '{
    "type": "flows",
    "query": {"curPage": 1, "pageSize": 10, "keyword": "审批流"},
    "header": {"X-App-Id": "20250730213114178360970", "Cookie": "user_id=admin"},
    "body": {}
  }'
```
...
Script 节点:
  ...
  3. header → 传递给 ctx.http 的请求头
  4. body  → 传递给 ctx.http 的请求体 (预留 POST 场景)
  ↓
ctx.http.get(url, {headers})  →  开放平台 OpenAPI
  ↓
返回 {code, data[]}
  ↓
Script formatItems → {result, domain, group, path}
  ↓
Exit 节点映射到 HTTP 响应
```
...
```json
{
    "result": "9000,连接器A,2|9001,连接器B,2",
    "domain": "2",
    "group": "连接器A",
    "path": "connectors"
}
...
```json
{
    "result": "8000,审批流,1|8001,数据同步流,2",
    "domain": "2",
    "group": "审批流",
    "path": "flows"
}
...
function main(ctx) {
    ...
    var resp = ctx.http.get(url, {headers: hdrs});
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `result` | `string` | 管道分隔的列表数据 `id,name,status\|id,name,status` |
| `domain` | `string` | 列表条目数 |
| `group` | `string` | 首条名称 |
| `path` | `string` | 路由类型标识 |
