...
import { getCategoryApiPermissions, subscribeApiPermissions } from '@/services/permission.service';
import { ApiPermission } from '@/services/permission.service';
import styles from './PermissionDrawer.module.less';

interface ApiPermissionDrawerProps {
  visible: boolean;
  appId: string;
  onClose: () => void;
}
 ...
 * API 权限申请抽屉（懒加载模式）
 */
const ApiPermissionDrawer: React.FC<ApiPermissionDrawerProps> = ({
  visible,
  appId,
  onClose,
}) => {
  ...
  const handleTreeSelect: TreeProps['onSelect'] = async (selectedKeys) => {
    if (selectedKeys.length > 0) {
      ...
      setLoading(true);
      try {
        const response = await getCategoryApiPermissions(categoryId, {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await subscribeApiPermissions(appId, selectedPermissionIds);
   ...
   * 权限表格列定义
   */
  const columns: ColumnsType<ApiPermission> = [
    {
      title: (
        <Checkbox
      ...
      key: 'nameCn',
      width: 200,
      render: (text: string, record: ApiPermission) => (
        <div>
          <Checkbox
    ...
    },
    {
      ...
      key: 'api',
      width: 200,
      render: (_, record: ApiPermission) => (
        <div>
          <div>{record.api?.method}</div>
          <div style={{ color: '#999', fontSize: 12 }}>{record.api?.path}</div>
        </div>
      ),
    },
    {
