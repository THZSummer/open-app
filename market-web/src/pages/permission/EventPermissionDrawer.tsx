...
import { getCategoryEventPermissions, subscribeEventPermissions } from '@/services/permission.service';
import { EventPermission } from '@/services/permission.service';
import styles from './PermissionDrawer.module.less';

interface EventPermissionDrawerProps {
  visible: boolean;
  appId: string;
  onClose: () => void;
}

const EventPermissionDrawer: React.FC<EventPermissionDrawerProps> = ({
  visible,
  appId,
  onClose,
}) => {
  ...
  const handleTreeSelect: TreeProps['onSelect'] = async (selectedKeys) => {
    if (selectedKeys.length > 0) {
      ...
      setLoading(true);
      try {
        const response = await getCategoryEventPermissions(categoryId, {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await subscribeEventPermissions(appId, selectedPermissionIds);
  ...
  const columns: ColumnsType<EventPermission> = [
    {
      title: (
        <Checkbox
      ...
      key: 'nameCn',
      width: 200,
      render: (text: string, record: EventPermission) => (
        <div>
          <Checkbox
    ...
    },
    {
      ...
      key: 'topic',
      width: 200,
      render: (_, record: EventPermission) => <Tag color="purple">{record.event?.topic}</Tag>,
    },
    {
      title: '需要审批',
...
export default EventPermissionDrawer;
