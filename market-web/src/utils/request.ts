...
const instance: AxiosInstance = axios.create({
  baseURL: import.meta.env.VITE_API_BASE_URL || '/service/open/v2',
  timeout: 30000,
  headers: {
...
instance.interceptors.response.use(
  (response: AxiosResponse<ApiResponse>) => {
    ...
    // 如果是分页数据，确保 total 是数字类型
    if (data.data && typeof data.data.total === 'string') {
    ...
    if (error.response) {
      ...
      if (data instanceof Blob) {
        try {
          ...
          try {
            ...
            message.error(jsonError.messageZh || jsonError.message || '请求失败');
          } catch {
            const preview = text.slice(0, 200);
            message.error(`请求失败: ${preview}`);
          }
        } catch {
          message.error('请求失败');
        }
      ...
      switch (status) {
        ...
        case 500:
          ...
          break;
        default:
          message.error(data?.messageZh || data?.messageEn || '请求失败');
      }
