...
>
> **v9.0 变更摘要**: Tab 页拆分为两个独立页面（待审批应用 + 已上架应用）；已审批列表改为已上架应用列表（按应用分组展示最新已上架版本）；查看按钮改为 window.open 新开标签页；新增审批状态机设计
...
当前固定为**应用版本发布审批**类型，拆分为两个独立菜单页面：

| 菜单项 | 路由 | 图标 | 说明 |
|--------|------|------|------|
| 待审批应用 | `/approval-pending` | `AuditOutlined` | 展示 status=PENDING(0) 的审批记录 |
| 已上架应用 | `/approval-published` | `CheckCircleOutlined` | 展示每个应用最新已上架（APPROVED）版本 |

后续新增其他审批类型时，每种类型创建独立的待审批/已上架页面。
...
<Route path="approval-published" element={<ApprovalPublished />} />
```
...
```js
// menuItems 数组新增：
{ key: '/approval-pending', icon: <AuditOutlined />, label: '待审批应用' },
{ key: '/approval-published', icon: <CheckCircleOutlined />, label: '已上架应用' },
```

#### 2.2.2 页面结构

两个独立模块，遵循 `routeRedBlue/` 目录约定 `[src/market-web/router/routeRedBlue/]`：

```
src/router/routeRedBlue/
├── approval-pending/
│   ├── index.js            # 待审批页主文件（useState, useEffect, 事件处理）
...
两个独立页面，各自包含表格 + 分页（无 Tabs、无搜索栏，后续迭代按需增加）。

**待审批应用页**（`/approval-pending`）：
...
```js
// approval-pending/thunk.js — API 调用
...
```js
// approval-published/thunk.js — API 调用
...
export const getTableColumns = ({ renderAppName, renderAction }) => [
...
```js
// approval-published/constant.js
export const API_CONFIG = {
...
```js
// approval-pending/index.js — 状态管理
...
```js
// approval-published/index.js — 状态管理（更简单，无审批操作）
const [dataSource, setDataSource] = useState([]);
const [loading, setLoading] = useState(false);
const [pagination, setPagination] = useState({ curPage: 1, pageSize: 10, total: 0 });
const [currentLang, setCurrentLang] = useState(localStorage.getItem('lang') || 'zh');
...
| # | 文件路径 | 说明 |
|:-:|---------|------|
| 1 | `approval-pending/index.js` | 待审批页主文件 |
| 2 | `approval-pending/index.module.less` | 待审批页样式 |
| 3 | `approval-pending/constant.js` | 常量（API 配置, 操作枚举, 列定义） |
| 4 | `approval-pending/thunk.js` | API 调用（fetchPendingList, processApproval） |
| 5 | `approval-published/index.js` | 已上架页主文件 |
| 6 | `approval-published/index.module.less` | 已上架页样式 |
| 7 | `approval-published/constant.js` | 常量（API 配置, 列定义） |
| 8 | `approval-published/thunk.js` | API 调用（fetchPublishedList） |

### 6.3 前端修改文件（3 个）

| # | 文件路径 | 变更 |
|:-:|---------|------|
| 1 | `market-web/src/router/index.tsx` | 新增 `<Route path="approval-pending" />` 和 `<Route path="approval-published" />` |
| 2 | `market-web/src/components/Layout/index.js` | menuItems 新增 2 项（AuditOutlined + CheckCircleOutlined） |
...
| # | 用例 | 预期 |
|:-:|------|------|
| F-01 | 导航到 `/approval-pending` 页面 | 显示待审批列表，自动加载数据 |
| F-02 | 导航到 `/approval-published` 页面 | 显示已上架列表，自动加载数据 |
| F-03 | 点击"待审批应用"菜单项 | 正确导航，菜单高亮 |
| F-04 | 点击"已上架应用"菜单项 | 正确导航，菜单高亮 |
| F-05 | 应用名称列 — 切换语言 | 中文环境显示 appNameCn，英文环境展示 appNameEn |
| F-06 | 待审批页点击"查看"按钮 | `window.open` 新开浏览器标签页，URL 为 `/app-detail/{appId}` |
