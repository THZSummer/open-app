...
| `inspect/test_connector_url_whitelist.py` | L2 | URL 白名单校验 (FR-015) |
...
def test_trigger(connector, deployed_flow):
    # connector fixture 自动建连接器
    # deployed_flow fixture 自动建并部署 Flow
    resp = trigger_invoke(deployed_flow, body={...})
    assert resp.status_code == 200
    # 测试结束自动清理，无需 try/finally
```
...
```bash
KEEP_TEST_DATA=1 pytest -m "L0 or L1"
# 跑完后 DB 中保留全部测试数据，可 SQL 查询验证
```
...
└── inspect/
    ...
    ├── test_connector_url_whitelist.py  ← URL 白名单
