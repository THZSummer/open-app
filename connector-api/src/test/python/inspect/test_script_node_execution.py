...
def trigger_invoke(flow_id, body=None, timeout=15):
    """Invoke a flow via HTTP trigger.

    Uses a longer default timeout (15s) to accommodate script execution.
    Returns the Response object, or None on connection error / HTTP timeout.
    """
    url = f"http://localhost:18180/api/v1/trigger/{flow_id}/invoke"
    h = {"Content-Type": "application/json", "X-Sys-Token": "test-token"}
    try:
        start = time.time()
        resp = req_lib.post(url, json=body or {}, headers=h, timeout=timeout)
        ...
        if not is_quiet():
            print(f"\n  TIMEOUT: request exceeded {timeout}s")
        return None
