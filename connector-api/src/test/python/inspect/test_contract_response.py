...
def test_contract_response():
    ...
    db(f"INSERT INTO openplatform_v2_cp_flow_t (id, name_cn, name_en, lifecycle_status, app_id, create_by, last_update_by) VALUES ({test_flow_id}, '契约测试', 'ContractTest', 2, {INTERNAL_APP_ID}, 'tester', 'tester')")
    ...
    # ── X- 响应头存在性 ──────────────────────────────────
    print("\n--- X- 响应头存在性 ---")
    for label, resp in [("401-无认证", auth_resp), ("400-flow不存在", notfound_resp)]:
        if resp is None:
            continue
    ...
    if notfound_resp is not None:
        check("HTTP 状态码为 400（flow 不存在）",
              notfound_resp.status_code == 400,
    ...
    for label, resp in [("401-无认证", auth_resp), ("400-flow不存在", notfound_resp)]:
        if resp is not None:
            ...
            if code and re.match(r"^\d+$", code):
                codes_seen.add(code)
    check("错误码 42001 覆盖（无认证）",
          "42001" in codes_seen,
          f"已覆盖: {codes_seen}")
    check("错误码 41101 覆盖（flow 不存在）",
          "41101" in codes_seen,
          f"已覆盖: {codes_seen}")
