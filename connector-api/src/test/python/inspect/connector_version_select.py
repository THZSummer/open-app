...
mock_ready = False
try:
    mock_server = HTTPServer((MOCK_HOST, MOCK_PORT), VersionMockHandler)
...
def trigger_invoke(flow_id, timeout=15):
    ...
    try:
        start = time.time()
        resp = req_lib.post(url, json={"msg": "version_test"}, headers=h, timeout=timeout)
    ...
    if not is_quiet():
        _print_request("POST", url, h, {"msg": "version_test"})
