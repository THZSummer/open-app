...
echo "🔨 编译启动中..."
mkdir -p logs
nohup mvn spring-boot:run -Dspring-boot.run.profiles="$PROFILE" -Dmaven.test.skip=true > "$LOG" 2>&1 &
echo $! > "$PID_FILE"
...
for i in $(seq 1 30); do
    sleep 2
    RESULT=$(curl -s "http://localhost:$PORT/connector-api/actuator/health" 2>&1) || true
