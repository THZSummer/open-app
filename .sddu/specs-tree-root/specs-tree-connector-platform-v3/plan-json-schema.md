...
**创建日期**: 2026-05-22  
**最后更新**: 2026-06-25
**修订说明**: v9.11 — 抽取 connectorConfigDef（连接器配置唯一定义源），§5.2 改 $ref 消除重复；connectorNodeDataDef v2：新增 connectorId + connectorConfig 快照
...
| 横跨层 | 多场景复用（认证/限流/基类/错误） | `connectorConfigDef`、`authConfigDef`、`rateLimitConfigDef`、`nodeDataBaseDef`、`errorInfoDef` |
...
| 8 | `connectorConfigDef` | 横跨 | 连接器配置完整定义（供 §5 connectionConfig 和编排快照共用） | §4.3.10 connectorNodeDataDef, §5.2 connectionConfig |
...
#### 4.3.7 connectorConfigDef

> 连接器配置完整定义。供 §5 `connectionConfig`（存储）和 §4.3.10 `connectorNodeDataDef.connectorConfig`（快照）共同引用，连接器配置的唯一定义源。

> **Def**

```json
{
  "$id": "urn:openapp:schema:connectorConfigDef:v1",
  "type": "object",
  "additionalProperties": false,
...
> 继承 `nodeDataBaseDef`（type / labelCn / labelEn / structConfig），扩展连接器独有字段。v2 重构：新增 `connectorId`（归属标识）+ `connectorConfig`（§5 connectionConfig 完整快照，选版本时立即抓取），实现编排自包含——不再需要跨表 JOIN 即可理解连接器节点的完整 API 画像。移除节点层 `rateLimitConfig`（连接器出站限流由 `connectorConfig` 快照中的连接器级配置管控）。
    ...
    { "$ref": "#/definitions/nodeDataBaseDef" },
    {
      ...
      "properties": {
        ...
        "connectorVersionId": {
          "type": "string",
          "pattern": "^[1-9][0-9]{15,19}$",
          "description": "引用的连接器版本 ID，必须为已发布版本。更换版本时 connectorConfig 全量替换为新版本的 connectionConfig"
        },
        "connectorConfig": {
          "$ref": "#/definitions/connectorConfigDef",
        ...
        "input": {
          ...
          "description": "字段映射。properties 中通过 value 字段声明映射表达式（遵循 §3 值表达式体系），字段名与 connectorConfig.input 中的声明对应"
        }
      },
      "required": ["connectorId", "connectorVersionId", "connectorConfig"]
    }
  ]
}
```
...
| connectorVersionId | string | ✅ | 独有 | 引用的连接器版本 ID。更换版本时 connectorConfig 全量替换 |
| connectorConfig | object | ✅ | 独有 | 连接器配置完整快照 |
...
| 快照 | `connectorConfig` | API 完整画像（url / method / auth / 入参声明 / 出参声明） | 选版本时立即从 connection_config 抓取 | ❌ 只读 |
...
```json
{
  "nodes": [
    ...
    { "id":"conn-1",        "type":"connector",     "position":{"x":510,"y":400}, "data":{ "type":"connector",     "labelCn":"被保护节点",     "connectorId":"1234567890123456000", "connectorVersionId":"1234567890123456789", "connectorConfig":{/* 连接器完整配置，简写 */}, "input":{}, "structConfig":{ "parentLoopV2GroupId":"err-1","parentLoopV2Role":"right-column-node" } } },
...
```json
{
  ...
  "definitions": {
    ...
    "connectorNodeDataDef": {
      ...
      "description": "连接器节点业务数据。v2 新增 connectorId + connectorConfig 快照，移除 rateLimitConfig",
      "allOf": [
        { "$ref": "#/definitions/nodeDataBaseDef" },
        {
          "type": "object",
          ...
          "properties": {
            ...
            "connectorVersionId": {
              "type": "string",
              "pattern": "^[1-9][0-9]{15,19}$",
              "description": "引用的连接器版本 ID，必须为已发布版本。更换版本时 connectorConfig 全量替换"
            },
            "connectorConfig": {
              "$ref": "#/definitions/connectorConfigDef",
...
```json
{
  ...
  "description": "连接器配置，声明如何调用下游 API。定义为 connectorConfigDef（§4.3.7），存储在 connector_version_t.connection_config MEDIUMTEXT 字段中",
  "$ref": "#/definitions/connectorConfigDef"
}
```
...
字段定义见 §4.3.7 `connectorConfigDef`。以下为 §5 特有的存储说明：
...
| 数据性质 | 连接器版本自身的对外 API 声明 |
| 生命周期 | 随连接器版本创建/发布，独立于编排 |
| 引用方式 | 编排中 connector 节点通过 connectorVersionId 引用已发布版本，选版本时立即全量快照至 connectorConfig |
...
| `connector` | `connector` | connectorNodeDataDef | connectorId, connectorVersionId, connectorConfig | 连接器调用（含配置快照 + 字段映射） |
  ...
  "nodes": [
    ...
    // ==================== 循环体：connector → data_processor ====================
    {
      ...
      "data": {
        ...
        "connectorId": "1234567890123456000",
        "connectorVersionId": "1234567890123456789",
        "connectorConfig": { "protocol": "HTTP", "protocolConfig": { "url": "https://api.example.com/item/detail", "method": "GET" }, "authConfigs": [{ "type": "SOA", "header": { "type": "object", "properties": { "X-Soa-Token": { "type": "string", "required": true, "sensitive": true, "value": "${$.system.env.soaToken}" } } } }], "input": { "protocol": "HTTP", "body": { "type": "object", "properties": { "itemId": { "type": "string", "required": true } } } }, "output": { "protocol": "HTTP", "body": { "type": "object", "properties": { "name": { "type": "string" } } } }, "timeoutMs": 5000 },
...
| v9.11 | 2026-06-25 | **connectorConfigDef 抽取 + connectorNodeDataDef v2 重构**：① 新增 `connectorConfigDef` 共享组件（§4.3.7），连接器配置唯一定义源（含 timeoutMs / rateLimitConfig）② §5.2 `connectionConfig` 改为 `$ref` connectorConfigDef，消除重复定义 ③ `connectorNodeDataDef`（§4.3.10）新增 `connectorId` + `connectorConfig`（`$ref` connectorConfigDef）快照，移除节点层 `rateLimitConfig` ④ §4.1/§4.2/§4.4/§5.3 同步更新 ⑤ 组件总数 17→18 |
