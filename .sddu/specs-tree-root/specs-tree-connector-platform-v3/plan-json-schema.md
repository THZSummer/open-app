...
| labelCn | string | ❌ | 基类 | 节点中文标签 |
| labelEn | string | ❌ | 基类 | 节点英文标签 |
| scriptContent | string | ✅ | 独有 | 最大 10000 字符。ctx 为上游节点数据 Map，return 为节点 output |
| outputSchema | object | ❌ | 独有 | 复用 jsonObjectDef，供下游引用提示，不参与运行时校验 |
...
| `script` | scriptNodeDataDef | §4.3.11a |
...
| `script` | `script` | scriptNodeDataDef | scriptContent | GraalJS 沙箱脚本执行 |
...
| `script` | 脚本编辑器 UI | `script` | scriptContent、outputSchema、timeout |
