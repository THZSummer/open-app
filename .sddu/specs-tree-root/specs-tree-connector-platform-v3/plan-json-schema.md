...
| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `flowMode` | string | ✅ | 编排模式：`single`（单节点）/ `serial`（串行）/ `parallel`（并行）。前端渲染编排画布必需。各模式的默认拓扑与约束详见 §6.3.3.1~§6.3.3.3 |
...
##### 6.3.3.1 single — 单节点模式

**默认拓扑**：`trigger → connector → exit`（3 节点）
...
**适用**：简单 HTTP 代理 / 透传，无预处理或后处理逻辑。

##### 6.3.3.2 serial — 串行模式

**默认拓扑**：`trigger → script → connector → script → exit`（5 节点）

**删除规则**：
- 可删除中间的 **script** 或 **connector** 节点
- **trigger 和 exit 不可删除**（始终保留为端点）

**新增规则**：
- 可新增 **script** 或 **connector** 节点
- 可在 trigger 与 exit 之间任意链位置插入
- 允许多个，不限制数量
- **connector 节点总数受平台配置 `Flow.Max.Serial.Connector.Nodes` 控制**（默认 3，含默认拓扑中的 1 个 connector）

**适用**：需要数据预处理、中间转换、后处理的线性编排。

##### 6.3.3.3 parallel — 并行模式

**默认拓扑**：`trigger → parallel → exit`（3 节点）

**约束**：
- 默认 3 节点**全部锁定，不可删除**（trigger、parallel、exit 均为必需）

**扩展能力**：
- **两侧扩展**：可在 trigger → parallel 之间、parallel → exit 之间追加 script / connector 节点
- **分支扩展**：可从 parallel 节点引出多条并行分支（connector / script），各分支独立执行后汇合进入 exit（或 exit 前的汇合节点）
- **分支数量受平台配置 `Flow.Max.Parallel.Branches` 控制**（默认 8）

**适用**：多服务并行调用后聚合结果（如同时查多个 API 后汇总返回）。
...
| v9.12 | 2026-07-03 | **§6.3.3 flowMode 精确化**：拆分 §6.3.3.1~§6.3.3.3 为 single / serial / parallel 三种模式独立章节，细化每种模式的默认拓扑、节点增删规则、扩展能力；串行 connector 数量上限引用 `Flow.Max.Serial.Connector.Nodes`，并行分支数量上限引用 `Flow.Max.Parallel.Branches`（plan-config #11/#12） |
