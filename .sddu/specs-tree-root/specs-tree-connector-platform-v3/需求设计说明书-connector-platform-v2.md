...
| 2 | 连接流增强 | 多版本管理，生命周期操作独立为部署（版本绑定）、启动（状态迁移）、停止（状态迁移），三方互不耦合。版本发布需三级审批并支持一键催办，流程编排支持入站限流、超时控制、缓存、错误处理（重试/忽略/终止）及并行分支，支持一键复制 |
...
> 💡 连接流仅 4 状态。**部署**不改变状态 — 部署是纯版本绑定切换动作，在「已停止」和「运行中」状态下均可执行。**启动**是独立于部署的状态迁移动作，将「已停止」推进至「运行中」。
...
- 连接流 CRUD：新增 app_id 归属、deployed_version_id 指针、4 状态流转、恢复能力（FR-016~023）
...
| F-04 | 连接流生命周期 | 部署（版本绑定）与启动（状态迁移）完全隔离，4 状态流转 | 修改 | G5 | 连接流生命周期增强 |
...
- **版本管理**：完整 JSON 快照 + 实体内递增版本号（ADR-004）
- **审批集成**：复用开放平台审批引擎，拓展 `connector_flow_version_publish` 业务场景
- **入站限流**：Redis 令牌桶 + Lua 原子操作（ADR-005）
- **业务缓存**：Redis 全流缓存 + 版本变更异步主动清理
- **平台缓存**：四实体 Cache-Aside + TTL 7天随机抖动 + 穿透/击穿/热点三层防护
- **运行记录**：MySQL 主存储 + 30 天定时清理 + FIFO 1000 条上限（ADR-006）
- **安全准入**：市场服务 Lookup/Property 复用 + 应用级 `app_id` 数据隔离

**系统分层架构**（8 层叠加，从内到外：版本层→外部依赖层→编排层→执行层→安全层/审批层/调试通道/运维层）：
...
```mermaid
graph LR
    subgraph 前端["前端 (wecodesite)"]
        UI["React + Ant Design 4\n14 新页面"]
    end

    subgraph open-server["open-server :18080（管理面）"]
        CM["连接器管理\nCRUD + 版本 + URL白名单"]
        FM["连接流管理\nCRUD + 版本 + 生命周期\n+ 复制 + 运行记录查询"]
        AM["审批集成\n引擎扩展 + 审批人配置"]
        SM["安全准入\n应用白名单 + 准入拦截"]
    ...
    subgraph connector-api["connector-api :18180（运行时）"]
        RE["运行时引擎\n版本解析 + 并行执行\n+ 限流 + 缓存 + 日志"]
        ERR["错误处理\n重试/忽略/终止"]
        AUTH["认证注入\nCookie/DigitalSign/MultiAuth"]
        DBG_["调试执行器"]
    end
...
| 运行时引擎 | connector-api | 版本解析 + 并行执行 + 限流 + 缓存 + 日志 + 调试 + 错误处理 | 🆕 多模块 |
...
│   ├── cache/            # [NEW] 连接流缓存管理 + 平台配置缓存 + 穿透/击穿/热点防护
...
| 服务 | 端口 | 职责 | CPU/内存 | 实例数 | 接口数 |
|------|:---:|------|:----:|:--:|:--:|
| open-server | 18080 | 管理面：连接器/连接流 CRUD、版本管理、审批、安全、操作日志 | 2C4G | 2 | 54 |
| connector-api | 18180 | 运行时：HTTP 触发、调试执行、运行记录写入、健康检查 | 2C4G | 2 | 2 |
| api-server | 18081 | API 认证鉴权（V2 无变更） | — | 2 | — |
| event-server | 18083 | 事件/回调网关（V2 无变更） | — | 1 | — |
...
sequenceDiagram
    ...
    participant Cache as 缓存引擎
    participant DAG as DAG 编排引擎
    participant Connector as 连接器调用
    participant Redis as Redis
    participant MySQL as MySQL
    participant Downstream as 下游业务系统

    Client->>GW: POST /api/v1/flows/{flowId}/invoke
    GW->>GW: ① SYSTOKEN 校验 + 入站限流

    GW->>Redis: ② GET cp:entity:flow:{flowId}
    Redis-->>GW: Flow 实体（含 deployed_version_id）

    GW->>Redis: ③ GET cp:entity:flowversion:{versionId}
    Redis-->>GW: FlowVersion（含完整编排快照）

    GW->>Cache: ④ 请求 + 编排配置
    Cache->>Cache: 解析 flowConfig.cache.key[]
    
    alt 缓存命中
        Cache->>Redis: GET cp:cache:{flowId}:{ver}:{key}
        Redis-->>Cache: 缓存值
        Cache->>MySQL: 写入 execution_record (cache_status=1)
        Cache-->>Client: 返回缓存响应（P99 < 30ms）
    else 缓存未命中
        Cache->>DAG: 执行编排 DAG

        loop 遍历 DAG 节点
            DAG->>Redis: ④ GET cp:entity:connectorversion:{id}
            Redis-->>DAG: ConnectorVersion（含 connectionConfig）
            ...
            alt 数据处理节点
                ...
                DAG->>Connector: ⑤ HTTP 调用（注入认证 + 参数映射）
                Connector->>Downstream: 实际 HTTP 请求
                Downstream-->>Connector: 响应
                Connector-->>DAG: 返回结果
            end
        end

        DAG->>MySQL: ⑥ 写入 execution_record + execution_step
        DAG->>Redis: ⑦ SET 缓存键（仅成功 + ≤1MB）
        DAG-->>Client: 返回执行结果
    end
```

**关键路径延迟预算**（NFR-001，2C4G 单节点）：

| 路径 | 操作 | 延迟预算 | 说明 |
|------|------|:--:|------|
| 缓存命中路径 | 网关校验 + Redis GET×3 + 写 execution_record | P99 < 30ms | 无 DAG/HTTP 调用 |
| 缓存未命中路径 | 网关校验 + Redis GET + DAG + HTTP 下游 + 写 DB | P99 < 2s | 依赖下游响应速度 |
| 调试触发路径 | 同缓存未命中 + 独立线程池隔离 | 30s 超时 | 不计入正常指标 |
...
| FR | 名称 | 描述 |
|----|------|------|
| **FR-016** | 创建连接流 | 创建时不自动生成草稿版本，进入「已停止」状态。创建时不校验业务必填 |
| **FR-017** | 一键复制 | 连接流列表一键复制，生成独立实体（含完整版本历史），名称追加随机后缀，状态为「已停止」 |
| **FR-018** | 部署 | 选择已发布版本绑定到连接流（设置 deployed_version_id），不改变连接流状态。部署与启动完全独立 |
| **FR-019** | 启动 | 已停止的连接流可启动（需已有已部署版本），状态迁移至「运行中」。与部署完全隔离 |
| **FR-020** | 停止 | 运行中的连接流可停止，当前执行中的实例继续完成 |
| **FR-021** | 恢复连接流 | 已失效的连接流可恢复至「已停止」状态 |
| **FR-022** | 失效连接流 | 已停止的连接流可标记失效 |
...
| flow.lifecycleStatus | 1/2/3/4 | 已停止/运行中/已失效/物理删除 |
...
**#17 创建连接流** — `POST /flows`：请求体含 nameCn/nameEn/descriptionCn/descriptionEn，响应返回 flowId、lifecycleStatus=1（已停止）、appId。
...
| 3 | `flow_t` | MODIFY | flow | 扩展 lifecycle_status 4状态；新增 deployed_version_id/app_id |
...
- **flow_t（MODIFY）**：新增 `deployed_version_id` BIGINT(20)、`deployed_version_number` INT、`app_id` BIGINT(20)；`lifecycle_status` 改为 4状态（1=已停止,2=运行中,3=已失效,4=物理删除）
...
| flow_t | lifecycle_status | 1=已停止, 2=运行中, 3=已失效, 4=物理删除 |
