...
**设计模型**：类比文件系统 — `path` 相当于目录，`classify_code` 相当于一个配置文件。这个文件内是一对多的列表数据（`classify_t` 1:N `item_t`），适合存储白名单、枚举项等列表型配置。`UK(path, classify_code)` 保证同一目录下文件名唯一。

**唯一约束**：`classify_t` 有 `UK(path, classify_code)` 联合唯一索引，与 Property 的 `(path, code)` 同模式；`item_t` 有 `UK(classify_id, item_code)` 保证同一分类下编码不重复。

**path + classify_code 设计示意**：

```
openplatform_lookup_classify_t                 openplatform_lookup_item_t
┌────┬──────┬──────────────────────────────────┐    ┌────┬─────────────┬───────────┬────────────┐
│ id │ path │ classify_code                    │    │ id │ classify_id │ item_code │ item_value │
├────┼──────┼──────────────────────────────────┤    ├────┼─────────────┼───────────┼────────────┤
│ 10 │ ""   │ connector_platform_app_whitelist │─1:N→│101 │     10      │  001      │  "app_001" │
│    │      │                                  │    │102 │     10      │  002      │  "app_002" │
│    │      │                                  │    │103 │     10      │  003      │  "app_003" │
├────┼──────┼──────────────────────────────────┤    ├────┼─────────────┼───────────┼────────────┤
│ 11 │ gray │ connector_platform_app_whitelist │─1:N→│201 │     11      │  001      │  "app_001" │
│    │      │                                  │    │202 │     11      │  005      │  "app_005" │
└────┴──────┴──────────────────────────────────┘    └────┴─────────────┴───────────┴────────────┘
```
...
| 表名 | 用途 | 关键列 |
|------|------|--------|
| `openplatform_property_t` | 键值对属性 | `id` (PK, 雪花ID), `code`, `name`, `value`, `path`, `status` |
...
│ id │ code                                     │ path     │ value │
├────┼──────────────────────────────────────────┼──────────┼───────┤
│  1 │ connector_platform.max_timeout_seconds   │ ""       │  5    │ ← 平台默认
│  2 │ connector_platform.max_timeout_seconds   │ app_001  │ 10    │ ← 应用覆盖
│  3 │ connector_platform.max_timeout_seconds   │ app_002  │  8    │ ← 应用覆盖
│  4 │ connector_platform.max_qps               │ ""       │ 1000  │ ← 平台默认
│  5 │ connector_platform.log_collection_enabled│ ""       │ true  │ ← 平台默认
│  6 │ connector_platform.log_collection_enabled│ app_002  │ false │ ← 应用覆盖
└────┴──────────────────────────────────────────┴──────────┴───────┘

唯一约束 UK(path, code) 保证同一 code 下 path 不重复：
  ✅ (app_001, max_timeout_seconds) 只能有一条
  ✅ ("",        max_timeout_seconds) 只能有一条
  ❌ 不能存在两条 (app_001, max_timeout_seconds)
```

**查询逻辑**：
```
1. 先查 path = {appId} AND code = {key}  → 有则返回（应用独立值）
2. 无则查 path = '' AND code = {key}      → 返回平台默认值
3. 平台默认值也无 → 使用代码级 fallback 常量
```

**实例推演**：应用 `app_001` 查询 `max_timeout_seconds`
```
 SELECT value FROM openplatform_property_t
  WHERE code = 'connector_platform.max_timeout_seconds'
    AND path = 'app_001'              → 命中 id=2，返回 10
```
应用 `app_003` 查询 `max_timeout_seconds`（未设独立值）
```
 SELECT value FROM openplatform_property_t
  WHERE code = 'connector_platform.max_timeout_seconds'
    AND path = 'app_003'              → 未命中
                                    ↓
 SELECT value FROM openplatform_property_t
  WHERE code = 'connector_platform.max_timeout_seconds'
    AND path = ''                     → 命中 id=1，返回 5
```
...
| path（目录） | code（文件名） | value（文件内容） | 取值逻辑 | 读写方 |
|:--:|------|:--:|------|------|
| `""` | `connector_platform.max_timeout_seconds` | `5` | `min(节点值, 此值)` | 平台管理员通过 market-web 写 |
| `app_001` | `connector_platform.max_timeout_seconds` | `10` | 覆盖根目录默认值 | 同上 |
| `app_002` | `connector_platform.max_timeout_seconds` | `8` | 覆盖根目录默认值 | 同上 |
| `""` | `connector_platform.max_qps` | `1000` | `min(流配置值, 此值)` | 同上 |
| `""` | `connector_platform.max_concurrency` | `1000` | `min(流配置值, 此值)` | 同上 |
| `""` | `connector_platform.max_execution_records_per_flow` | `1000` | FIFO 清理阈值 | 同上 |
| `""` | `connector_platform.log_collection_enabled` | `true` | 运行时判断 | 平台管理员+应用管理员写 |
| `app_002` | `connector_platform.log_collection_enabled` | `false` | 覆盖根目录默认值 | 同上 |
...
| id | path | code | value | 说明 |
|----|------|------|-------|------|
| 1 | `""` | `connector_platform.max_timeout_seconds` | `5` | 平台默认超时 |
| 2 | `app_001` | `connector_platform.max_timeout_seconds` | `10` | 应用 app_001 超时上限覆盖为 10s |
| 3 | `app_002` | `connector_platform.max_timeout_seconds` | `8` | 应用 app_002 超时上限覆盖为 8s |
| 4 | `""` | `connector_platform.max_qps` | `1000` | 平台默认 QPS 上限 |
| 5 | `""` | `connector_platform.max_concurrency` | `1000` | 平台默认并发上限 |
| 6 | `""` | `connector_platform.max_execution_records_per_flow` | `1000` | 平台默认运行记录条数上限 |
| 7 | `""` | `connector_platform.log_collection_enabled` | `true` | 平台默认日志采集开启 |
| 8 | `app_002` | `connector_platform.log_collection_enabled` | `false` | 应用 app_002 关闭日志采集 |
