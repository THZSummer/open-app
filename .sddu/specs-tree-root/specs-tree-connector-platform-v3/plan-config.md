...
当前全部 16 项配置均以 **Java 硬编码常量** 形式存在于 `ConnectorPlatformConstants.java`。v2.0 已将存储方案从 Property 逐项查询迁移到 Lookup 批量读取（详见 §3）。以下逐项记录实现现状。
...
发布时 ───── 全部校验集中执行：
   ...
   ├── 并行分支数 ≤ #11 flow_max_parallel_branches (EC-027)
    ├── 脚本长度 ≤ #13 script_max_length_chars
    ├── 脚本超时 ≤ #14 script_max_timeout_seconds
   └── 连接器版本可用性 (FR-039)
```
...
| 🟢 P2 | #1~#14 全部接入 Property 动态读取 | 将硬编码常量替换为 Property，支持运行时调整 |
...
market-server 管理端无需代码改动。仅需执行 §3.7 数据初始化 SQL 写入 classify + item 默认数据。`CacheServiceV2.clearLookUpItemCache(path, classifyCode)` 已就绪，与 connector-api 缓存 key 对齐。
