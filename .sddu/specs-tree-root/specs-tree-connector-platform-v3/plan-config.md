...
> 💡 "平台限制"列指 #1~#16 中所有可配置上限项——发布时取 Property/Lookup 实际值与用户输入做对比，超出即拦截。

---

### 2.6 Property 化实施路线

> ⚠️ **v2.0 废弃提示**：本节描述的 Property 逐项查询方案已被 §3「Lookup 化优化方案」取代。v2.0 将 16 项配置全量迁移到 Lookup 存储，DB 查询次数从最多 16 次降至 2 次。本节保留作为历史方案记录，§2.1~§2.5 的配置项定义仍然有效。**新代码实现请以 §3 为准。**
...
public class ConnectorPlatformPropertyService {
    ...
    // 全局配置
    public int getConnectorMaxVersions()           // item_code=Connector.Max.Versions, default=1000
    public String getUrlRegexPattern()             // item_code=Connector.Url.Regex.Pattern, default=null
    public int getFlowMaxVersions()                // item_code=Flow.Max.Versions, default=1000

    // 按应用区分配置
    public int getConnectorConfigMaxBytes(String appId)     // item_code=Connector.Config.Max.Bytes
    public int getMaxExecutionRecordsPerFlow(String appId)  // item_code=Max.Execution.Records.Per.Flow, default=1000
    public int getNodeMaxTimeoutSeconds(String appId)       // item_code=Node.Max.Timeout.Seconds, default=5
    public int getFlowConfigMaxBytes(String appId)          // item_code=Flow.Config.Max.Bytes
    public int getFlowMaxQps(String appId)                  // item_code=Flow.Max.Qps, default=1000
    public int getFlowMaxConcurrency(String appId)          // item_code=Flow.Max.Concurrency, default=1000
    public int getFlowMaxCacheTtlSeconds(String appId)      // item_code=Flow.Max.Cache.Ttl.Seconds, default=1296000
    public int getFlowMaxParallelBranches(String appId)     // item_code=Flow.Max.Parallel.Branches, default=8
    public int getFlowMaxSerialConnectorNodes(String appId)  // item_code=Flow.Max.Serial.Connector.Nodes, default=3
    public int getScriptMaxLengthChars(String appId)        // item_code=Script.Max.Length.Chars, default=10000
    public int getScriptMaxTimeoutSeconds(String appId)     // item_code=Script.Max.Timeout.Seconds, default=5
    public boolean isLogCollectionEnabled(String appId)     // item_code=Log.Collection.Enabled, default=true
}
```
...
| 12 | `FlowPublishValidator.validateOrchestrationConfig()` | `getFlowMaxSerialConnectorNodes(appId)` |
| 13 | `FlowPublishValidator.validateOrchestrationConfig()` | `getScriptMaxLengthChars(appId)` |
| 14 | `FlowPublishValidator.validateOrchestrationConfig()` | `getScriptMaxTimeoutSeconds(appId)` |
| 15 | `ExecutionStepService` | `isLogCollectionEnabled(appId)` |
| 16 | `AppWhitelistService.isWhitelisted()` | `LookupWhitelistMapper.selectItemValuesByClassifyCode("Connector.Platform.AppWhitelist")` |
...
('connector_platform', 'connector_config_max_bytes',        '连接器配置JSON长度上限',   '0',   1),
...
('connector_platform', 'flow_config_max_bytes',             '连接流配置JSON长度上限',   '0',   1),
...
('connector_platform', 'flow_max_serial_connector_nodes',  '串行编排连接器节点数量上限', '3',    1),
('connector_platform', 'script_max_length_chars',           '脚本源码长度上限',        '10000', 1),
('connector_platform', 'script_max_timeout_seconds',        '脚本超时范围',            '5',   1),
('connector_platform', 'log_collection_enabled',            '日志采集开关',            'true', 1);
```

Lookup 初始化：
```sql
-- path=CEC.Open, classify_code=Connector.Platform.AppWhitelist
INSERT INTO openplatform_lookup_classify_t (classify_code, classify_name, path, status) VALUES
('Connector.Platform.AppWhitelist', '连接器平台开放应用范围', 'CEC.Open', 1);
```
...
| 优先级 | 项目 | 原因 |
|:---:|------|------|
| 🔴 P0 | #6 运行态超时去掉 Math.min 截断 | 按 §3.3.4，③存在用③不截断 |
| 🔴 P0 | #8 运行态 QPS 读取路径修正+去掉截断 | 从 flowConfig 读取③，不截断 |
| 🟡 P1 | #5 运行记录 FIFO 清理接入调用链 | 方法已实现但未调用，数据会无限增长 |
| 🟡 P1 | #2 URL 正则规则、#3/#7 JSON 长度、#14 脚本超时校验 | 发布时校验缺失，存在安全风险 |
| 🟡 P1 | #16 应用白名单接入 Lookup | MVP 基于 Spring 属性，需迁移 |
| 🟢 P2 | #15 日志采集开关 | 存储优化类功能 |
| 🟢 P2 | #1~#14 全部接入 Lookup 批量读取 | 已实现，详见 §3.3.4 设计态/运行态模型 |
...
| v2.1 | 2026-06-30 | 统一命名：§2/§2.5/§2.6 的 Property 残留命名全部对齐 §1 Lookup 命名（path=CEC.Open, classify_code/item_code=PascalCase）；修正默认值（#3/#7=0, #14=5）；补充遗漏的 #12（串行编排节点上限）；修正 §2.6.2 编号错位；SQL 值修正 | SDDU |

---

*最后更新: 2026-06-30 | 基于 spec.md v3.0 + plan.md v3.0*
