...
| 运行监控 | 无 | 运行记录查看 · 记录条数上限 1000 FIFO 按应用可配 + 节点日志采集 · 日志采集开关 按应用可配 |
...
| 设计态校验放宽后存量配置兼容风险 | 🟢 低 | 保存时不校验，仅温和提示；发布时统一校验（spec v2.24）；批量修复工具 |
...
| 版本发布审批 | 无 | 三级审批 + 催办 |
| 运行监控 | 无 | 运行记录 + 节点日志 · 记录条数上限 1000 FIFO 按应用可配 · 日志采集开关 按应用可配 |
