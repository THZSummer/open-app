...
**版本**: v2.0
**创建日期**: 2026-06-09
**对齐基线**: spec.md v2.24-draft

---

## SQL 脚本输出位置

V3 DDL 脚本追加到 open-server 工程内，沿用 V1 的 FlywayDB 命名风格，但 **open-server 不新增 Flyway 依赖**，本规划也**不考虑脚本自动执行机制**。
...
└── V3__connector_platform_v3_schema.sql     ← V3 新增（4 张新表 + 5 张 ALTER）
```
...
> 💡 V3 全面沿用 V1 `plan-db.md §0` 已确立的设计规范，以下各子节从 V1 完整复制并标注 V3 变更项。
...
| **软删除** | V1 不引入；V3 不引入（统一物理删除） |
| **审计字段** | 每表必备 `create_time` / `last_update_time` / `create_by` / `last_update_by` |
| **执行表分区** | V3 不分区；单表接近 500w 时按 `trigger_time` 月度分区 + 30 天定时清理 |
...
> ⚠️ **V3 新增表注意**：所有新增表均需完整 4 个审计字段（含 `connector_version_ref_t`）。
...
| 表 | 字段 | 枚举值 | 说明 |
|----|------|--------|------|
| `connector_t` | `connector_type` | 1=HTTP | 协议类型（V3 新增：2=MySQL 预留） |
| `connector_t` | `status` | 1=有效不可用, 2=有效可用, 3=已失效, 4=物理删除 | V3 启用 |
| `connector_version_t` | `status` | 1=草稿, 2=已发布, 3=已失效, 4=物理删除 | V3 启用 |
| `flow_t` | `lifecycle_status` | 1=已停止, 2=运行中, 3=已失效, 4=物理删除 | V3 扩展 |
| `flow_version_t` | `status` | 1=草稿, 2=待审批, 3=已撤回, 4=已驳回, 5=已发布, 6=已失效, 7=物理删除 | V3 新增 |
| `execution_record_t` | `trigger_type` | 1=http, 2=debug | V3 启用 |
| `execution_record_t` | `status` | 0=success, 1=failed | V3 启用 |
| `execution_record_t` | `rate_limit_status` | 0=未触发, 1=触发限流 | V3 启用 |
| `execution_step_t` | `status` | 0=success, 1=failed | 步骤执行结果 |
| `execution_step_t` | `node_type` | 1=trigger, 2=connector, 3=script, 4=parallel, 5=exit | V3 新增 |
| `execution_record_t` | `cache_status` | 0=未命中（正常执行）, 1=全流命中, 2=部分命中（V3） | V3 新增 |
| `execution_step_t` | `cache_status` | 0=未命中, 1=节点级命中（V3） | V3 预留，V3 启用 |

### 0.8 V3 规范变更（相对 V1）

| 规范项 | V1 | V3 |
|--------|-----|-----|
| 软删除 | MVP 不引入 | V3 不引入（统一物理删除） |
...
| 5 | `openplatform_v2_cp_connector_version_ref_t` | **NEW** | flow | 连接器版本引用中间表（M:N）；含 `flow_id`/`connector_id` 冗余 + `node_id` 定位编排节点 |
| 6 | `openplatform_v2_cp_execution_record_t` | NEW | runtime | 运行记录表（V1 预留 DDL 但未使用），V3 全新启用并修正枚举值 |
| 7 | `openplatform_v2_cp_execution_step_t` | NEW | runtime | 运行日志表（V1 预留 DDL 但未使用），V3 全新启用；`node_type` VARCHAR→TINYINT |
...
| 13 | `openplatform_property_t`（market-server） | REUSE | security | 系统配置（超时上限/限流上限/记录条数上限/日志开关），复用 market-server Property 能力，code 规则 `cp_` 前缀+配置项名，按 app_id 区分 |
...
> 💡 **V1→V3 关系变化**：V1 的 `flow_version_t` ⇢ `connector_version_t` 通过编排 JSON 中的 `connectorVersionId` 字段隐式引用，V3 新增 `connector_version_ref_t` 中间表显式管理，用于「标记版本失效/删除」的前置校验。

---

## 3. 表结构定义

> 💡 以下 DDL 全部遵循 §0 设计规范：BIGINT 雪花 ID 主键、TINYINT 枚举、MEDIUMTEXT 存 JSON、idx_xxx/uk_xxx 索引命名、不使用物理外键。MODIFY 表示在 V1 表基础上 ALTER；NEW 表示 V3 新建（含 V1 预留 DDL 但未实际使用的表）。

### 3.1 openplatform_v2_cp_connector_t（MODIFY）

**变更理由**：V3 引入连接器 4 状态生命周期（有效不可用/有效可用/已失效/物理删除）和应用级数据隔离。
...
| `status` | TINYINT(10) | MODIFY | V1 预留未用 → V3 启用 4 状态（与 connector_version 的发布/失效联动） |

### 3.2 openplatform_v2_cp_connector_version_t（MODIFY）

**变更理由**：V1 单版本模型（1:1，`uk_connector_id` 唯一约束）→ V3 多版本模型（1:N），需要移除唯一约束并新增版本号、状态、发布时间字段。
...
| `idx_connector_id` | — | DROP | 移除 1:1 约束，允许多版本 |

### 3.3 openplatform_v2_cp_flow_t（MODIFY）

**变更理由**：V3 引入连接流 4 状态生命周期（已停止⇄运行中→已失效→物理删除）、部署版本指针、应用归属。部署不改变状态，仅切换版本绑定。
...
| `app_id` | BIGINT(20) | NEW | 归属应用 ID，实现 G13 应用数据隔离 |
| `lifecycle_status` | TINYINT(10) | MODIFY | V1: 1=running, 2=stopped → V3: 4 状态（1=已停止, 2=运行中, 3=已失效, 4=物理删除） |

### 3.4 openplatform_v2_cp_flow_version_t（MODIFY）

**变更理由**：V1 单版本模型 → V3 多版本模型，新增 7 状态生命周期（含审批中间状态）。流级配置（超时/限流/缓存）内嵌在 `orchestration_config` JSON 中，不单独建列。
...
| `idx_flow_id` | — | DROP | 移除 1:1 约束 |

### 3.5 openplatform_v2_cp_connector_version_ref_t（NEW）

**建表理由**：编排中 connector 节点引用特定 ConnectorVersion，需要显式管理引用关系以支持「标记版本失效/删除」的前置「被引用」校验（ADR-007）。`node_id` 定位流程编排中的具体连接器节点，`flow_id`/`connector_id` 冗余避免 JOIN 穿透版本表。编排保存时同步写入，编排删除时级联清理。
...
| `node_id` | VARCHAR(64) | 流程编排中的连接器节点 ID（React Flow node.id），同一流版本可包含多个连接器节点 |
...
**建表理由**：V1 预留 DDL 但未实际使用，V3 全新启用。运行记录仅存储列表查询所需字段（元数据），节点级 I/O 详情全部走 `execution_step_t`。`flow_version_snapshot` 冗余完整版本快照（编排拓扑 + 流级配置），确保版本删除后仍可还原执行现场。
...
**建表理由**：V1 预留 DDL 但未实际使用，V3 全新启用。步骤表专注记录每个节点的执行现场（输入/输出/耗时/状态），拓扑还原依赖 `execution_record_t.flow_version_snapshot` 中的 `nodes[]` + `edges[]`。`iteration` 预留循环场景。
...
| `node_type` | TINYINT(10) | 1=trigger, 2=connector, 3=script, 4=parallel, 5=exit |
| `iteration` | INT | 循环轮次（预留），默认 0 |
| `status` | TINYINT(10) | 0=success, 1=failed |
| `cache_status` | TINYINT(10) | 0=未命中, 1=节点级命中（V3 启用，V3 始终为 0） |
| `cache_key` | VARCHAR(500) | 命中的节点级缓存键（V3 启用，V3 始终 NULL） |
| `cache_ttl_remaining` | INT | 命中时缓存剩余 TTL 秒（V3 启用，V3 始终 NULL） |
...
> 💡 V3 连接流仅 4 状态，部署不改变状态。创建连接流默认进入「已停止」(1)，需独立执行部署（绑定版本）和启动（状态迁移）后方可响应触发。
...
| 1 | trigger | 触发器节点 |
| 2 | connector | 连接器节点 |
| 3 | script | 脚本节点（GraalJS 沙箱） |
| 4 | parallel | 并行处理节点（FR-038a） |
| 5 | exit | 出口节点 |

### 4.8 execution_record_t.cache_status（V3 新增）
...
### 4.9 execution_step_t.cache_status（V3 预留，V3 启用）

| 值 | 含义 | 说明 |
|:--:|------|------|
| 0 | 未命中 | 该节点正常执行（V3 始终为 0） |
...
V1 为内部验证 MVP，无真实用户数据，V3 **不执行数据迁移**：

- 已存在的 V1 表（connector_t / connector_version_t / flow_t / flow_version_t）：通过 §3 的 `ALTER TABLE` 变更
- V3 新建表（connector_version_ref_t）：通过 `CREATE TABLE` 新建
- V1 预留表（execution_record_t / execution_step_t）：V1 仅保留 DDL 未实际使用，V3 全新 `CREATE TABLE`（含完整修正）
- 初始数据（审批流模板等）在现有平台手动注册，不需要脚本

> 💡 无需编写 V1→V3 数据回填 SQL，也无需备份 V1 数据。
...
V3 启用 `execution_record_t` / `execution_step_t` 写入，需配套清理策略（ADR-006）。
...
### 6.1a 运行记录条数上限与 FIFO 清理

| 策略项 | 说明 |
|--------|------|
| 上限 | 每连接流最大 **1000 条**（平台默认，按应用可配，复用 market-server Property） |
| 触发时机 | 每次 `execution_record_t` 新写入成功后 |
| 清理方式 | 检查该 `flow_id` 的记录数，超过上限时按 `create_time ASC` 批量 DELETE 最早的多余记录 |
| 与时间清理关系 | 与 §6.1 的 30 天时间清理策略独立互补 |
...
| `connector_version_ref_t` | — | **V3 全新建表** |
| `approval_flow_t` | V1 能力开放平台 | ALTER：新增 `app_id`；uk_code → uk_code_app；新增 businessType 模板 |
| `execution_record_t` | V1 预留 DDL（未使用） | NEW：V3 全新 `CREATE TABLE`，修正枚举值、审计字段 |
| `execution_step_t` | V1 预留 DDL（未使用） | NEW：V3 全新 `CREATE TABLE`；`node_type` VARCHAR→TINYINT |
