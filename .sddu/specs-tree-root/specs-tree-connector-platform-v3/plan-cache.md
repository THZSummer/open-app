...
> 💡 本文档分为两部分：**Part 1（§1-§11）— 业务缓存**：用户配置的 `flowConfig.cache`，缓存连接流执行结果。**Part 2（§12）— 平台配置缓存**：系统自动管理的实体对象缓存，加速运行时配置读取。
...
| 节点级缓存 | 缓存单个连接器节点的返回结果 | ❌ | 复杂度高，需处理节点间依赖和缓存一致性；V3 暂不引入 |
| 子图级缓存 | 管理员选择缓存 DAG 中某段子图 | ❌ | 需前端支持子图选择交互，超出 V3 scope |
...
**背景**：
V3 需要在连接流级别引入缓存能力（FR-037），通过缓存子图执行结果减少下游重复调用、提升响应速度。需决定缓存粒度、存储引擎、键设计、失效策略。
...
| 校验项 | 校验时机 | 违规处理 |
|--------|---------|---------|
| `cache.key[]` 不为空数组 | 编排保存 | 温和提示（不禁用保存）「缓存键至少需要一个表达式」 |
| `cache.key[]` 每个元素符合表达式语法 | 编排保存 | 温和提示（不禁用保存）具体语法错误 |
| `cache.ttl` ≥ 1 | 编排保存 | 温和提示（不禁用保存）「缓存时长最小为 1 秒」 |
| `cache.key[]` 表达式引用非 trigger.input | 编排保存 | WARN 提示「建议使用触发器输入字段作为缓存键」 |
| `cache.key[]` 表达式数量 > 5 | 编排保存 | WARN 提示「缓存键过多可能影响匹配效率」 |
| `cache.ttl` ≤ 1296000 | 提交发布 (FR-026) | 禁止提交，提示「缓存时长最大 15 天」（EC-026） |
...
> 💡 **范围约束**：平台配置缓存仅服务于 **connector-api 运行时** 的读操作。**open-server 管理面不读缓存**，仅负责在实体生命周期变更时**写入/清理缓存**。

### 12.1 为什么需要平台配置缓存

connector-api 每次处理 HTTP 触发请求时，存在以下**必经的 MySQL 读操作**：

```
每次 HTTP 触发请求：
  ┌─ ① 读 flow_t                  → 取 deployed_version_id
  ├─ ② 读 flow_version_t           → 取 orchestration_config（MEDIUMTEXT，可达数百 KB）
  └─ ③ 遍历 DAG 中每个 connector 节点：
       ├─ ④ 读 connector_version_ref_t → 取 connector_version_id
       └─ ⑤ 读 connector_version_t     → 取 connection_config（MEDIUMTEXT，含认证/URL白名单等）
```

以 NFR-001 的 300 TPS 为目标，每次请求 1+N 次 MEDIUMTEXT 读是不可接受的。**平台配置缓存将这些不变/缓变的数据放入 Redis，运行时优先命中缓存。**

### 12.2 缓存对象

缓存四张核心表的**完整实体对象**（含状态字段），序列化为 JSON。这样运行时拿到缓存对象后**无需再回查 MySQL 做任何状态校验**。

| # | 实体 | 缓存键 | 来源表 | 核心字段 |
|:--:|------|--------|--------|---------|
| 1 | Flow | `cp:entity:flow:{flowId}` | `flow_t` | `id`, `lifecycle_status`, `deployed_version_id`, `deployed_version_number`, `app_id`, `name_cn`, `name_en` |
| 2 | FlowVersion | `cp:entity:flowversion:{versionId}` | `flow_version_t` | `id`, `flow_id`, `version_number`, `status`, **`orchestration_config`**（MEDIUMTEXT）, `published_time`, `published_by` |
...
| 创建连接流 | `SET` | 写入新实体 |
| 更新基本信息 | `SET` | 覆盖更新 |
| 部署（绑定版本） | `SET` | `deployed_version_id` 变更 |
| 启动 / 停止 | `SET` | `lifecycle_status` 变更 |
| 标记失效 / 恢复 | `SET` | 状态流转 |
| 物理删除 | `DEL` | 移除缓存 |

#### 12.4.2 FlowVersion 缓存

| 事件（open-server） | 缓存操作 | 说明 |
|------|:--:|------|
| 审批通过（草稿→已发布） | `SET` | **核心写入点**——运行时依赖此缓存读取 `orchestration_config` |
| 提交审批 / 撤回 / 驳回 | `SET` | 状态变更（非运行时关注，但保持缓存一致） |
| 标记失效 | `SET`（可选）或 `DEL` | 失效版本不应被运行时读取，建议直接 `DEL` |
| 恢复版本 | `SET` | 恢复后可重新部署 |
| 物理删除 | `DEL` | 移除缓存 |
...
```
connector-api 收到 HTTP 触发请求
    │
    ▼
① GET cp:entity:flow:{flowId}
    ├── 命中 → 取 deployed_version_id（跳过 MySQL）
    └── 未命中 → 读 MySQL → SET 缓存 → 继续
    │
    ▼
② GET cp:entity:flowversion:{deployedVersionId}
    ├── 命中 → 取 orchestration_config（含 nodes/edges/flowConfig，跳过 MEDIUMTEXT MySQL 读）
    └── 未命中 → 读 MySQL → SET 缓存 → 继续
    │
    ▼
③ 遍历 DAG nodes，对每个 connector 节点：
    └── 直接从 node.data.connectorVersionConfig 快照取连接器配置（含 protocol/auth/input/output/timeoutMs）——编排自包含，无需查询 ConnectorVersion
    │
    ▼
④ 执行 DAG（已持有全部配置）
```

> 💡 理想情况下（全命中），一次 HTTP 触发请求只需 **1 次 Redis 批量读取**（`MGET` 多个 key），完全跳过 MySQL。

### 12.6 缓存预热

| 策略 | 说明 |
|------|------|
| **部署时预热** | 连接流部署时，open-server 在写入 `deployed_version_id` 后立即 `SET` FlowVersion 缓存（含完整的 connectorVersionConfig 快照） |
...
```
                         ...
                         │           Redis                   │
                         │                                  │
connector-api 运行时 ───▶│ cp:entity:*          平台配置缓存 │◀─── open-server 写入/清理
  （读 + 写业务缓存）     │ cp:cache:*           业务结果缓存  │     （仅清理业务缓存）
                         │ cp:config:*          限流令牌桶   │
                         └──────────────────────────────────┘
```

| 维度 | 业务缓存（Part 1） | 平台配置缓存（Part 2） |
|------|-------------------|----------------------|
...
| MODIFY | `open-server/.../service/FlowDeployService.java` | 部署时 SET Flow 缓存 + 预热 FlowVersion/ConnectorVersion；发送异步业务缓存清理消息 |
| MODIFY | `open-server/.../service/FlowService.java` | Flow 状态变更时同步 SET/DEL 缓存 |
| MODIFY | `open-server/.../service/ConnectorService.java` | Connector 状态变更时同步 SET/DEL 缓存 |
| — | — | **基础设施** | — |
| MODIFY | `connector-api/.../config/RedisConfig.java` | Redis 连接配置（复用已有） |
