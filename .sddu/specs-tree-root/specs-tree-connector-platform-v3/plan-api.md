...
sequenceDiagram
    ...
    participant OS as "open-server #51<br/>DebugController"
    participant CA as "connector-api #53<br/>DebugExecutionService"
    ...
    Note over CA: ③ connector-api 内部执行<br/>（详见 #53 执行时序）
...
> 💡 **与 #54（生产调用）的关键区别**：
> - **版本来源**：#51 直接按 `versionId` 加载 FlowVersion（草稿/已发布均可），不经过 `deployed_version_id`
...
```mermaid
sequenceDiagram
    participant OS as "open-server #51"
...
sequenceDiagram
    ...
    participant CA as "connector-api #54<br/>FlowRuntimeEngine"
    participant VCR as "VersionConfigResolver"
    participant DB as "MySQL"
...
> 💡 **与 #53（调试）的关键区别**：
> - **版本来源**：#54 走 `flow_t.deployed_version_id`，仅已部署版本生效
