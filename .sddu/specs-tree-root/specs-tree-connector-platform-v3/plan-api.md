...
| **服务归属** | open-server（管理面 55 个） + connector-api（运行时 2 个，其中 #55 采用透明穿透模式） | plan.md §1 |
| 端点总数 | **57**（open-server 55 + connector-api 2） | — |
...
| 应用隔离 | open-server 管理面接口（#1~#53）统一通过 `Header: X-App-Id` 传递，三层校验：白名单准入 → 用户权限 → 数据归属<br>connector-api 运行时（#54~#55）从 flow 自动获取 |
...
> ⚠️ **例外**：connector-api 运行时 `#55 调用连接流` 采用**透明穿透**模式，不使用标准信封，详见 §3.9 #55 设计理念。

所有 open-server 管理面接口（#1~#53）统一使用以下响应格式：
...
> ⚠️ **应用隔离**：以下 open-server 管理面接口（#1~#53）统一通过 `Header: X-App-Id` 传递应用 ID，三层校验：
...
> connector-api 运行时（#54~#55）从 `flow_t.app_id` 自动获取，无需传入。
>
...
| — | POST | `/flows/{flowId}/test-run` | 测试运行 | 删除 | ⑤ V1 接口，V3 由 #52 替代 |
...
| — | — | **open-server — 审批流模板配置** | — | — | — |
| 45 | GET | `/approval-flows` | 查询审批流模板列表 | 改造 | ② 查询参数新增 `?appId`（外部业务标识） |
| 46 | GET | `/approval-flows/{id}` | 查询审批流模板详情 | 改造 | ② 响应新增 `appId` 字段（外部业务标识） |
| 47 | POST | `/approval-flows` | 创建审批流模板 | 改造 | ② 请求体新增 `appId` 字段<br>② `appId` 为外部业务标识 |
| 48 | PUT | `/approval-flows/{id}` | 更新审批流模板 | 改造 | ② 请求体新增 `appId` 字段 |
| 49 | DELETE | `/approval-flows/{id}` | 删除审批流模板 | 存量 | — |
| — | — | **open-server — 运行记录** | — | — | — |
| 50 | GET | `/executions` | 查询运行记录列表 | 新增 | ① 三层权限校验<br>④ 新增接口 |
| 51 | GET | `/executions/{executionId}` | 查询运行记录详情 | 新增 | ① 三层权限校验<br>④ 新增接口 |
| — | — | **open-server — 调试代理** | — | — | — |
| 52 | POST | `/flows/{flowId}/versions/{versionId}/debug` | 调试连接流版本（代理） | 新增 | ① 三层权限校验<br>④ 新增接口<br>⑥ 替换 V1 test-run |
| — | — | **open-server — 函数列表** | — | — | — |
| 53 | GET | `/data-processor/functions` | 查询数据处理函数列表 | 新增 | ① 三层权限校验<br>④ 新增接口<br>⚠️ V3 本期不实现，函数列表在 open-server 侧静态维护，后续可通过 market-server Property 扩展为动态配置 |

> ⚠️ **#53 V3 本期不实现**：数据处理函数列表（toString/toNumber/toBoolean/formatDate）在 open-server 侧静态维护，不暴露为独立端点。后续可通过 `market-server` Property 扩展为动态配置。前端使用方直接硬编码这四个函数。
| — | — | **connector-api — 运行时** | — | — | — |
| 54 | POST | `/flows/{flowId}/versions/{versionId}/debug` | 调试执行 | 新增 | ④ 新增接口<br>（由 open-server #52 代理调用） |
| 55 | POST | `/flows/{flowId}/invoke` | 调用连接流 | 改造 | ③ 路径变更<br>⑥ 替换 V1 trigger invoke<br>⚠️ **透明穿透模式**：请求/响应均由用户自定义（触发器入参 + 出口出参），平台元数据放入 `X-` 响应头，不使用标准响应信封 |
...
> 💡 **#45~#49** 是审批流模板 CRUD 接口，归属 `approvalflow` 包（`ApprovalFlowController` + `ApprovalFlowService`）。`appId` 字段为外部业务应用标识（`App.appId`），`null` = 平台级/全局模板。

**端点统计**：新增 36 + 改造 16 + 删除 5 = 57 个（open-server 55 + connector-api 2）。connector-api #55 采用透明穿透模式（请求/响应用户自定义，平台元数据 X- 响应头）。各接口 FR 对应关系见 [spec.md §A](./spec.md#a-需求追溯)。
...
> 💡 #45~#49 是审批流模板 CRUD 接口，V3 改造：新增 `appId` 字段实现应用级审批人隔离。控制器归属 `approvalflow` 包（`ApprovalFlowController`），Service 归属 `ApprovalFlowService`。
>
> **`appId` 数据流**：
> - **API 层**：`appId` 为外部业务应用标识（`App.appId`，String 类型），`null` = 平台级/全局模板
> - **存储层**：`approval_flow_t.app_id` 存储 `App.id`（内部主键 Long），NULL = 平台级
> - **查询返回**：通过 LEFT JOIN `openplatform_app_t` 直接返回外部业务 `appId`

#### #45 查询审批流模板列表

`GET /approval-flows`

> 🔧 **V3 改造**：查询参数新增 `?appId`（外部业务标识），支持按应用过滤。不传返回全部（平台级 + 应用级）。

**查询参数**

| 字段 | 类型 | 必填 | 说明 |
|------|------|:--:|------|
| appId | string | ❌ | V3 新增，外部业务应用 ID，`null`/不传=返回全部 |
| keyword | string | ❌ | 关键词（名称/编码模糊搜索） |
| curPage | int | ❌ | 页码，默认 1 |
| pageSize | int | ❌ | 每页数量，默认 20 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id | string | 模板 ID（雪花ID） |
| nameCn | string | 中文名称 |
| nameEn | string | 英文名称 |
| code | string | 流程编码 |
| appId | string | V3 新增，外部业务应用 ID（`null`=平台级/全局模板） |
| status | int | `0` 禁用 / `1` 启用 |
| nodes[] | array | 审批节点配置，见 `ApprovalNodeDto` |

**示例**

```json
// Query: ?appId=app-001&keyword=发布&curPage=1&pageSize=20
  ...
  "data": [
    {
      ...
      "status": 1,
      "nodes": [
        { "type": "approver", "userId": "user_lisi", "userName": "李四", "order": 1 },
        { "type": "approver", "userId": "user_wangwu", "userName": "王五", "order": 2 }
      ]
    },
    {
      "id": "2222222222222222222",
      "nameCn": "全局审批",
      "nameEn": "Global Approval",
      "code": "global",
      "appId": null,
      "status": 1,
      "nodes": [
        { "type": "approver", "userId": "user_admin", "userName": "管理员", "order": 1 }
      ]
    }
  ],
  "page": { "curPage": 1, "pageSize": 20, "total": 2 }
}
```
...
| appId | string | V3 新增，外部业务应用 ID（`null`=平台级/全局模板） |
| status | int | `0` 禁用 / `1` 启用 |
| nodes[] | array | 审批节点，见 `ApprovalNodeDto` |
...
> 🔧 **V3 改造**：请求体新增 `appId`（外部业务标识）字段。不传/传空 = 创建平台级/全局模板。
...
| nameCn | string | ✅ | 中文名称 |
| nameEn | string | ✅ | 英文名称 |
| code | string | ✅ | 流程编码，全局唯一（同 appId 下） |
| appId | string | ❌ | V3 新增，外部业务应用 ID，不传=平台级/全局模板 |
| nodes[] | array | ✅ | 审批节点配置 |
| nodes[].type | string | ✅ | 节点类型，固定 `"approver"` |
| nodes[].userId | string | ✅ | 审批人 ID |
| nodes[].userName | string | ✅ | 审批人姓名 |
| nodes[].order | int | ✅ | 审批顺序，从 1 开始 |
...
> 🔧 **V3 改造**：请求体新增 `appId`（外部业务标识）字段。传空 = 清除应用归属（变为平台级模板）。
...
| nameEn | string | ❌ | 英文名称 |
| code | string | ❌ | 流程编码 |
| appId | string | ❌ | V3 新增，外部业务应用 ID，传空=清除归属变为平台级 |
| nodes[] | array | ❌ | 审批节点配置 |
...
| code | 说明 |
|------|------|
| 404 | 模板不存在，或 `appId` 对应的应用不存在 |
| 409 | `code` + `appId` 组合已存在（排除自身） |
...
// 请求体 — 修改审批人和归属应用
{
  ...
  "appId": "app-002",
  "nodes": [
    { "type": "approver", "userId": "user_zhao", "userName": "赵六", "order": 1 }
  ]
}
...
前端调用 open-server，open-server 代理转发到 connector-api #54。
...
sequenceDiagram
    ...
    participant OS as "open-server #52<br/>DebugController"
    participant CA as "connector-api #54<br/>DebugExecutionService"
    participant DB as "MySQL"

    FE->>OS: POST /flows/{flowId}/versions/{versionId}/debug<br/>{ triggerData }

    Note over OS: ① 校验
    OS->>OS: 校验 X-App-Id / 用户权限
    OS->>DB: SELECT flow_version_t WHERE id={versionId}
    DB-->>OS: FlowVersion row
    OS->>OS: 校验 version.status ≠ 已失效<br/>（草稿/已发布均可调试）

    Note over OS,CA: ② 代理转发
    OS->>CA: POST /api/v1/flows/{flowId}/versions/{versionId}/debug<br/>{ triggerData }

    Note over CA: ③ connector-api 内部执行<br/>（详见 #54 执行时序）
    CA-->>OS: { executionId, status, durationMs, nodes[] }

    Note over OS: ④ 透传响应
    OS-->>FE: { executionId, status, durationMs, nodes[] }
```

> 💡 **与 #55（生产调用）的关键区别**：
> - **版本来源**：#52 直接按 `versionId` 加载 FlowVersion（草稿/已发布均可），不经过 `deployed_version_id`
> - **跳过限流与缓存**：调试模式下入站限流和响应缓存均不生效
> - **同步返回**：调试请求等待完整 DAG 执行完毕才返回

---

### 3.8a 函数列表（#53）— open-server
...
### 3.9 运行时（#54~#55）— connector-api

#### #54 调试连接流版本

<p>

由 open-server #52 代理调用，不直接暴露给前端。
...
```mermaid
sequenceDiagram
    participant OS as "open-server #52"
    participant CA as "DebugExecutionService"
    ...
    CA->>DAG: execute(resolvedConfig, triggerData)
    loop 遍历编排节点
        DAG->>DAG: 过滤 isStructural 边 + text 节点
        DAG->>DAG: 按 Kahn 拓扑排序
        alt trigger 节点
...
> 💡 **调试模式特性**：
> - 直接按 `versionId` 加载 FlowVersion，不依赖 `deployed_version_id`
> - 入站限流和响应缓存均跳过
> - 草稿和已发布版本均可调试，已失效版本拒绝
> - 同步等待完整 DAG 执行完毕才返回
...
`#55` 接口采用**透明穿透**模式——除了接口地址（URL 路径）由平台固定外，**请求和响应的所有参数、返回值均由用户在连接流中自定义**：
...
> 💡 此模式下，`#55` 接口**不使用**标准 `{ code, messageZh, messageEn, data, page }` 响应信封。响应 Body 即为出口节点的出参数据，响应 Header 中含平台 `X-` 前缀元数据。
...
sequenceDiagram
    ...
    participant CA as "connector-api #55<br/>FlowRuntimeEngine"
    participant VCR as "VersionConfigResolver"
    participant DB as "MySQL"
    participant LIMITER as "InboundRateLimiter"
    participant CACHE as "FlowCacheManager"
    participant DAG as "DAG 调度器"
    participant CONN as "外部 API"

    EXT->>CA: POST /api/v1/flows/{flowId}/invoke<br/>（Header / Query / Body 由触发器 input 定义）

    Note over CA,VCR: ① 版本配置解析
    CA->>VCR: resolve(flowId)
    VCR->>DB: SELECT flow_t WHERE id={flowId}
    ...
    CA->>LIMITER: check(flowConfig.rateLimitConfig)
    LIMITER-->>CA: 放行（超限则 429）

    Note over CA,CACHE: ③ 缓存检查
    CA->>CACHE: check(flowConfig.cache)
    alt 缓存命中
        CACHE-->>CA: 返回缓存
        CA-->>EXT: 响应（X-Cache: HIT）
    else 未命中
        CACHE-->>CA: 继续执行

        Note over CA,CONN: ④ DAG 执行
        CA->>DAG: execute(resolvedConfig, triggerData)
        loop 遍历编排节点
            DAG->>DAG: Kahn 拓扑排序
            alt connector 节点
                ...
                DAG->>DAG: 构造响应 Header + Body
            end
        end
        DAG-->>CA: 执行结果

        Note over CA: ⑤ 回写缓存 + 响应
        CA->>CACHE: SET（如有缓存配置）
        CA-->>EXT: 响应 Body（出口节点 output）<br/>+ X- 前缀平台元数据
    end
```

> 💡 **与 #54（调试）的关键区别**：
> - **版本来源**：#55 走 `flow_t.deployed_version_id`，仅已部署版本生效
> - **入站限流**：生产调用受 flowConfig.rateLimitConfig 管控
> - **响应缓存**：生产调用支持 flowConfig.cache 缓存命中
> - **响应格式**：透明穿透模式——Body 为出口节点 output，平台元数据在 X- 响应头

---

### 3.9a 系统配置（#55~#56）— open-server（复用 market-server Property）
