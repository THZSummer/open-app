...
- **连接流**：配置支持多版本管理，生命周期增强为部署→启动→停止，版本发布需经过三级审批并支持一键催办，流程编排支持限流、超时控制、缓存、错误处理及并行分支，节点间数据支持字段类型转换
...
| FR-022 | SYSTOKEN 白名单 | V1 无。V2 新增：触发器节点选择 SYSTOKEN 认证类型后，配置允许触发当前连接流的 SYSTOKEN 凭证白名单 |
...
| OQ-006 | 版本发布审批对接开放平台审批流程的改造范围 | 审批集成复杂度 | Plan 阶段 |
