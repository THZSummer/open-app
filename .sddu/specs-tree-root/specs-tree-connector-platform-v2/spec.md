...
| 状态 | 触发动作 | 允许操作 | 关键约束 |
|------|---------|---------|---------|
| **草稿 (Draft)** | 编辑连接配置时自动生成 | 保存（覆盖）、发布 | 每连接器**仅一个草稿**，再次编辑覆盖当前草稿 (EC-008)；草稿配置为空时禁止发布 (EC-009) |
| **已发布 (Published)** | 草稿发布 | 查看、被连接流引用、标记失效 | 发布时**快照完整连接配置**（协议/地址/认证/入参出参/超时），生成不可变版本；多已发布版本**并行共存** (FR-002) |
...
| 状态 | 含义 | 下一跳 | 关键约束 |
|------|------|--------|---------|
| **草稿 (Draft)** | 编辑中，未提交或已从撤回/驳回回到编辑态 | → 提交审批 | 每连接流**仅一个草稿**；编排为空时禁止提交审批 (EC-010) |
...
| 比较维度 | 撤回 (Withdrawn) | 驳回 (Rejected) |
|---------|------------------|-----------------|
| **执行人** | 提交人（应用管理员） | 审批人 |
| **发生时机** | 审批中任意阶段 | 审批中任意一级 |
| **审批记录** | ✅ 已产生部分审批记录 | ✅ 已产生审批记录 + 驳回原因 |
| **回到 Draft 的条件** | 需显式执行「重新编辑」 | 需显式执行「修改后重提」 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| **FR-001** | 创建草稿 | V1 编辑即生效。V2 新增：编辑连接配置时自动生成草稿版本，不影响已发布版本 | ① 点击「编辑配置」进入编辑态，系统自动创建草稿版本（若无）或打开已有草稿<br>② 草稿保存覆盖当前草稿内容，不产生新版本号<br>③ 每连接器同时最多一个草稿，再次编辑复用已有草稿（EC-008）<br>④ 草稿不影响运行中的连接流——它们仍使用各自引用的已发布版本 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| **FR-009** | 创建草稿 | V1 无版本概念（编排配置编辑即生效）。V2 新增：编辑编排配置生成草稿版本，保存覆盖当前草稿 | ① 点击「编辑编排」进入编排画布，系统自动创建草稿版本（若无）或打开已有草稿<br>② 草稿保存覆盖当前草稿内容（nodes + edges + flowConfig），不产生新版本号<br>③ 每连接流同时最多一个草稿<br>④ 草稿不影响当前部署的运行中版本 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| **FR-033** | 操作日志 | V1 无变更审计。V2 新增：连接器、连接流增删改、启停等变更操作记录日志，复用应用现有操作日志模块 | ① 记录的操作类型包括：连接器：创建、编辑基本信息、删除、发布版本、失效版本、删除版本<br>连接流：创建、编辑基本信息、删除、部署、启动、停止、发布版本、失效版本、删除版本、复制<br>审批域：提交审批、审批通过、审批驳回、撤回审批<br>② 每条日志包含：操作人、操作时间、操作类型、操作对象（连接器/连接流/版本 ID）、变更前/后快照（关键字段）<br>③ 日志查看入口：连接器/连接流详情页提供「操作日志」Tab<br>④ 复用应用现有操作日志模块，不重新实现日志存储和查询
