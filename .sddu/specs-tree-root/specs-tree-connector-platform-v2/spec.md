...
- **连接流**：配置支持多版本管理，生命周期增强为部署→启动→停止，部署需经过审批并支持一键催办，流程编排支持限流、超时控制、SYSTOKEN 白名单校验、缓存、错误处理及并行分支，节点间数据支持字段类型转换
...
| **G8** | **连接流：流程配置增强** — 编排支持连接器节点超时可配、连接流自身触发限流、**SYSTOKEN 凭证白名单校验**、错误处理、串行/并行分支（仅并行，无条件和循环）；连接器节点可选引用版本；新增数据处理节点 |
...
| US-06 | 创建和管理连接流，管理配置的多版本（草稿→发布→失效→删除），执行部署→启动→停止 | G4、G5 |
| US-07 | 提交连接流部署审批，支持一键催办 | G6、G7 |
...
| FR | 名称 | 描述 |
|----|------|------|
| FR-014 | 部署 | V1 编辑保存即视为部署。V2 增强：选择已发布版本提交部署（需审批），状态「待部署」→ 审批通过后可启动 |
...
| FR | 名称 | 描述 |
|----|------|------|
| FR-017 | 提交审批 | V1 无审批。V2 新增：部署时提交审批，复用开放平台审批流程能力 |
| FR-018 | 审批人配置 | V2 新增：平台管理员按应用维度配置部署审批人 |
| FR-019 | 一键催办 | V2 新增：部署审批支持一键催办，复用开放平台催办能力 |
...
| FR-022 | SYSTOKEN 白名单 | V1 无。V2 新增：在 flowConfig 中配置允许触发当前连接流的 SYSTOKEN 凭证白名单 |
...
| ID | 需求 | 目标值 |
|----|------|--------|
| NFR-001 | 连接器目录查询响应 | P99 < 200ms（沿用 V1） |
| NFR-002 | 连接器搜索响应 | P99 < 500ms |
| NFR-003 | 连接流列表查询响应 | P99 < 200ms（沿用 V1） |
| NFR-004 | HTTP 触发到执行开始延迟 | P99 < 2s（沿用 V1） |
| NFR-005 | 版本历史查询响应 | P99 < 300ms |
| NFR-006 | 版本创建/发布响应 | P99 < 1s |
| NFR-007 | 运行记录查询响应 | P99 < 500ms |
| NFR-008 | 运行日志查询响应 | P99 < 1s |
| NFR-009 | 系统可用性 | ≥ 99.9%（沿用 V1） |
| NFR-010 | 单连接流并发执行 | ≥ 10 并发实例（沿用 V1） |
...
| NFR-015 | 审计日志 | 沿用 V1 + 新增：版本发布/失效、部署审批、操作日志记录 |

### 4.3 兼容性要求

| ID | 需求 | 描述 |
|----|------|------|
| NFR-016 | V1 兼容 | V1 已创建的连接器和连接流可正常使用；单版本配置自动迁移为 V2 的第一个已发布版本 |
...
| 运行日志 | 无 | 节点输入/输出日志采集 |
| 审批 | 无 | 部署审批 + 一键催办 |
| 安全 | 无 | URL 正则白名单、SYSTOKEN 白名单、应用白名单准入 |
...
| 连接流运行配置引擎 | 解析 flowConfig（超时、限流、SYSTOKEN 白名单、缓存） |
| 缓存服务 | 按 flowConfig 缓存配置管理缓存键和 TTL |
| 审批集成适配器 | 对接开放平台审批流程，处理部署审批和催办 |
| 运行记录服务 | 执行记录的写入、查询 |
...
| 连接流生命周期 API | 部署（提交审批）、启动、停止 | V2 增强 |
| 部署审批 API | 审批提交、催办、审批人配置 | V2 新增 |
| 编排配置 API | flowConfig（超时/限流/SYSTOKEN/缓存）、节点编排 | V2 增强 |
...
| 连接流版本历史页 | 版本列表、详情、失效/删除、版本切换查看 |
| 编排画布（增强） | 串行/并行边切换、连接器版本选择、数据处理节点、超时设置 |
| 连接流配置面板 | flowConfig（限流、SYSTOKEN 白名单、缓存） |
| 部署审批页 | 提交审批、审批状态查看、一键催办 |
| 运行记录页 | 运行记录列表 |
| 运行日志页 | 按执行实例查询节点日志 |
...
| EC-003 | 连接流部署审批超时未处理 | 部署保持「待部署」状态，可催办 |
...
| EC-011 | SYSTOKEN 白名单为空 | 默认无白名单校验，所有凭证可触发 |
...
| OQ-002 | V1 数据迁移方案：单版本如何迁移为多版本 | V1→V2 兼容性 | Plan 阶段 |
| OQ-003 | 版本号策略：SemVer vs 递增序号 | 版本标识体系 | Plan 阶段 |
| OQ-004 | 限流实现方案：V1 Bucket4j 替换为 Redis 原生限流 | 出站/入站限流实现 | Plan 阶段 |
| OQ-005 | 多版本并存时限流配置值的选取策略 | 运行时限流行为 | Plan 阶段 |
| OQ-006 | 部署审批对接开放平台审批流程的改造范围 | 审批集成复杂度 | Plan 阶段 |
| OQ-007 | 缓存一致性策略：版本变更后缓存处理 | 缓存可靠性 | Plan 阶段 |
| OQ-008 | 运行记录和日志的存储方案：MySQL vs 独立存储 | 查询性能和数据量 | Plan 阶段 |
...
| 审批落地 | 连接流部署需审批通过，支持催办 | G6、G7 |
| 编排增强 | 串行/并行连接 + 数据处理节点 + 节点超时 | G8、G9 |
| 安全准入 | URL 白名单 + SYSTOKEN 白名单 + 应用白名单准入 | G12、G8、G14 |
| 运维可见 | 运行记录查看 + 运行日志 + 操作日志审计 | G10、G11、G17 |
| 调试高效 | 草稿和已发布版本直接调试触发 | G16 |
| V1 兼容 | V1 连接器和连接流无缝迁移至 V2 | G11 |
...
| 开放平台审批能力可无缝复用至连接流部署审批场景 | 中 | 技术预研对接 |
| Redis 限流方案可替换 V1 Bucket4j | 低 | V1 已有 Redis 依赖，技术栈无变化 |
...
| 部署审批流程涉及多系统改造，集成复杂度高 | 中 | 复用开放平台审批引擎，减少自研 |
...
| **V2（本规范）** | 多版本管理 + 数字签名认证 + 限流增强 + 部署审批 + 串并行编排 + 数据处理节点 + 运行监控/日志 + URL/SYSTOKEN/应用白名单准入 + 调试触发 + 操作日志 | 安全迭代 · 灵活配置 · 运维可见 |
| **V2.5 建议** | 条件分支 + 事件/定时触发器 + 模板库 + 失败重试 | 编排能力补全 |
| **V3 展望** | AI 编排 + 三方开放发布 + 社区市场 + 多集群 | 生态与智能 |

> **V1→V2 迁移策略**：V1 连接器/连接流兼容运行，单版本配置自动迁移为 V2 首个已发布版本。V2 所有新增能力均为叠加增强，不影响已有功能。
...
| G6 连接流部署审批 | US-03、US-07 | FR-017 ~ FR-018 |
| G7 部署审批一键催办 | US-07 | FR-019 |
