...
| 前端 wecodesite | ~14 | ~15 |
| 数据库 | 2 CREATE + 6 ALTER | URL白名单复用字典表，应用白名单复用LookUp |
| ADR | 4 | — |
...
| 文档 | 内容 |
|------|------|
| [plan-db.md](./plan-db.md) | **数据库设计** — 4 MODIFY + 2 NEW + 2 ENABLE，URL 白名单复用字典表，应用白名单复用 LookUp |
...
| 审计 | 无 | 变更操作日志 |
| 端点总数 | 18 | 45 |
| 表数量 | 7（4 启用 + 3 预留） | 9（4 MODIFY + 2 NEW + 2 ENABLE + 1 EXTEND） |
