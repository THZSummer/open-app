...
| [plan-code.md](../specs-tree-connector-platform/plan-code.md) | **代码规范**（引用 V1） — 16 条规范（注释中文、日志英文、SQL 禁 SELECT * 等），V2 遵循 | 841 |
| [plan-cache.md](../specs-tree-connector-platform/plan-cache.md) | **缓存与限流策略**（引用 V1 + V2 增量） — Redis 配置缓存、入站限流令牌桶、版本切换失效 | 575 |
