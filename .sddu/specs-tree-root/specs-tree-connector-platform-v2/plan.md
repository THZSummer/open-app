...
> 💡 **项目说明**：V1（CONN-PLAT-001）已验证了「零代码编排」核心价值。V2 在 V1 基础上叠加七个增强层（版本/编排/运行时/安全/审批/调试/运维），不引入新服务。前端统一在 `wecodesite`，后端管理面在 `open-server`，运行时在 `connector-api`。
...
| 审计 | 无 | 变更操作日志 |
| 端点总数 | 18 | 45 |
| 表数量 | 7（4 启用 + 3 预留） | 11（4 MODIFY + 4 NEW + 2 ENABLE + 1 EXTEND） |
