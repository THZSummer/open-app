...
**规范版本**: [spec.md](./spec.md) v2.23-draft
**前置文档**: spec.md v2.23, [plan-json-schema.md](./plan-json-schema.md) v9.10, ADR-001~003（V1 已验证）, ../specs-tree-connector-platform/plan.md（V1 基线）
...
| 审批集成复杂度 | 🟡 中 | 审批引擎仅拓展场景（低风险）+ 审批人配置涉及 open-server 改造（中风险）；提前对齐接口 |
| 并行分支执行复杂度 | 🟡 中 | Reactor `Flux.merge()`；每分支独立超时+错误不扩散 |
| 版本快照数据量增长 | 🟡 中 | 1000 版本硬上限；物理删除真删除；>800 时告警 |
| 设计态校验放宽后存量配置兼容风险 | 🟢 低 | 保存时不校验，仅温和提示；发布时统一校验（spec v2.23）；批量修复工具 |
...
| v2.1 | 2026-06-10 | **对齐 spec v2.16**：① 版本号声明 v2.15→v2.16，更新日期；② §1.1 架构 mermaid 新增 DBG/Ops 图层；③ §2.1 OQ-005 审批集成拆为两层依赖描述；④ §3.1 审批集成/安全准入模块补充外部依赖说明；⑤ §3.2/§3.4/附录B 数值对齐 plan-api.md 和 plan-db.md；⑥ §4 新增 market-server 依赖风险评估 | SDDU Plan Agent |
| v2.2 | 2026-06-15 | **对齐 spec v2.22**：① 版本号/架构图/变更表同步更新 ② 错误处理策略（try-catch→retry/ignore/terminate）③ 草稿手动创建 ④ 并行处理节点 8 分支上限 ⑤ 风险评估补充 ⑥ 迭代规划 FR 引用修正 ⑦ 端点总数 41→51 | SDDU Plan Agent |

## 附录 B：V1→V2 spec 变更对照

| 维度 | V1 (spec v5.0) | V2 (spec v2.16) |
|------|---------------|----------------|
| 版本模型 | 单版本 | 多版本（草稿→发布→失效→删除） |
| 认证方式 | SOA、APIG | 新增数字签名、Cookie，支持多选 |
| 编排模式 | 纯串行 | 串行 + 并行（边级）+ 并行处理节点（2~8 分支） |
| 节点类型 | 3 种 | 5 种（新增数据处理节点 + 错误处理节点（重试/忽略/终止）） |
| 版本发布审批 | 无 | 三级审批 + 催办 |
| 运行监控 | 无 | 运行记录 + 节点日志 |
| 安全 | 无 | URL 白名单 + SYSTOKEN 白名单 + 应用白名单 |
| 数据模型 | 无强制类型展开 | object/array 递归展开到基本类型，发布时硬校验，草稿保存不校验 |
| 调试 | 必须发布部署后验证 | 草稿/已发布版本直接调试 |
| 审计 | 无 | 变更操作日志 |
| 依赖管理 | 无 | 12 条外部依赖明确（审批引擎/审批人配置/应用白名单/URL白名单/SYSTOKEN白名单/HTTP触发器网关等），复用边界清晰 |
| 端点总数 | 18 | 51 |
| 表数量 | 7（4 启用 + 3 预留） | 12（5M + 3N + 4R） |
