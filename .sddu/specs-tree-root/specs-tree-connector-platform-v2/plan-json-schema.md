...
**创建日期**: 2026-05-22  
**最后更新**: 2026-06-10
**修订说明**: v7.0 — 章节重构：正文按认知递进组织（基础约束→组件库→配置域→执行约定），参考材料降级为附录。V2 新增内容已在 v7.0 初版中完成。
...
| # | 来源 | 语法 | 示例 | 适用场景 |
|:---:|------|------|------|---------|
| 1 | **节点数据引用** | `${$.node.{nodeId}.{input/output}.xxx.yyy}` | `${$.node.trigger.input.sender}` | 引用上游节点的输入/输出字段 |
| 2 | **字面常量** | `${$.constant:value}` | `${$.constant:0}`、`${$.constant:success}` | 固定值，如默认页码、状态码 |
| 3 | **系统变量** | `${$.system.xxx.yyy}` | `${$.system.env.region}`、`${$.system.app.name}` | 环境变量、应用配置等平台级上下文 |
| 4 | **内置函数** | `${$.fn.{name}(args...)}` | `${$.fn.upper($.node.trigger.input.name)}` | 数据处理器节点中调用字符串/数学/数组等转换函数（V2） |

> 表达式层级：`$.` = 根 → `node`/`constant`/`system`/`fn` = 作用域 → 具体路径或参数。
...
#### 1.4.3 内置函数（V2，data_processor 专用）

数据处理器（`data_processor`）节点可在 `config.fieldMappings[].source` 中使用内置函数表达式：

| 类别 | 函数示例 | 说明 |
|------|---------|------|
| 字符串 | `upper(s)` / `lower(s)` / `concat(a,b)` / `substring(s,from,to)` | 大小写转换、拼接、截取 |
| 数学 | `add(a,b)` / `multiply(a,b)` / `round(n)` / `abs(n)` | 四则运算、取整、绝对值 |
| 数组 | `length(arr)` / `first(arr)` / `join(arr,sep)` | 长度、首元素、连接 |
| 逻辑 | `if(cond, then, else)` / `equals(a,b)` / `isEmpty(v)` | 条件判断、相等比较、空值检测 |

函数参数可以是其他表达式（嵌套引用）：
```
${$.fn.concat($.node.trigger.input.firstName, $.constant:" ", $.node.trigger.input.lastName)}
```
...
| 3 | **表达式体系统一** | 四种值来源共用同一套 `${$.scope.path}` 语法，不因值来源不同而异 |
...
| # | 组件名 | 用途 | 被引用方 |
|:---:|--------|------|---------|
| 1 | `authConfigDef` | 认证类型声明（含凭证字段列表） | §4 connectionConfig, §3.4.10 triggerDataDef |
| 2 | `rateLimitDef` | 限流配置（QPS + 并发） | §4 connectionConfig, §3.4.10 triggerDataDef |
| 3 | `jsonSchemaObjectDef` | 纯字段形状描述（类型 + 属性 + 必填） | §3.4.4 httpInputContractDef, §3.4.5 httpOutputContractDef |
| 4 | `httpInputContractDef` | HTTP 入参契约（header/query/body 三段式） | §3.4.6 inputContractDef |
| 5 | `httpOutputContractDef` | HTTP 出参契约（header/body 两段式） | §3.4.7 outputContractDef |
| 6 | `inputContractDef` | 入参契约路由器（oneOf 按协议分发） | §4 connectionConfig, §3.4.10 triggerDataDef |
| 7 | `outputContractDef` | 出参契约路由器（oneOf 按协议分发） | §4 connectionConfig |
| 8 | `errorInfoDef` | 错误详情（code + 双语 message + 根因） | §6 执行数据 |
| 9 | `nodeDataDef` | 节点 data 路由器（oneOf 按 node.type 分发至 7 种 data Schema） | §5.4 orchestrationConfig |
| 10 | `triggerDataDef` | 触发器节点业务数据（触发类型 + 认证 + 入参契约） | §3.4.9 nodeDataDef |
| 11 | `connectorDataDef` | 连接器节点业务数据（版本引用 + 字段映射） | §3.4.9 nodeDataDef |
| 12 | `dataProcessorDataDef` | 数据处理器节点业务数据（字段映射管道） | §3.4.9 nodeDataDef |
| 13 | `exitDataDef` | 出口节点业务数据（出参字段映射） | §3.4.9 nodeDataDef |
| 14 | `structureNodeDataDef` | 结构体主节点业务数据（循环/并行/条件/错误处理，config 预留） | §3.4.9 nodeDataDef |
| 15 | `textMarkerDataDef` | text 标记节点数据（纯渲染，不参与执行，config 预留） | §3.4.9 nodeDataDef |
...
以下 JSON 为所有共享组件定义的聚合，是整个文档中 JSON Schema 定义的唯一权威来源。各组件通过 `#/definitions/xxx` 路径被 §4 和 §5 引用。
...
```json
{
  ...
  "definitions": {
      ...
      "oneOf": [
        {
          ...
          "properties": {
            ...
            "value": { "type": "string", "description": "映射表达式" },
            "enum": { "type": "array" },
            "default": {}
          },
          "required": ["type", "value"]
          ...
          "properties": {
            "fieldMappings": {
              ...
              "items": {
                ...
                "additionalProperties": false,
                "properties": {
                  "source": { "type": "string", "pattern": "^($\\{[a-zA-Z0-9_.]+\\}|constant:[a-zA-Z0-9_]+)$" },
                  "target": { "type": "string" }
                },
    ...
    "structureNodeDataDef": {
      ...
      "description": "结构体主节点业务数据。覆盖 loop_v2 / parallel / condition_branch / error_handler 四种结构类型。config 字段为预留槽，具体子字段待 V2 专项细化",
      "type": "object",
      "additionalProperties": false,
      "properties": {
        "labelCn": { "type": "string" },
        ...
        "config": {
          "type": "object",
          "description": "结构体配置（预留槽）。预期包含：循环体—iterationSource/maxIterations/iterationVar；并行—parallelMode；条件分支—conditionBranches；错误处理—errorHandlingStrategy"
        }
      },
      "required": ["type"]
    },
    ...
    "textMarkerDataDef": {
      ...
      "description": "text 标记节点数据。纯渲染标记节点，不参与 DAG 执行。引擎拓扑排序时通过 node.type='text' 过滤。config 字段为预留槽，具体归属 schema 待 groupingDef 专项讨论后细化",
      "type": "object",
      "additionalProperties": false,
      "properties": {
        "labelCn": { "type": "string" },
        ...
        "config": {
          "type": "object",
          "description": "text 节点归属配置（预留槽）。预期包含 groupId/role/branchId 等结构归属信息"
        }
      }
    }
  }
...
| 协议 | 目标定义 | 说明 |
|------|---------|------|
| `HTTP` | `httpInputContractDef`（§3.4.4） | header / query / body |
| `REDIS`（V1） | `redisInputContractDef` | 待定义 |
| `MYSQL`（V1） | `mysqlInputContractDef` | 待定义 |

#### 3.4.7 outputContractDef
...
| 协议 | 目标定义 | 说明 |
|------|---------|------|
| `HTTP` | `httpOutputContractDef`（§3.4.5） | header / body |
...
#### 3.4.9 nodeDataDef（路由汇总）

节点业务数据路由器，根据 `node.type` 选择对应的 data Schema。V2 扩展至 **7 分支**：

| `node.type` 值 | 对应 data Schema | $ref 目标 | 详见 |
|----------------|-----------------|-----------|:--:|
| `trigger` | triggerDataDef | `#/definitions/triggerDataDef` | §3.4.10 |
| `connector` | connectorDataDef | `#/definitions/connectorDataDef` | §3.4.11 |
| `data_processor` | dataProcessorDataDef | `#/definitions/dataProcessorDataDef` | §3.4.12 |
| `exit` | exitDataDef | `#/definitions/exitDataDef` | §3.4.13 |
| `loop_v2` / `parallel` / `condition_branch` / `error_handler` | structureNodeDataDef | `#/definitions/structureNodeDataDef` | §3.4.14 |
| `text` | textMarkerDataDef | `#/definitions/textMarkerDataDef` | §3.4.15 |

#### 3.4.10 triggerDataDef
...
| authConfig | object | ❌ ⚡ | 认证配置（HTTP 触发时必填），见 §3.4.1 |
| inputContract | object | ❌ ⚡ | 入参数据契约（HTTP 触发时必填），见 §3.4.6 |
| rateLimitConfig | object | ❌ | 限流配置，见 §3.4.2 |

⚡ = `type="http"` 时必填。

#### 3.4.11 connectorDataDef
...
#### 3.4.14 structureNodeDataDef（V2 新增）
...
#### 3.4.15 textMarkerDataDef（V2 新增）
...
| authConfig | object | ❌ | 认证类型声明 | §3.4.1 |
| inputContract | object | ❌ | 入参契约 | §3.4.6 |
| outputContract | object | ❌ | 出参契约 | §3.4.7 |
| timeoutMs | integer | ❌ | 单次调用超时（毫秒），默认 3000 | — |
| rateLimitConfig | object | ❌ | 限流配置 | §3.4.2 |
...
graph TB
    subgraph Storage["存储位置：flow_version_t.orchestration_config"]
        ...
        subgraph Business["业务拓展层（§5.3）"]
            BND["nodes[].data（§5.3.1）<br/>按 node.type 路由到 7 种配置"]
            BED["edges[].data（§5.3.2）<br/>控制流语义：条件/并行/循环"]
        end
    end
    Framework -.-> Business
    Business -.-> Framework
...
| 层 | 职责 | 谁管 | 定义位置 |
|----|------|------|---------|
| 框架渲染层 | 节点渲染位置、连线拓扑、画布交互 | React Flow (@xyflow/react v12) | §5.2 |
| 业务拓展层 | 节点业务配置、字段映射、控制流语义 | 应用层（本项目） | §5.3 |
...
⍟ = HTTP 触发时必填。各 data Schema 完整定义见 §3.4.10 ~ §3.4.15。
...
```json
{
  ...
  "properties": {
    ...
    "conditionExpr": {
      "type": "string",
      "description": "条件表达式（businessType=condition 时必填），JSON Path 语法"
    },
    "connectionMode": {
      "type": "string",
...
| conditionExpr | string | ❌ | — | 条件表达式（businessType=condition 时必填） |
  ...
  "nodes": [
    ...
    { "id":"loop_start_1",  "type":"text", "position":{"x":510,"y":300}, "data":{"labelCn":"循环开始","type":"text","config":{}} },
    {
      ...
      "data": {
        ...
        "inputMapping": { "body": { "type": "object", "properties": { "itemId": { "type":"string","value":"${$.node.loop_1.output.currentItem}" } } } }
      }
    },
    ...
    { "id":"loop_break_1", "type":"text", "position":{"x":250,"y":580}, "data":{"labelCn":"循环跳出","type":"text","config":{}} },
    {
      ...
      "data": {
        "labelCn": "返回结果",
        "outputMapping": { "body": { "type": "object", "properties": { "count": { "type":"integer","value":"${$.node.loop_1.output.totalProcessed}" } } } }
      }
    }
  ],
  "edges": [
...
执行数据的结构由对应节点的 inputContract / outputContract 动态决定，不在数据库层约束。errorInfo 使用结构化格式（定义见 §3.4.8）。
...
| Node type 枚举 | V2 扩展至 9 种（含结构节点 + text） | 纯渲染类型，业务语义进 data | ✅ v7.0 |
| Edge 语义拆分 | `edge.type` → 渲染；`edge.data.*` → 业务 | 遵循 React Flow 规范 | ✅ |
| node.data 路由 | nodeDataDef oneOf 7 分支 | V2 新增结构节点 + text 标记 | ✅ v7.0 |
| 文档结构 | 正文按认知递进，参考材料归附录 | 减少阅读跳转 | ✅ v7.0 |
| 入参出参按协议区分 | 删除 dataContractDef，新增协议专用 contract + 路由器 | 消除协议无感问题 | ✅ |
| 结构节点 data | 新增 structureNodeDataDef / textMarkerDataDef，config 预留 | 先定义框架，子字段待专项细化 | ✅ v7.0 |
| text 标记持久化 | 全量持久化，执行时引擎过滤 | 零翻译、diff 清晰 | ✅ v7.0 |
| DAG 拓扑约束 | 移除 MVP 线性限制，新增 V2 结构体完整性/并行一致性 | V2 不再是线性 DAG | ✅ v7.0 |
...
| v7.1 | 2026-06-10 | 章节重构：正文按认知递进（基础约束→组件库→配置域→执行约定），§7-§11 降级为附录 A-D，新增 §3.1 组件速查表，§3.4 详解各自含 JSON 片段，§6 精简为执行数据约定 |
