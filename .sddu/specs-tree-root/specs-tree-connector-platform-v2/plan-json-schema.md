...
| — | `mappedFieldDef` | 第四层 | 带映射值的字段定义（叶子/嵌套/数组递归） | connectorNodeDataDef.inputMapping, exitNodeDataDef.outputMapping |
...
```json
{
  ...
  "description": "所有上下文 Schema 共用的组件定义。v7：V2 对齐——nodeDataDef 扩展至 7 分支（含结构节点 + text 标记），新增 structureNodeDataDef / textMarkerNodeDataDef",
  ...
  "definitions": {
    ...
    "nodeDataDef": {
      ...
      "oneOf": [
        ...
        { "$ref": "#/definitions/textMarkerNodeDataDef",        "description": "node.type='text'（不参与 DAG 执行）" }
      ]
    },
