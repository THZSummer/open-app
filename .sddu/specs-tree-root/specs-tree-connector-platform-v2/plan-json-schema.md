...
**创建日期**: 2026-05-22  
**最后更新**: 2026-06-12
**修订说明**: v9.7 — errorHandlerNodeDataDef 按 spec FR-039a 重构（successCondition + failureResponse）
...
| v9.4 | 2026-06-11 | connectionConfig: nodeInput/nodeOutput→input/output；protocolConfig 删除 headers（由 input.header 覆盖）；§5.4 示例改为 SOA+COOKIE 双认证含 value 表达式 |
| v9.5 | 2026-06-11 | §4.3.7 补充两层 type 对照表；§6.7.2 示例增强（新增 data_processor + 嵌套 error_handler）；§4.3.10 标记 V2 新增 |
| v9.6 | 2026-06-11 | 章节重排：exit(10)/dataProcessor(11)、textNodeDataDef(4.3.13 紧邻 loop)、errorHandlerNodeDataDef(4.3.12 紧邻 loop 前) |
| v9.7 | 2026-06-11 | errorHandlerNodeDataDef 按 spec FR-039a 重构：新增 successCondition(checkField+expectedValue) + failureResponse(复用 jsonObjectDef)；§6.7.2 示例同步 |
