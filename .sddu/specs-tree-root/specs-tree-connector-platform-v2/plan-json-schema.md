...
**创建日期**: 2026-05-22  
**最后更新**: 2026-06-11
**修订说明**: v9.0 — V2 全量缺口修复（10 项），详见附录 E
...
> 结构节点主节点，承载循环/并行/条件分支/错误处理的结构定义。**不继承 `nodeDataBaseDef`**——`type` 直接使用前端注册组件名。

> **Def**

```json
{
  "$id": "urn:openapp:schema:structureNodeDataDef:v2",
  "type": "object",
  "additionalProperties": false,
  "properties": {
    "labelCn": { "type": "string" },
    "labelEn": { "type": "string" },
    "type": { "type": "string", "enum": ["loop-v2", "parallel", "condition-branch", "error-handler"] },
    "structConfig": {
      "type": "object",
      "description": "结构节点配置。用于 React Flow 画布中构建、解析和运行 DAG 拓扑结构：\n- 循环/错误处理: loopV2GroupId, loopV2Role, parentLoopV2GroupId, parentLoopV2Role\n- 并行/条件分支: parallelGroupId, parallelRole, parallelBranchId, parallelBranchIndex\n- 嵌套: parentParallelGroupId, parentParallelBranchId 等"
    }
  },
  "required": ["type"]
}
...
| labelCn | string | ❌ | 节点中文标签 |
| labelEn | string | ❌ | 节点英文标签 |
| type | string | ✅ | 结构体类型：`loop-v2` / `parallel` / `condition-branch` / `error-handler` |
| structConfig | object | ❌ | 结构节点配置。用于 React Flow 中构建 DAG 拓扑（节点的归属关系、分组 ID、角色标识），不参与运行时数据传递 |

> **示例** — 循环节点：

```json
{ "labelCn": "遍历处理", "type": "loop-v2", "structConfig": {} }
```

#### 4.3.13 textMarkerNodeDataDef（V2 新增）

> 继承 `nodeDataBaseDef`（type / labelCn / labelEn）。纯渲染标记节点，不参与 DAG 执行，引擎拓扑排序时过滤。
  ...
  "allOf": [
    { "$ref": "#/definitions/nodeDataBaseDef" },
    {
      ...
      "properties": {
        "structConfig": {
          "type": "object",
          "description": "结构归属配置。用于 React Flow 画布中标记节点与结构主节点的归属关系：\n- 循环/错误处理: loopV2GroupId + loopV2Role（region/start/end/break）\n- 并行/条件分支: parallelGroupId + parallelRole（branch-start/branch-end/merge）+ parallelBranchId + parallelBranchIndex"
        }
      }
    }
  ]
...
| structConfig | object | ❌ | 独有 | 结构归属配置。标记节点与结构主节点的分组归属关系，用于前端布局渲染

> **示例**

```json
{ "type": "text", "labelCn": "循环开始", "structConfig": {} }
```
...
| `loop-v2` / `parallel` / `condition-branch` / `error-handler` | structureNodeDataDef | `#/definitions/structureNodeDataDef` | §4.3.12 |
| `text` | textMarkerNodeDataDef | `#/definitions/textMarkerNodeDataDef` | §4.3.13 |
### 4.4 完整组件定义
...
```json
{
  ...
  "definitions": {
    ...
    "nodeDataDef": {
      ...
      "title": "nodeDataDef",
      "description": "节点业务数据路由器。V2 扩展至 6 分支——按 node.type 分发到对应 data Schema",
      "type": "object",
      "oneOf": [
        ...
        { "$ref": "#/definitions/structureNodeDataDef",     "description": "node.type='loop-v2'|'parallel'|'condition-branch'|'error-handler'" },
    ...
    "structureNodeDataDef": {
      ...
      "description": "结构体主节点业务数据。structConfig 承载 DAG 拓扑配置",
      "type": "object",
      "additionalProperties": false,
      "properties": {
        "labelCn": { "type": "string" },
        ...
        "structConfig": {
          "type": "object",
          "description": "结构节点配置。用于 React Flow 画布中构建 DAG 拓扑（分组 ID、角色标识、嵌套归属），不参与运行时数据传递"
        }
      },
      "required": ["type"]
    },
      ...
      "allOf": [
        { "$ref": "#/definitions/nodeDataBaseDef" },
        {
          ...
          "properties": {
            "structConfig": {
              "type": "object",
              "description": "结构归属配置。标记节点与结构主节点的分组归属关系（groupRole/groupType），用于前端布局渲染"
            }
          }
        }
      ]
...
| `loop-v2` | structureNodeDataDef | type | 循环主节点 |
| `parallel` | structureNodeDataDef | type | 并行主节点 |
| `condition-branch` | structureNodeDataDef | type | 条件分支主节点 |
| `error-handler` | structureNodeDataDef | type | 错误处理主节点 |
...
```json
{
  "nodes": [
    ...
    { "id":"loop_1", "type":"loop-v2", "position":{"x":250,"y":160}, "data":{"labelCn":"遍历处理","type":"loop-v2","config":{}} },
...
| 结构节点 data | 新增 structureNodeDataDef / textMarkerNodeDataDef，config 预留 | 先定义框架，子字段待专项细化 | ✅ |
