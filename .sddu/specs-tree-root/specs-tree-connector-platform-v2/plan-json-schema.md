...
| # | 原则 | 规则 | 示例 |
|:---:|------|------|------|
| 一 | **同名同构** | 同一语义的字段在不同上下文中使用同一 Schema 组件，不重复定义 | ① `authConfig` → 触发器和连接器共用 `authConfigDef`<br>② `rateLimitConfig` → 入站和出站共用 `rateLimitConfigDef`<br>③ `nodeInput` → 触发器和连接器统一走 `nodeInputDef` |
...
| # | 原则 | 说明 |
|:---:|------|------|
| 1 | **映射结构镜像需求结构** | connector 的 nodeInput 分 header/query/body，则 inputMapping 也分 header/query/body |
...
| 4 | **语义化名 + `Def`** | 第四层基础复用组件，无直接对应 JSON 字段 | `jsonSchemaObjectDef`（v2 合并 mappedFieldDef/mappedJsonSchemaObjectDef，value 可选） |
...
graph TB
    ...
    subgraph L4["第四层：基础复用"]
        JS["jsonSchemaObjectDef<br/>value可选"]
    end
...
| 第四层 | 基础复用组件（被上层引用） | `jsonSchemaObjectDef`（v2 合并 mappedFieldDef/mappedJsonSchemaObjectDef，value 可选） |
...
```json
{
  ...
  "description": "所有上下文 Schema 共用的组件定义。v8：合并 mappedFieldDef/mappedJsonSchemaObjectDef 至 jsonSchemaObjectDef v2（value 可选）",
  ...
  "definitions": {
    ...
    "jsonSchemaObjectDef": {
      "$id": "urn:openapp:schema:jsonSchemaObjectDef:v2",
            ...
            "oneOf": [
              ...
              },
              {
                ...
                "properties": {
                  ...
                  "description": { "type": "string" },
                  "items":       { "$ref": "#/definitions/jsonSchemaObjectDef" }
                },
                "required": ["type", "items"]
              }
            ]
    ...
    "nodeInputDef": {
      ...
      "title": "nodeInputDef",
      "description": "入参路由器——oneOf 按协议类型分发",
      "type": "object",
      "oneOf": [
    ...
    "nodeOutputDef": {
      ...
      "title": "nodeOutputDef",
      "description": "出参路由器——oneOf 按协议类型分发",
      "type": "object",
      "oneOf": [
    ...
    "structureNodeDataDef": {
      ...
      "description": "结构体主节点业务数据。config 预留",
      "type": "object",
      "additionalProperties": false,
      "properties": {
        "labelCn": { "type": "string" },
...
```json
{
  ...
  "properties": {
    ...
    "authConfig": { "$ref": "#/definitions/authConfigDef" },
    "nodeInput": { "$ref": "#/definitions/nodeInputDef" },
    "nodeOutput": { "$ref": "#/definitions/nodeOutputDef" },
...
|-----------|------------|:----------:|------|
| `trigger` | triggerNodeDataDef | type, authConfig(⍟), nodeInput(⍟) | 触发器 |
| `connector` | connectorNodeDataDef | connectorVersionId, inputMapping | 连接器 |
| `data_processor` | dataProcessorNodeDataDef | config | 数据管道 |
| `exit` | exitNodeDataDef | outputMapping | 出口 |
  ...
  "nodes": [
    ...
    { "id":"loop_start_1",  "type":"text", "position":{"x":510,"y":300}, "data":{"labelCn":"循环开始","type":"text","config":{}} },
    {
      ...
      "data": {
        ...
        "inputMapping": { "body": { "type": "object", "properties": { "itemId": { "type":"string","value":"${$.node.loop_1.current.item}" } } } }
      }
    },
    ...
    { "id":"loop_break_1", "type":"text", "position":{"x":250,"y":580}, "data":{"labelCn":"循环跳出","type":"text","config":{}} },
    {
      ...
      "data": {
        "labelCn": "返回结果",
        "outputMapping": { "body": { "type": "object", "properties": { "count": { "type":"integer","value":"${$.node.loop_1.current.total}" } } } }
      }
    }
  ],
  "edges": [
...
| v5.6 | 2026-05-27 | 映射字段结构化：mappedFieldDef / jsonSchemaObjectDef |
