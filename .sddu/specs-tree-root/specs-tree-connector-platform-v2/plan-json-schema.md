...
**创建日期**: 2026-05-22  
**最后更新**: 2026-06-15
**修订说明**: v9.10 — errorHandler 按 spec v2.22 FR-039a 重构：try-catch → retry(1~5)/ignore/terminate 策略模型
...
| React Flow node.type | snake_case（注册组件名） | `trigger` / `data_processor` / `loop-v2` |
| node.data.type（业务节点类型） | camelCase（JSON 内部规范，§2.1 第 1 行） | `trigger` / `dataProcessor` / `loopV2` |
...
>
> `loop_v2` / `error_handler` 节点的 `output` 字段结构（持久化属性）和 `current` 下可用字段的完整列表，待 §4.3.12~§4.3.13 `errorHandlerNodeDataDef` / `loopNodeDataDef` 专项细化后确定。
...
| 10 | `exitNodeDataDef` | 第二层 | 出口节点业务数据 | §4.3.10 |
| 11 | `dataProcessorNodeDataDef` | 第二层 | 数据处理器节点业务数据 | §4.3.11 |
| 12 | `errorHandlerNodeDataDef` | 第二层 | 错误处理节点业务数据（继承 nodeDataBaseDef） | §4.3.12 |
| 13 | `loopNodeDataDef` | 第二层 | 循环节点业务数据（继承 nodeDataBaseDef） | §4.3.13 |
| 14 | `textNodeDataDef` | 第二层 | text 标记节点数据（继承 nodeDataBaseDef） | §4.3.14 |
| 15 | `parallelNodeDataDef` | 第二层 | 并行节点业务数据（继承 nodeDataBaseDef） | §4.3.15 |
| 16 | `conditionBranchNodeDataDef` | 第二层 | 条件分支节点业务数据（继承 nodeDataBaseDef） | §4.3.16 |
...
| JSON 字段 | 类型 | 必填 | 说明 |
|-----------|------|:----:|------|
| type | string | ✅ | 业务节点类型：`trigger` / `connector` / `dataProcessor` / `exit` / `loopV2` / `parallel` / `conditionBranch` / `errorHandler` / `text` |
...
| retryConfig.maxRetries | integer | ✅ ⚡ | 独有 | 最大重试次数，范围 1~5。超出设计态标红（EC-022） |
| retryConfig.retryInterval | integer | ✅ ⚡ | 独有 | 重试间隔（秒），范围 1~300。超出设计态标红（EC-023） |
...
| `dataProcessor` | dataProcessorNodeDataDef | §4.3.11 |
| `exit` | exitNodeDataDef | §4.3.10 |
| `loopV2` | loopNodeDataDef | §4.3.13 |
| `parallel` | parallelNodeDataDef | §4.3.15 |
| `conditionBranch` | conditionBranchNodeDataDef | §4.3.16 |
| `errorHandler` | errorHandlerNodeDataDef | §4.3.12 |
| `text` | textNodeDataDef | §4.3.14 |
### 4.4 完整组件定义
...
| rateLimitConfig | object | ❌ | 限流配置 | §4.3.3 |
...
每个节点由 **框架字段**（React Flow 画布渲染必需）和 **业务字段**（`node.data`）构成。`node.data` 的具体结构由 `node.data.type` 决定，通过 `nodeDataDef`（§4.3.17）按 9 种类型路由。框架层的 `node.type` 仅用于 React Flow 组件注册。
...
> `node.type` 的 9 种枚举值（框架层）：`trigger` / `connector` / `data_processor` / `exit` / `loop-v2` / `error-handler` / `parallel` / `condition-branch` / `text`

**node.data 路由**：

| `node.type`（框架） | `node.data.type`（业务） | `data` Schema | 必填字段 | 说明 |
|------|------|------------|:--:|------|
| `trigger` | `trigger` | triggerNodeDataDef | triggerType, authConfigs(⍟), nodeInput(⍟) | 触发器（激活方式由 triggerType 区分） |
| `connector` | `connector` | connectorNodeDataDef | connectorVersionId, input | 连接器调用 |
...
⍟ = `triggerType="http"` 时必填。各 data Schema 完整定义见 §4.3.8 ~ §4.3.16。
...
执行数据的结构由对应节点的 nodeInput / nodeOutput 动态决定，不在数据库层约束。errorInfo 使用结构化格式（定义见 §4.3.4）。
示例：
```json
// 下游调用失败
...
|------|------|------|------|
| `trigger` | 触发器 UI | `trigger` | triggerType（http/manual）、authConfigs、nodeInput |
...
| v9.5 | 2026-06-12 | §4.3.7 补充两层 type 对照表；§6.7.2 示例增强（新增 data_processor + 嵌套 error_handler）；§4.3.10 标记 V2 新增 |
| v9.6 | 2026-06-12 | 章节重排：exit(10)/dataProcessor(11)、textNodeDataDef(4.3.13 紧邻 loop)、errorHandlerNodeDataDef(4.3.12 紧邻 loop 前) |
...
| v9.8 | 2026-06-12 | §6 章节重构：Schema 组装提前到 §6.2、框架层/业务层按字段路径合并至 §6.3、新增 §6.3.3 flowConfig 独立说明、§6.6 执行模型降级为附录 F；§5.4 示例增强为完整 HTTP 入参三段式+出参两段式；§4.2 速查表/§4.3.17 路由表引用号修复；§4.3.14 textNodeDataDef 重复内容删除 |
| v9.9 | 2026-06-12 | **双层 type 规范化**：① triggerNodeDataDef 拆分 `triggerType`（http/manual），`node.data.type` 恢复为固定值 `"trigger"`；② nodeDataBaseDef type enum 中 kebab-case 值改为 camelCase（`loopV2`/`errorHandler`/`conditionBranch`），严格遵循 §2.1；③ nodeDataDef oneOf 描述措辞修正（`node.type`→`node.data.type`）；④ §4.3.7/§6.3.1/附录 C.2 新增完整 9 型双层 type 映射表 |
| v9.10 | 2026-06-15 | **errorHandler 策略模型重构（对齐 spec v2.22 FR-039a）**：① §4.3.12 从旧 try-catch 模型（successCondition + failureResponse）重构为 retry/ignore/terminate 策略模型 ② 新增 strategy（三选一）、errorTypes（all 与其他互斥）、retryConfig（maxRetries 1~5 / retryInterval 1~300）③ §6.7.2 循环编排示例中的 error_handler 节点数据同步更新 |
