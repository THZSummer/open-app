...
DAG 中节点按拓扑顺序执行，上游节点的输出数据需要传递给下游节点使用。每个节点所需的数据维度不同：

| 节点类型 | 需要什么数据 | 数据从哪来 |
|----------|-------------|-----------|
| **connector** | 连接器的 inputContract 声明的入参（header / query / body） | 上游节点的输出 |
| **data_processor** | 需要转换的字段及目标路径 | 上游节点的输出 |
| **exit** | 对外返回的字段（响应头 / 响应体） | 上游节点的输出 |

核心问题：编排设计时，用户需要显式声明映射关系。如果映射结构扁平，引擎无法判断字段应放到 HTTP 请求的 header、query 还是 body。

#### 1.4.2 统一映射模型

所有节点的字段映射基于 **JSON 节点上下文对象**，区分设计态和运行态：
...
表达式语法 — 引用上游节点 JSON 上下文对象中的字段：

| 表达式 | 含义 | 示例 |
|--------|------|------|
| `${$.node.{nodeId}.{input/output}.xxx.yyy}` | 引用某节点上下文指定路径的值 | `${$.node.trigger.input.sender}` |
| `${$.system.xxx.yyy}` | 引用系统级上下文（V2） | `${$.system.env.region}` |
| `${$.constant:value}` | 固定字面值 | `${$.constant:0}` |

#### 1.4.3 设计原则

1. **映射结构镜像需求结构**：connector 的 inputContract 分 header/query/body，则 inputMapping 也分 header/query/body
2. **Schema 不硬编码协议**：inputMapping/outputMapping 定义为 `"type": "object"`，具体分段结构由应用层按协议校验
3. **表达式体系统一**：所有节点使用相同的 JSON Path 表达式语法
