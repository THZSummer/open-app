...
| 现有表 | 迁移策略 |
|--------|----------|
| `openplatform_permission_api_t` | 1. 迁移数据到新的 `openplatform_api_t` 表<br/>2. 根据 API 信息创建 `openplatform_permission_t` 记录 |
| `openplatform_event_t` | 1. 迁移数据到新的 `openplatform_event_t` 表<br/>2. 创建 `openplatform_permission_t` 记录（之前无关联） |
| `openplatform_app_permission_t` | 迁移到新的 `openplatform_subscription_t` 表 |

```sql
-- 迁移示例：API 权限
INSERT INTO openplatform_api_t (id, name_cn, name_en, path, method, status, create_time)
SELECT 
    id,
    name,
    name,
...
INSERT INTO openplatform_permission_t (id, name_cn, name_en, scope, resource_type, resource_id, status, create_time)
SELECT 
    <雪花ID>,
    name,
    name,
