...
> 💡 **定位**：API 目录的根节点（L1）管理。**仅限平台运营方**访问，负责维护一级分组结构并配置责任人。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-009 | API 一级分组创建/编辑 | **运营方**维护 API 目录的根节点 | • 支持新增/编辑 API 一级分组（如"IM 业务"、"云盘业务"）<br/>• 支持拖拽调整一级分组顺序<br/>• 删除时需检查是否包含子分组或 API |
| FR-010 | API 分组责任人配置 | **运营方**为 API 一级分组指定责任人 | • 支持为一级分组配置**责任人清单**<br/>• 责任人自动获得该分组下创建子分组及注册 API 的权限 |

### 3.4 事件一级分组管理

> 💡 **定位**：事件目录的根节点（L1）管理。**仅限平台运营方**访问，负责维护一级分组结构并配置责任人。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-011 | 事件一级分组创建/编辑 | **运营方**维护事件目录的根节点 | • 同 API 一级分组创建逻辑，针对事件资源 |
| FR-012 | 事件分组责任人配置 | **运营方**为事件一级分组指定责任人 | • 同 API 分组责任人配置逻辑，针对事件资源 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-013 | 应用权限列表查看 | 消费方查看当前应用已申请/申请中的 API 权限状态 | • 列表展示：权限名称、codeName、认证方式、分类、状态<br/>• 支持操作：查看文档、复制审批地址、撤回审核（审核中）<br/>• 提供"添加 API"入口唤起申请弹框 |
| FR-014 | API 权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择 API 权限 | • 弹框内按**应用类型 -> 凭证类型 -> 分组 -> 权限列表**展示全量权限<br/>• 支持多选加入申请单<br/>• 支持搜索/过滤 |
| FR-015 | API 权限申请提交 | 消费方提交 API 权限申请单 | • 申请单自动汇总所选权限及对应审批流<br/>• 支持依赖自动级联（置灰不可取消）<br/>• 提交后等待提供方审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-016 | 应用事件列表查看 | 消费方查看当前应用已订阅/申请中的事件权限 | • 同 API 应用权限列表查看逻辑 |
| FR-017 | 事件权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择事件权限 | • 同 API 权限树形选择逻辑 |
| FR-018 | 事件权限申请提交 | 消费方提交事件权限申请，配置消费通道 | • 支持配置事件**消费通道**（WebHook URL、WebSocket Token 等）<br/>• 提交后等待提供方审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-019 | 审批流程配置 | 支持多层级审批流配置，维护全局及场景默认审批流 | • 支持**全局默认**审批流<br/>• 支持**场景特有**审批流（如 API、事件通用审批流） |
| FR-020 | 资源注册审批 | 提供方注册 API/事件后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后资源变为「已上架」状态 |
| FR-021 | 权限申请审批 | 消费方申请权限后，需经资源提供方审批 | • 申请提交后，根据权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活订阅关系 |
...
**最后更新**: 2026-04-16（API/事件一级分组管理拆分）
