...
| FR-005 | API 权限注册 | **分组责任人**统一注册 API 信息，附带定义权限属性 | • 支持填写 API 基本信息（路径、方法、认证方式等）<br/>• **附带定义权限信息**（名称、标识、审批人）<br/>• 支持选择所属的**权限分组**<br/>• 支持指定**资源特有审批流** |
| FR-006 | API 权限编辑 | **分组责任人**修改 API 及权限相关信息 | • 支持修改 API 属性或权限配置<br/>• 关键属性（如路径、审批人）变更需重新审批<br/>• 修改记录需留痕 |
