...
| FR-005 | API 权限注册 | **分组责任人**统一注册 API 信息，附带定义权限属性 | • **API 关键字段**：中文名称、英文名称、路径（Path）、方法（Method）、接口文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识（如 `im:message:get`，**全局唯一**）<br/>• 支持选择所属的**权限分组**<br/>• 支持指定**资源特有审批流**（含配置审批人/组、节点顺序）<br/>• **注册提交后自动进入审批流程**，审批通过后资源变为「已上架」状态 |
...
| FR-009 | 事件权限注册 | 分组责任人统一注册事件信息，附带定义权限属性 | • **事件关键字段**：中文名称、英文名称、Topic、事件文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识（全局唯一）<br/>• 支持指定**资源特有审批流**（含配置审批人/组）<br/>• **注册提交后自动进入审批流程**，审批通过后资源变为「已上架」状态 |
...
| FR-013 | 回调权限注册 | 分组责任人统一注册回调信息，附带定义权限属性 | • **回调关键字段**：中文名称、英文名称、回调文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识（全局唯一）<br/>• 支持指定**资源特有审批流**（含配置审批人/组）<br/>• **注册提交后自动进入审批流程**，审批通过后资源变为「已上架」状态 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-019 | 应用事件列表查看与配置 | 消费方查看事件订阅状态，**获批后配置事件接收参数** | • 列表展示：**事件名称**、**Scope**、**Topic**、**状态**（审核中/已授权等）<br/>• 支持操作：**查看文档**、**复制审批地址**、**撤回**（仅"审核中"状态展示）<br/>• **获批权限支持配置事件参数**：支持选择**通道**（企业内部消息平台、WebHook 等）、填写**通道地址**（WebHook 场景）、选择**认证类型**（应用类型凭证 A/B 等）<br/>• 支持修改/更新配置 |
...
| **认证类型 (Auth Type)** | 身份验证方式，对应枚举值：<br/>• 0 = Cookie<br/>• 1 = SOA<br/>• 2 = APIG<br/>• 3 = IAM<br/>• 4 = 免认证<br/>• 5 = AKSK<br/>• 6 = CLITOKEN<br/>注：规范中提到的"应用类型凭证 A/B"为历史术语，实际实现使用上述枚举值 |
