...
| FR-004 | API 注册审批 | 提供方注册 API 后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后 API 变为「已上架」状态 |
| FR-005 | 事件注册审批 | 提供方注册事件后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后事件变为「已上架」状态 |
| FR-006 | API 权限申请审批 | 消费方申请 API 权限后，需经资源提供方审批 | • 申请提交后，根据 API 权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活 API 订阅关系 |
| FR-007 | 事件权限申请审批 | 消费方申请事件权限后，需经资源提供方审批 | • 申请提交后，根据事件权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活事件订阅关系 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-008 | 权限多维分类 | 支持按应用类型和凭证类型构建多维分类视图 | • 支持按**应用类型**（业务/个人）区分权限根节点<br/>• 支持按**凭证类型**（AKSK/应用类 A/B...）进一步细分<br/>• 同一 API 在不同凭证类型下视为**独立的权限记录** |
| FR-011 | 权限分组管理（树形） | **白名单用户**自定义树形分组结构 | • 支持新增、编辑、删除分组节点<br/>• 支持拖拽或指定父节点构建层级关系<br/>• 删除分组时需处理关联的权限（如转移至其他分组） |

### 3.4 API 管理

> 💡 **访问控制**：API 注册、编辑功能**仅限白名单用户**（各业务模块责任人）访问。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-009 | API 权限注册 | **白名单用户**提交权限信息，关联底层 API | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层 API**（自动同步 API 属性字段如路径、方法）<br/>• 支持选择关联的**权限分组** |
| FR-010 | API 权限编辑 | **白名单用户**修改已注册的权限信息 | • 支持修改权限名称、审批人、分组<br/>• 关联的底层 API 变更需重新审批<br/>• 修改记录需留痕 |

### 3.5 事件权限管理

> 💡 **核心逻辑**：同 API 权限管理，事件为底层资源。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-012 | 事件权限多维分类 | 支持按应用类型、凭证类型构建事件权限分类 | • 同 API 权限多维分类逻辑 |
| FR-013 | 事件权限注册 | 提供方提交事件权限，关联底层事件 | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层事件**（Topic、通道类型等）<br/>• 支持配置**通道类型**（WebHook、WebSocket、内部消息队列）<br/>• **WebSocket 通道仅限个人应用场景下配置** |
| FR-014 | 事件权限编辑 | 提供方修改已注册的事件权限信息 | • 支持修改权限基本信息、通道配置<br/>• 关联的底层事件变更需重新审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-015 | 应用权限列表（外侧） | 消费方查看当前应用已申请/申请中的 API/事件权限状态 | • 列表展示：**权限名称**、**codeName**、**认证方式**（如 SOA/APIG）、**分类**、**状态**（已审核/审核中/已中止）<br/>• 支持操作：查看文档、复制审批地址、撤回审核（审核中）<br/>• 提供“添加权限”入口唤起申请弹框 |
| FR-016 | 权限树形选择（弹框） | 消费方通过 L1-L4 树形结构浏览并选择权限 | • 弹框内按**应用类型 -> 凭证类型 -> 分组 -> 权限列表**展示全量权限<br/>• 支持多选加入申请单<br/>• 支持搜索/过滤 |
| FR-017 | 依赖自动级联 | 申请单自动处理权限依赖关系 | • 当选中含依赖的权限时，**自动关联加入依赖权限**（置灰不可取消）<br/>• 提交时若自动加入了新权限，需提示消费方 |
| FR-018 | 订阅与配置管理 | 消费方管理已获批的权限订阅，配置事件通道 | • 支持查看当前应用的已授权权限列表<br/>• 支持**取消订阅**权限<br/>• 支持配置事件的**消费通道**（如 WebHook URL、WebSocket Token 等） |
