...
| G4 | 提供事件管理能力 | 支持事件的**注册、订阅、取消订阅**、分组、分类；事件消费区分通道类型（企业内部消息队列/WebHook），区分凭证类型（应用类 A/B）；支持按应用隔离消费 |
| G5 | 提供回调管理能力 | 支持回调的**注册、订阅、取消订阅**、分组、分类；支持配置**通道类型（WebHook、SSE 等）**及**凭证类型（应用类型凭证 A/B）**；支持按应用隔离消费 |
...
| US-06 | 作为 **三方平台负责人**，我想要 **配置事件的消费方式与凭证**，以便 **正确接收事件通知** | P0 | 支持针对不同事件配置消费类型（WebHook 等）及关联凭证（AKSK、应用类凭证等） |

### 2.3 平台管理方（开放平台运营人员）

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-07 | 作为 **平台运营人员**，我想要 **审批 API/事件/回调的注册申请**，以便 **确保上架能力的合规性** | P0 | 运营人员可以查看 API、事件或回调注册申请，执行同意/驳回/撤销操作 |

| US-08 | 作为 **三方平台负责人**，我想要 **配置回调的消费通道与凭证**，以便 **正确接收回调通知** | P0 | 支持针对不同回调配置消费通道（WebHook、SSE 等）及关联凭证（应用类型凭证 A/B 等） |
...
| FR-005 | API 权限注册 | **分组责任人**统一注册 API 信息，附带定义权限属性 | • **API 关键字段**：中文名称、英文名称、路径（Path）、方法（Method）、接口文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识（如 `im:message:get`）<br/>• 支持选择所属的**权限分组**<br/>• 支持指定**资源特有审批流**（含配置审批人/组、节点顺序） |
...
| FR-009 | 事件权限注册 | 分组责任人统一注册事件信息，附带定义权限属性 | • **事件关键字段**：中文名称、英文名称、Topic、事件文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识<br/>• 支持指定**资源特有审批流**（含配置审批人/组） |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-012 | 回调权限列表查看 | 分组责任人查看本分组已注册的回调列表及状态 | • 列表展示：**回调名称**、**Scope**、**触发条件**、**分组**、**状态**<br/>• 支持操作：查看文档、复制审批地址、撤回审核（审核中） |
| FR-013 | 回调权限注册 | 分组责任人统一注册回调信息，附带定义权限属性 | • **回调关键字段**：中文名称、英文名称、触发条件、回调文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识<br/>• 支持指定**资源特有审批流**（含配置审批人/组） |
| FR-014 | 回调权限编辑 | 分组责任人修改回调及权限相关信息 | • 支持修改上述回调/权限关键字段<br/>• 核心属性（触发条件、Scope）变更需重新审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-019 | 应用事件列表查看与配置 | 消费方查看事件订阅状态，**获批后配置消费通道与凭证** | • 列表展示：**事件名称**、**Topic**、**认证方式**、**状态**（审核中/已授权等）<br/>• **获批权限支持配置消费通道**：支持选择**企业内部消息平台**或**WebHook**<br/>• **WebHook 场景凭证配置**：支持选择**应用类型凭证 A**、**应用类型凭证 B**等<br/>• 支持修改/更新通道与凭证配置 |
| FR-020 | 事件权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择事件权限 | • 同 API 权限树形选择逻辑 |
| FR-021 | 事件权限申请提交 | 消费方提交事件权限申请 | • 申请单自动汇总所选事件权限及对应审批流<br/>• 支持依赖自动级联（置灰不可取消）<br/>• 提交后等待提供方审批（**获批后再进行通道配置**） |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-022 | 应用回调列表查看与配置 | 消费方查看回调订阅状态，**获批后配置回调接收地址** | • 列表展示：**回调名称**、**Scope**、**触发条件**、**状态**<br/>• **获批权限支持配置接收地址**（Callback URL）及签名密钥<br/>• 支持修改/更新接收地址配置 |
| FR-023 | 回调权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择回调权限 | • 同 API 权限树形选择逻辑 |
| FR-024 | 回调权限申请提交 | 消费方提交回调权限申请 | • 申请单自动汇总所选回调权限及对应审批流<br/>• 支持依赖自动级联<br/>• 提交后等待提供方审批 |
...
| FR-026 | 资源注册审批 | 提供方注册 API/事件/回调后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后资源变为「已上架」状态 |
| FR-027 | 权限申请审批 | 消费方申请权限后，需经资源提供方审批 | • 申请提交后，根据权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活订阅关系 |
