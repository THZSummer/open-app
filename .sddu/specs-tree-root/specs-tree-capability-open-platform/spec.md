...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-019 | 应用事件列表查看与配置 | 消费方查看事件订阅状态，**获批后配置事件接收参数** | • 列表展示：**事件名称**、**Scope**、**Topic**、**状态**（审核中/已授权等）<br/>• **获批权限支持配置事件参数**：支持选择**协议**（企业内部消息平台、WebHook 等）、填写**协议地址**（WebHook 场景）、选择**凭证类型**（应用类型凭证 A/B 等）<br/>• 支持修改/更新配置 |
| FR-020 | 事件权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择事件权限（**抽屉模式**） | • 支持搜索：按**事件名称**、**Scope**搜索<br/>• 支持过滤：按**是否需要审核**过滤<br/>• 支持多选权限 |
| FR-021 | 事件权限申请提交 | 消费方提交事件权限申请（**独立单据**） | • **无汇总**：每条权限提交后生成**一条单独的申请单**<br/>• 提交成功：**关闭抽屉**<br/>• 提交失败：展示 **Toast 提示** |
...
| FR-024 | 回调权限申请提交 | 消费方提交回调权限申请（**独立单据**） | • **无汇总**：每条权限提交后生成**一条单独的申请单**<br/>• 提交成功：**关闭抽屉**<br/>• 提交失败：展示 **Toast 提示** |
...
**最后更新**: 2026-04-17（权限申请模式统一：抽屉交互/独立单据/Scope 字段）
