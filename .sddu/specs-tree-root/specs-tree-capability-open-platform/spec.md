...
| FR-005 | API 权限注册 | **分组责任人**统一注册 API 信息，附带定义权限属性 | • **API 关键字段**：中文名称、英文名称、路径（Path）、方法（Method）、接口文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识（如 `im:message:get`）、审批人/组<br/>• 支持选择所属的**权限分组**<br/>• 支持指定**资源特有审批流** |
...
| FR-009 | 事件权限注册 | 分组责任人统一注册事件信息，附带定义权限属性 | • 支持填写事件基本信息（Topic、通道类型等）<br/>• **附带定义权限信息**（名称、标识、审批人）<br/>• 支持指定**资源特有审批流** |
| FR-010 | 事件权限编辑 | 分组责任人修改事件及权限相关信息 | • 支持修改事件属性或权限配置<br/>• 关键属性变更需重新审批 |
