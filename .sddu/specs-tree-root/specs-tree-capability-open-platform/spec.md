...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-022 | 应用回调列表查看与配置 | 消费方查看回调订阅状态，**获批后配置回调接收参数** | • 列表展示：**回调名称**、**Scope**、**状态**<br/>• **获批权限支持配置回调参数**：支持选择**通道**（WebHook、SSE 等）、填写**通道地址**、选择**认证类型**（应用类型凭证 A/B 等）<br/>• 支持修改/更新配置 |
...
| **订阅** | `subscriptions` | 存储应用对权限的订阅关系及消费配置（通道、地址、认证） |
...
**最后更新**: 2026-04-17（术语统一：通道/通道地址/认证类型）
