...
| FR-002 | API 权限注册 | **白名单用户**提交权限信息，关联底层 API | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层 API**（自动同步 API 属性字段如路径、方法）<br/>• 支持选择关联的**权限分组** |
...
| FR-006 | 事件权限注册 | 提供方提交事件权限，关联底层事件 | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层事件**（Topic、通道类型等）<br/>• 支持配置**通道类型**（WebHook、WebSocket、内部消息队列）<br/>• **WebSocket 通道仅限个人应用场景下配置** |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-015 | 审批流程配置 | 支持多层级审批流配置，支持从全局到特定资源的精细化控制 | • 支持**全局默认**审批流<br/>• 支持**场景特有**审批流（如 API、事件通用审批流）<br/>• 支持**资源特有**审批流（特定 API/事件独立配置，优先级最高） |
