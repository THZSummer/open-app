...
> 💡 **核心逻辑**：权限为主体，API 为底层资源。目录结构遵循：应用类型 (L1) -> 凭证类型 (L2) -> 权限分组 (L3) -> 权限列表 (L4，列表展示 API 属性)。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-008 | 权限多维分类 | 支持按应用类型和凭证类型构建多维分类视图 | • 支持按**应用类型**（业务/个人）区分权限根节点<br/>• 支持按**凭证类型**（AKSK/应用类 A/B...）进一步细分<br/>• 同一 API 在不同凭证类型下视为**独立的权限记录** |
| FR-009 | API 权限注册 | 提供方提交权限信息，关联底层 API | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层 API**（自动同步 API 属性字段如路径、方法）<br/>• 支持选择关联的**权限分组** |
| FR-010 | API 权限编辑 | 提供方修改已注册的权限信息 | • 支持修改权限名称、审批人、分组<br/>• 关联的底层 API 变更需重新审批<br/>• 修改记录需留痕 |
| FR-011 | 权限分组管理（树形） | 支持提供方自定义树形分组结构 | • 支持新增、编辑、删除分组节点<br/>• 支持拖拽或指定父节点构建层级关系<br/>• 删除分组时需处理关联的权限（如转移至其他分组） |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-015 | 权限目录浏览与检索 | 消费方可按分类树浏览已上架的 API/事件权限 | • 支持按**应用类型 -> 凭证类型 -> 分组**的层级展开浏览<br/>• 支持点击权限查看详情及**跳转说明文档** |
| FR-016 | 权限多选申请 | 消费方可发起权限申请，支持多选 | • 支持在目录中勾选多个 API/事件权限加入申请单<br/>• 申请单需自动汇总所选权限及其对应的审批流 |
...
**最后更新**: 2026-04-16（审批 FR 按 API/事件拆分）
