...
| FR-009 | 事件权限注册 | 分组责任人统一注册事件信息，附带定义权限属性 | • **事件关键字段**：中文名称、英文名称、Topic、通道类型<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识<br/>• 支持指定**资源特有审批流**（含配置审批人/组） |
| FR-010 | 事件权限编辑 | 分组责任人修改事件及权限相关信息 | • 支持修改事件属性或权限配置<br/>• 关键属性（Topic）变更需重新审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-015 | 应用事件列表查看与配置 | 消费方查看事件订阅状态，**获批后配置消费通道** | • 列表展示：事件名称、Topic、状态（审核中/已授权等）<br/>• **获批权限支持配置消费通道**（WebHook URL、WebSocket Token 等）<br/>• 支持修改/更新通道配置 |
