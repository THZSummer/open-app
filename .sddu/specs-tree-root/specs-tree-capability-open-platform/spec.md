...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-015 | 应用事件列表查看与配置 | 消费方查看事件订阅状态，**获批后配置消费通道与凭证** | • 列表展示：**事件名称**、**Topic**、**认证方式**、**状态**（审核中/已授权等）<br/>• **获批权限支持配置消费通道**：支持选择**企业内部消息平台**或**WebHook**<br/>• **WebHook 场景凭证配置**：支持选择**应用类型凭证 A**、**应用类型凭证 B**等<br/>• 支持修改/更新通道与凭证配置 |
| FR-016 | 事件权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择事件权限 | • 同 API 权限树形选择逻辑 |
| FR-017 | 事件权限申请提交 | 消费方提交事件权限申请 | • 申请单自动汇总所选事件权限及对应审批流<br/>• 支持依赖自动级联（置灰不可取消）<br/>• 提交后等待提供方审批（**获批后再进行通道配置**） |
