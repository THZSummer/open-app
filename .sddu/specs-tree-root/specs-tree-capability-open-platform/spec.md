...
> 💡 **定位**：API 资源生命周期管理（查看、注册、编辑、删除）。**仅限白名单用户**（各业务模块责任人）访问。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | API 权限列表查看 | 白名单用户查看本模块已注册的 API 列表及状态 | • 列表展示：权限名称、codeName、认证方式、分类、状态<br/>• 支持操作：查看文档、复制审批地址、撤回审核（审核中） |
| FR-002 | API 权限注册 | **白名单用户**提交权限信息，关联底层 API | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层 API**（自动同步 API 属性字段如路径、方法）<br/>• 支持选择关联的**权限分组**<br/>• 支持指定**资源特有审批流**（优先级高于全局/场景默认） |
| FR-003 | API 权限编辑 | **白名单用户**修改已注册的权限信息 | • 支持修改权限名称、审批人、分组<br/>• 关联的底层 API 变更需重新审批<br/>• 修改记录需留痕 |
| FR-004 | API 权限删除 | **白名单用户**下线已注册的 API 权限 | • 支持删除未关联应用的 API 权限<br/>• 已订阅的权限需提示或禁止删除<br/>• 删除记录需留痕 |

### 3.2 事件管理

> 💡 **定位**：事件资源生命周期管理（查看、注册、编辑、删除）。同 API 管理，仅限白名单用户访问。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-005 | 事件权限列表查看 | 白名单用户查看本模块已注册的事件列表及状态 | • 同 API 权限列表查看逻辑 |
| FR-006 | 事件权限注册 | 提供方提交事件权限，关联底层事件 | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层事件**（Topic、通道类型等）<br/>• 支持配置**通道类型**（WebHook、WebSocket、内部消息队列）<br/>• **WebSocket 通道仅限个人应用场景下配置**<br/>• 支持指定**资源特有审批流** |
| FR-007 | 事件权限编辑 | 提供方修改已注册的事件权限信息 | • 支持修改权限基本信息、通道配置<br/>• 关联的底层事件变更需重新审批 |
| FR-008 | 事件权限删除 | 提供方下线已注册的事件权限 | • 支持删除未订阅的事件权限<br/>• 已订阅的事件需提示或禁止删除 |

### 3.3 API 权限管理

> 💡 **定位**：消费方视角。查看当前应用权限列表、浏览全量权限树、提交申请。外侧列表展示已申请/申请中权限；"添加 API"弹框按 L1-L4 树多选提交。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-009 | 应用权限列表查看 | 消费方查看当前应用已申请/申请中的 API 权限状态 | • 列表展示：权限名称、codeName、认证方式、分类、状态<br/>• 支持操作：查看文档、复制审批地址、撤回审核（审核中）<br/>• 提供"添加 API"入口唤起申请弹框 |
| FR-010 | API 权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择 API 权限 | • 弹框内按**应用类型 -> 凭证类型 -> 分组 -> 权限列表**展示全量权限<br/>• 支持多选加入申请单<br/>• 支持搜索/过滤 |
| FR-011 | API 权限申请提交 | 消费方提交 API 权限申请单 | • 申请单自动汇总所选权限及对应审批流<br/>• 支持依赖自动级联（置灰不可取消）<br/>• 提交后等待提供方审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-012 | 应用事件列表查看 | 消费方查看当前应用已订阅/申请中的事件权限 | • 同 API 应用权限列表查看逻辑 |
| FR-013 | 事件权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择事件权限 | • 同 API 权限树形选择逻辑 |
| FR-014 | 事件权限申请提交 | 消费方提交事件权限申请，配置消费通道 | • 支持配置事件**消费通道**（WebHook URL、WebSocket Token 等）<br/>• 提交后等待提供方审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-015 | 审批流程配置 | 支持多层级审批流配置，维护全局及场景默认审批流 | • 支持**全局默认**审批流<br/>• 支持**场景特有**审批流（如 API、事件通用审批流） |
| FR-016 | 资源注册审批 | 提供方注册 API/事件后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后资源变为「已上架」状态 |
| FR-017 | 权限申请审批 | 消费方申请权限后，需经资源提供方审批 | • 申请提交后，根据权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活订阅关系 |
