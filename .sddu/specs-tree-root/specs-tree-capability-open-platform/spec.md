...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | 审批流程配置 | 支持多层级审批流配置，支持从全局到特定资源的精细化控制 | • 支持**全局默认**审批流<br/>• 支持**场景特有**审批流（如 API、事件通用审批流）<br/>• 支持**资源特有**审批流（特定 API/事件独立配置，优先级最高）<br/>• 资源特有配置需支持为该资源及其**关联依赖**（如 API 关联的数据权限）分别指定审批流 |
| FR-002 | API 注册审批 | 提供方注册 API 后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后 API 变为「已上架」状态 |
| FR-003 | 事件注册审批 | 提供方注册事件后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后事件变为「已上架」状态 |
| FR-004 | API 权限申请审批 | 消费方申请 API 权限后，需经资源提供方审批 | • 申请提交后，根据 API 权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活 API 订阅关系 |
| FR-005 | 事件权限申请审批 | 消费方申请事件权限后，需经资源提供方审批 | • 申请提交后，根据事件权限上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活事件订阅关系 |

### 3.2 API 权限分类

> 💡 **定位**：定义权限的展示维度与分组结构，为消费方提供浏览视图。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-006 | 权限多维分类 | 支持按应用类型和凭证类型构建多维分类视图 | • 支持按**应用类型**（业务/个人）区分权限根节点<br/>• 支持按**凭证类型**（AKSK/应用类 A/B...）进一步细分<br/>• 同一 API 在不同凭证类型下视为**独立的权限记录** |
| FR-007 | 权限分组管理（树形） | **白名单用户**自定义树形分组结构 | • 支持新增、编辑、删除分组节点<br/>• 支持拖拽或指定父节点构建层级关系<br/>• 删除分组时需处理关联的权限（如转移至其他分组） |

### 3.3 API 管理

> 💡 **访问控制**：API 注册、编辑功能**仅限白名单用户**（各业务模块责任人）访问。权限信息（名称、审批人）在注册时附带定义。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-008 | API 权限注册 | **白名单用户**提交权限信息，关联底层 API | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层 API**（自动同步 API 属性字段如路径、方法）<br/>• 支持选择关联的**权限分组** |
| FR-009 | API 权限编辑 | **白名单用户**修改已注册的权限信息 | • 支持修改权限名称、审批人、分组<br/>• 关联的底层 API 变更需重新审批<br/>• 修改记录需留痕 |

### 3.4 事件权限管理

> 💡 **核心逻辑**：同 API 权限分类，事件为底层资源。权限信息在注册时附带定义。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-010 | 事件权限多维分类 | 支持按应用类型、凭证类型构建事件权限分类 | • 同 API 权限多维分类逻辑 |
| FR-011 | 事件权限注册 | 提供方提交事件权限，关联底层事件 | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层事件**（Topic、通道类型等）<br/>• 支持配置**通道类型**（WebHook、WebSocket、内部消息队列）<br/>• **WebSocket 通道仅限个人应用场景下配置** |
| FR-012 | 事件权限编辑 | 提供方修改已注册的事件权限信息 | • 支持修改权限基本信息、通道配置<br/>• 关联的底层事件变更需重新审批 |

### 3.5 能力目录与订阅

> 💡 **交互模式**：消费方视角分为两部分。外侧为**应用权限列表**（展示当前应用已申请/申请中的权限）；点击"添加权限"唤起**全量权限树弹框**（L1-L4 结构），支持多选提交申请。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-013 | 应用权限列表（外侧） | 消费方查看当前应用已申请/申请中的 API/事件权限状态 | • 列表展示：**权限名称**、**codeName**、**认证方式**（如 SOA/APIG）、**分类**、**状态**（已审核/审核中/已中止）<br/>• 支持操作：查看文档、复制审批地址、撤回审核（审核中）<br/>• 提供"添加权限"入口唤起申请弹框 |
| FR-014 | 权限树形选择（弹框） | 消费方通过 L1-L4 树形结构浏览并选择权限 | • 弹框内按**应用类型 -> 凭证类型 -> 分组 -> 权限列表**展示全量权限<br/>• 支持多选加入申请单<br/>• 支持搜索/过滤 |
