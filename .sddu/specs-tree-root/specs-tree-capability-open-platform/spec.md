...
| FR-007 | API 权限注册 | 提供方提交权限信息，关联底层 API | • 支持填写权限基本信息（名称、标识、审批人）<br/>• 支持关联**底层 API**（自动同步 API 属性字段如路径、方法）<br/>• 支持选择关联的**权限分组** |
| FR-008 | API 权限编辑 | 提供方修改已注册的权限信息 | • 支持修改权限名称、审批人、分组<br/>• 关联的底层 API 变更需重新审批<br/>• 修改记录需留痕 |
| FR-009 | 权限分组管理（树形） | 支持提供方自定义树形分组结构 | • 支持新增、编辑、删除分组节点<br/>• 支持拖拽或指定父节点构建层级关系<br/>• 删除分组时需处理关联的权限（如转移至其他分组） |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-013 | 权限目录浏览与检索 | 消费方可按分类树浏览已上架的 API/事件权限 | • 支持按**应用类型 -> 凭证类型 -> 分组**的层级展开浏览<br/>• 支持点击权限查看详情及**跳转说明文档** |
