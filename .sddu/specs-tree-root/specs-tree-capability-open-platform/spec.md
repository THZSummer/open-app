...
| G5 | 提供回调管理能力 | 支持回调的**注册、订阅、取消订阅**、分组、分类；支持配置**通道类型（WebHook、SSE、WebSocket 等）**（注：仅 **WebSocket** 连接方向待定：三方主动连平台 或 平台主动连三方）及**凭证类型（应用类型凭证 A/B）**；支持按应用隔离消费 |
...
| FR-013 | 回调权限注册 | 分组责任人统一注册回调信息，附带定义权限属性 | • **回调关键字段**：中文名称、英文名称、回调文档地址<br/>• **权限关键字段**：中文名称、英文名称、Scope 标识<br/>• 支持指定**资源特有审批流**（含配置审批人/组） |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-019 | 应用事件列表查看与配置 | 消费方查看事件订阅状态，**获批后配置事件接收参数** | • 列表展示：**事件名称**、**Topic**、**状态**（审核中/已授权等）<br/>• **获批权限支持配置事件参数**：支持选择**协议**（企业内部消息平台、WebHook 等）、填写**协议地址**（WebHook 场景）、选择**凭证类型**（应用类型凭证 A/B 等）<br/>• 支持修改/更新配置 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-022 | 应用回调列表查看与配置 | 消费方查看回调订阅状态，**获批后配置回调接收参数** | • 列表展示：**回调名称**、**Scope**、**状态**<br/>• **获批权限支持配置回调参数**：支持选择**协议**（WebHook、SSE 等）、填写**协议地址**、选择**凭证类型**（应用类型凭证 A/B 等）<br/>• 支持修改/更新配置 |
