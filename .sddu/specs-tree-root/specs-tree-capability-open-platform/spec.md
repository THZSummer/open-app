...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-012 | 应用权限列表查看 | 消费方查看当前应用已申请/申请中的 API 权限状态 | • 列表展示：**中文名称**、**Scope**、**认证类型**、**状态**<br/>• 支持操作：**查看文档**、**复制审批地址**、**撤回**（仅“审核中”状态展示）<br/>• 提供"添加 API"入口唤起**订阅抽屉** |
| FR-013 | API 权限树形选择 | 消费方通过 L1-L4 树形结构浏览并选择 API 权限（**抽屉模式**） | • 支持搜索：按**名称**、**Scope**搜索<br/>• 支持过滤：按**是否需要审核**过滤<br/>• 支持多选权限 |
| FR-014 | API 权限申请提交 | 消费方提交 API 权限申请（**独立单据**） | • **无汇总**：每条权限提交后生成**一条单独的申请单**<br/>• 提交成功：**关闭抽屉**<br/>• 提交失败：展示 **Toast 提示** |
