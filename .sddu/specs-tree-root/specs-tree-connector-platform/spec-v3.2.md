...
| G6 | 提供平台托管运行时 | 连接流在平台侧调度执行，消费方无需部署运行时 |
...
> 💡 **定位**：XX 通讯平台内部各业务模块（IM、云盘、审批等）的人员，作为能力供给方，负责将自身模块的 API 封装为**平台连接器**（纯出站端点定义）并注册到连接器平台。MVP 阶段三方平台人员与内部业务模块人员并重——只有平台连接器和连接器都注册后，三方平台人员才能编排连接流将内外连接起来产生新业务。内部业务模块人员的身份机制（是否需要注册应用等）待定。

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-01 | 作为 **业务模块负责人**，我想要 **将本模块的 API 封装为连接器**，以便 **三方平台可通过连接器消费我模块的能力** | P0 | 业务模块负责人可创建连接器基本信息（名称/图标/描述/类型），然后配置连接配置（协议/认证/参数）并发布版本 |
| US-02 | 作为 **业务模块负责人**，我想要 **管理本模块连接器的版本和生命周期**，以便 **控制能力的开放节奏和兼容性** | P0 | 支持连接器编辑、新版本发布；已发布连接器编辑不影响运行中连接流 |
| US-03 | 作为 **业务模块负责人**，我想要 **查看本模块连接器的使用情况**，以便 **了解哪些能力被三方平台消费、消费频率如何** | P1 | 可查看连接器被哪些连接流引用、调用次数统计 |

### 2.2 三方平台人员（开发者 / 业务人员）

> 💡 **定位**：企业内三方平台的人员（通过应用成员身份操作），集两个角色于一身：① **注册连接器** — 将自己的系统（ERP/CRM/OA 等）封装为连接器（纯出站端点定义），使连接流可引用其能力；② **编排连接流** — 消费已注册的平台连接器和连接器，通过可视化编排实现跨系统业务流程。核心诉求：降本提效——通过连接器减少编码工作量；零代码——通过可视化编排配置连接流。MVP 阶段，三方平台人员是业务推广的核心用户。

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-04 | 作为 **三方平台人员**，我想要 **将我的系统（ERP/CRM/OA 等）封装为连接器并注册到平台**，以便 **连接流可引用我系统的能力** | P0 | 三方平台人员可创建连接器基本信息（名称/图标/描述/类型），配置连接配置，提交版本发布审批 |
| US-05 | 作为 **三方平台人员**，我想要 **管理我所注册连接器的版本和生命周期**，以便 **控制自己系统能力的开放节奏** | P0 | 支持连接器编辑、新版本发布；已发布连接器编辑不影响运行中连接流 |
| US-06 | 作为 **三方平台人员**，我想要 **查看我所注册连接器的使用情况**，以便 **了解哪些连接流在引用我的系统能力** | P1 | 可查看连接器被哪些连接流引用、调用次数统计 |
| US-07 | 作为 **三方平台人员**，我想要 **浏览和使用已注册的连接器**，以便 **减少将 XX 平台能力集成到自己系统的编码工作量** | P0 | 消费方可浏览连接器目录，查看连接器基本信息、已发布版本的连接配置详情 |
| US-08 | 作为 **三方平台人员**，我想要 **通过可视化拖拽编排连接流**，以便 **无需硬编码即可串联多个能力完成业务流程** | P0 | 消费方可从流入口节点开始，拖拽连接器节点/数据处理节点到画布，配置数据映射，定义流出口节点，形成完整连接流 |
...
> 💡 **定位**：开放平台运营人员，负责审核连接器发布和连接流版本发布申请，监控平台运行状态，确保合规性与运维可控。

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-14 | 作为 **平台运营人员**，我想要 **审核连接器版本发布申请**，以便 **确保发布连接器的合规性** | P0 | 运营人员可查看连接器发布申请，执行审批操作 |
| US-15 | 作为 **平台运营人员**，我想要 **审核连接流版本发布申请**，以便 **确保连接流的合规运行** | P0 | 运营人员可查看连接流版本发布申请，执行审批操作 |
...
> 💡 **定位**：连接器分两层：**连接器基本信息**（代表连接器本身）和**连接配置**（属于连接器版本）。基本信息描述"是什么连接器"（名称、图标、类型等），连接配置描述"怎么连接"（协议、地址、认证、参数等）。编辑基本信息不影响版本，编辑连接配置生成新版本。MVP 不做上架/下架，所有连接器注册后即可被连接流引用。

#### 3.1.1 连接器注册与管理

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | 连接器创建 | 创建连接器基本信息 | • **名称**：连接器名称<br/>• **图标**：连接器图标<br/>• **描述**：连接器功能描述<br/>• **类型**：协议类型（**MVP 仅支持 HTTP**）<br/>• 创建后连接器基本信息和第一个版本（草稿状态）同时生成 |
| FR-002 | 连接器编辑 | 编辑连接器基本信息 | • 支持修改名称、图标、描述、类型<br/>• **编辑基本信息时同步更新到最新版本的 basic_info_fields**，保证使用侧始终看到最新的基本信息<br/>• **特殊：若所有版本均已发布（无草稿版本），编辑基本信息时自动创建一个新的草稿版本**，基于最新已发布版本复制连接配置，仅更新 basic_info_fields，**自动分配新版本号**<br/>• 基本信息变更需重新发布审批 |
| FR-003 | 连接器删除 | 删除连接器基本信息和所有版本 | • 删除前校验：无运行中的连接流引用该连接器才允许删除<br/>• 有引用时提示影响范围，禁止删除或要求先停用关联连接流<br/>• 删除后不可恢复 |
| FR-004 | 连接器列表查看 | 浏览可用连接器目录 | • 列表展示：连接器名称、图标、描述、类型、最新版本<br/>• 支持按类型过滤；支持搜索 |
| FR-005 | ~~连接器上架/下架~~ **[已移除]** | **MVP 不做上架/下架，移至 NG13** | — |
...
| FR-012 | 连接流编辑 | 编辑连接流基本信息 | • 支持修改名称、描述<br/>• **编辑基本信息时同步更新到最新版本的 basic_info_fields**<br/>• **特殊：若所有版本均已发布（无草稿版本），编辑基本信息时自动创建新草稿版本**，基于最新已发布版本复制连接配置，仅更新 basic_info_fields，自动分配新版本号<br/>• 基本信息变更需重新发布（触发审批） |
...
| FR-015 | 连接流启用/停用 | 控制连接流的运行状态 | • 启用：流变为可运行状态，响应 HTTP 触发或手动触发（需先有已发布版本；**启用无需审批**）<br/>• 停用：停止运行，不再响应触发（**停用无需审批**）<br/>• 已停用连接流可随时重新启用 |

#### 3.2.2 连接流配置（版本）

> 💡 **定位**：连接流配置属于连接流版本的信息，定义连接流的编排内容。编辑连接流配置即编辑最新未发布的草稿版本，发布时**连接流基本信息+连接流配置**一并写入版本表快照。使用侧直接使用最新版本即可。

**编排的流逻辑节点体系共 6 种基础节点类型**，类比代码函数体的不同语句类型。**MVP 支持连接器节点 + 数据处理节点**，其余节点类型属 V1 范围：
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-016 | 连接流配置查看 | 查看连接流的编排配置（默认查询最新版本） | • 默认展示最新已发布版本的完整编排配置<br/>• 无已发布版本时展示最新草稿版本<br/>• 展示内容：流入口节点配置、连接器节点/数据处理节点列表及参数、节点间数据映射、流出口节点配置 |
| FR-017 | 连接流配置编辑 | 编辑连接流的编排内容（仅允许编辑最新未发布的版本） | • 编辑目标始终为「最新未发布的草稿版本」<br/>• 无草稿版本时，基于最新已发布版本创建草稿，自动分配新版本号<br/>• 草稿版本可重复编辑，每次保存覆盖当前草稿<br/>• 已发布版本不可编辑（只读）<br/>• **可编辑内容**：① 流入口节点：触发方式（HTTP/手动）及配置、触发数据 Schema<br/>② 连接器节点：选择已发布连接器的版本，配置输入参数（支持引用上游节点输出/常量值）<br/>③ 数据处理节点：配置字段映射（源字段→目标字段）、内置函数转换（类型转换/字符串处理/数值运算等）、条件过滤<br/>④ 节点间数据映射：源字段→目标字段一对一映射，可视化展示<br/>⑤ 流出口节点：返回字段列表定义，返回值格式选择<br/>• 提供拖拽式可视化编排画布：画布三区域（入口区→编排区→出口区），支持拖拽、缩放、平移、删除 |
| FR-018 | 版本切换 | 切换查看历史版本的连接流配置 | • 支持查看版本列表（版本号、发布时间、状态、变更说明）<br/>• 支持切换到任意历史版本查看其编排配置（只读）<br/>• 版本切换仅为查看操作，不改变当前编辑目标 |
| FR-019 | 版本发布 | 将草稿版本发布为已发布版本 | • 数据已在编辑阶段（FR-012/FR-017）写入版本表，版本号已在创建草稿时分配<br/>• **发布动作仅更新状态**：草稿→已发布<br/>• 发布前校验：编排配置完整（至少有一个节点（连接器节点或数据处理节点））<br/>• 校验通过后自动进入审批流程（复用审批引擎）<br/>• 审批通过后新版可用，使用侧直接使用最新版本<br/>• 已引用旧版本的运行中连接流不受影响 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-022 | ~~事件触发调度~~ **[已移除]** | **事件触发器移至 NG16** | — |
| FR-023 | HTTP 触发调度 | 接收 HTTP 请求，触发对应连接流（异步） | • 为每个 HTTP 触发的连接流分配独立 URL<br/>• 接收 HTTP 请求后，解析请求体作为触发数据<br/>• **每次触发生成唯一执行ID**，异步调度执行，立即返回执行ID<br/>• 支持请求签名验证，防止非法调用 |
| FR-024 | ~~定时触发调度~~ **[已移除]** | **定时触发器移至 NG17** | — |
| FR-025 | 手动触发调度 | 响应用户手动触发操作（异步） | • 支持界面"立即执行"和"测试运行"两种手动触发<br/>• **每次触发生成唯一执行ID**，异步调度执行，立即返回执行ID |
...
| FR-028 | ~~资源配额与隔离~~ **[已移除]** | **移至 NG14** | — |
| FR-029 | 节点级错误处理 | 单个节点执行失败时的处理 | • 节点执行失败时，标记该节点为「失败」<br/>• 节点失败后，连接流整体标记为「执行失败」<br/>• 失败上下文数据保留，供排查 |
| FR-030 | 连接流级错误处理 | 连接流执行失败后的处理 | • 连接流执行失败时，发送告警通知（通知方式待定）<br/>• 失败的执行记录保留，供排查 |

#### 3.3.2 监控日志

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-031 | 连接流运行状态查看 | 消费方查看连接流的当前运行状态 | • 状态类型：运行中/已停用/执行失败/审批中<br/>• 支持查看最后执行时间 |
...
| FR-037 | ~~审批流复用 - 连接流部署~~ **[已移除]** | **部署/启停无需审批，仅版本发布需审批（已由 FR-019 覆盖）** | — |
...
| NFR-009 | 权限控制 | 连接器发布仅限连接器提供方（业务模块负责人）；连接流操作仅限创建者及管理员；Scope 权限复用能力开放平台模型 |
...
| 模块 | 主要接口 | 说明 |
|------|---------|------|
| 连接器管理 | 连接器CRUD、版本管理、连接配置查看/编辑/发布 | 供给方管理连接器 |
| 连接流管理 | 连接流CRUD、版本管理、编排配置查看/编辑/发布、启停、测试 | 消费方管理连接流 |
| 运行监控 | 执行历史列表、执行详情、执行状态查询、平台运行指标 | 运维监控连接流运行 |
| HTTP 触发入口 | HTTP 触发 | 外部系统通过 HTTP 请求触发连接流 |
...
> 💡 能力开放平台负责 API、事件、回调的**注册与订阅管理**，核心是**Scope 权限**。连接器平台定义的连接器或连接流若依赖内部业务系统提供的 API/事件，需通过能力开放平台申请对应 Scope 权限。
...
graph TB
    ...
    Biz -->|注册 API/事件| CapPlat
    C -->|关联 API Scope| API
    C -->|需要已获得 API Scope 权限| Sub
    F -->|关联事件 Scope| Event
    F -->|需要已获得事件 Scope 权限| Sub
    Sub -->|复用| Approval
    CapPlat -.->|提供 Scope 鉴权| ConnPlat

    style ConnPlat fill:#e1bee7,stroke:#7b1fa2
...
| 集成点 | 方向 | 说明 |
|--------|------|------|
| **API Scope 权限申请** | 连接器平台 → 能力开放平台 | 连接器定义若引用内部系统 API，需在能力开放平台申请对应 API 的 Scope 权限，获得授权后才能被连接流引用调用 |
| **事件 Scope 权限申请** | 连接器平台 → 能力开放平台 | **V1 阶段**：连接流入口节点若使用事件触发，需在能力开放平台申请对应事件的 Scope 权限，获得授权后才能订阅该事件 |
| **Scope 鉴权** | 连接器平台 → 能力开放平台 | 运行时连接器调用 API 时，通过能力开放平台进行 Scope 鉴权（MVP 无事件触发，仅涉及 API Scope） |
| **审批流复用** | 连接器平台 → 能力开放平台 | 连接器版本发布、连接流版本发布复用能力开放平台的审批引擎 |
| **回调触发连接流** | 能力开放平台 → 连接器平台 | 未来可在能力开放平台的回调配置中支持**连接流类型**的回调目标。内部业务系统触发回调时，不再直接调用外部系统，而是驱动连接流执行，通过编排的连接流将数据路由至外部系统 |
...
| EC-003 | 连接流执行中，被引用连接器的认证凭证过期 | 执行失败，触发告警通知，提示更新凭证 |
| EC-004 | ~~事件触发时，同一事件驱动大量连接流并发执行~~ **[已移除]** | **事件触发器移至 NG16** |
| EC-005 | HTTP 触发 URL 被非法调用 | 签名验证失败返回 401；记录非法调用日志；支持限流 |
...
| EC-009 | 能力开放平台 API/事件网关不可用 | 连接流执行失败，触发告警；运行时检测网关健康状态 |
| EC-010 | 连接流编排为空（仅有流入口节点无连接器节点） | 编排校验不通过，禁止提交发布，提示至少添加一个连接器节点 |
...
| 版本 | 范围 | 核心价值 |
|------|------|---------|
| **MVP** | 平台连接器（纯出站端点定义）+ 连接流线性编排（HTTP/手动触发器→连接器节点/数据处理节点→流出口节点）+ 平台托管运行时 + 运行监控 + 基础数据映射 | 验证"零代码配置连接流"核心价值 |
| **V1** | 完整编排（分支/循环/子流程）+ 代码模式互转 + 函数库+脚本 + 连接器 SDK + 非 HTTP 协议连接器（MySQL/Redis 等）+ 连接器上架/下架 + 资源配额与隔离 + 失败重试 + 事件/定时触发器 | 满足复杂场景，开放开发者生态 |
...
| 需求挖掘报告需求编号 | 对应本规范 FR |
|---------------------|--------------|
| MH-01 平台连接器 | FR-001 ~ FR-004, FR-006 ~ FR-010 |
| MH-02 连接器生命周期管理 | FR-001 ~ FR-004, FR-006, FR-010, FR-036 |
| MH-03 连接流线性编排 | FR-011 ~ FR-021 |
| MH-04 连接流生命周期管理 | FR-011 ~ FR-015, FR-019 |
| MH-05 基础数据转换 | FR-017（作为连接流配置编辑的一部分） |
| MH-06 平台托管运行时 | FR-022 ~ FR-027, FR-029 ~ FR-030（事件触发 FR-022 / 定时触发 FR-024 已移除） |
