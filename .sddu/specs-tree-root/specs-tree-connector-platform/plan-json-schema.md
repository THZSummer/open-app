  ...
  "definitions": {
    ...
    "connectorDataDef": {
      ...
      "properties": {
        ...
        "inputMapping": {
          "type": "object",
          "description": "字段映射。下游连接器需要什么字段，在此声明字段值从上游哪个表达式获取。映射结构镜像连接器 inputContract 的协议结构（HTTP: header/query/body），详见 §1.4"
        }
      },
    ...
    "exitDataDef": {
      ...
      "properties": {
        ...
        "outputMapping": {
          "type": "object",
          "description": "字段映射。对外返回什么字段，在此声明字段值从上游哪个表达式获取。映射结构镜像 exit 的协议结构（HTTP: header/body），详见 §1.4"
        }
      },
...
| inputMapping | object | ✅ | 字段映射，结构镜像连接器 inputContract 协议（HTTP: header/query/body）。key 为 inputContract 字段名，value 为表达式，详见 §1.4 |
...
| labelCn | string | ❌ | 节点中文标签 |
| labelEn | string | ❌ | 节点英文标签 |
| outputMapping | object | ✅ | 字段映射，结构镜像 exit 协议（HTTP: header/body）。key 为对外返回的字段名，value 为表达式，详见 §1.4 |
...
| **v5.3** | 2026-05-26 | **修复：description 字段传播**：① 所有 httpContract 示例中的 description 字段补齐；② dataProcessorDataDef 中 `result.id` / `result.status` 等文字描述统一；③ $ref example 格式修复（移除 description 多行） | SDDU Plan Agent |
