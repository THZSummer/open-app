...
**创建日期**: 2026-05-22  
**最后更新**: 2026-05-26
**修订说明**: v5.0 — 结构性重构：按存储目标与框架归属明确边界——§4 独立承载连接器配置（connector_version_t），§5 承载连接流编排配置并清晰拆分为框架渲染层（§5.2）与业务拓展层（§5.3），业务拓展层下分 node.data（§5.3.1）和 edge.data（§5.3.2）
...
> 本配置中引用的各字段定义详见 §3.3：`authConfig`（§3.3.1）、`rateLimitConfig`（§3.3.2）、`inputContract`/`outputContract`（§3.3.3）。
...
| inputContract | object | ❌ | 入参数据契约 | §3.3.3 dataContractDef |
| outputContract | object | ❌ | 出参数据契约 | §3.3.3 dataContractDef |
| timeoutMs | integer | ❌ | 单次调用超时（毫秒），默认 30000 | — |
...
`nodeDataDef` 是 oneOf 路由定义，根据 `node.type` 选择对应的 data Schema。完整 Schema 定义见 §3.2 → `nodeDataDef`，详细说明见 §3.3.6。
