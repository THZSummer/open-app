...
**创建日期**: 2026-05-22  
**最后更新**: 2026-05-26
**修订说明**: v5.1 — 入参出参按协议区分：删除协议无感的 dataContractDef / dataContractDefAlias，新增 jsonSchemaObjectDef（纯字段形状）、httpInputContractDef（HTTP入参 header/query/body）、httpOutputContractDef（HTTP出参 header/body）、inputContractDef / outputContractDef（oneOf 协议路由器）；connectionConfig / triggerDataDef 引用链更新
...
nodeDataDef 本身是 oneOf 路由定义，没有独立的数据示例。各子类型的示例见对应定义（§3.3.11-§3.3.14）。

##### 3.3.10.3 def 字段说明

| 路由条件 | 目标定义 | 说明 |
|----------|---------|------|
| `node.type` = `"trigger"` | `triggerDataDef`（§3.3.11） | 触发器节点业务数据 |
| `node.type` = `"connector"` | `connectorDataDef`（§3.3.12） | 连接器节点业务数据 |
| `node.type` = `"data_processor"` | `dataProcessorDataDef`（§3.3.13） | 数据处理器节点业务数据 |
| `node.type` = `"exit"` | `exitDataDef`（§3.3.14） | 出口节点业务数据 |

#### 3.3.11 triggerDataDef
...
```json
{
  ...
  "properties": {
    ...
    "position": { "$ref": "#/definitions/positionDef", "description": "画布坐标" }
  },
...
| position | object | ✅ | 框架字段，不进 data。画布坐标（浮点数），React Flow 框架字段 |
...
**Schema 定义**（完整定义见 §3.3.14，以下为独立展示）：
...
`nodeDataDef` 是 oneOf 路由定义，根据 `node.type` 选择对应的 data Schema。完整 Schema 定义见 §3.2 → `nodeDataDef`，详细说明见 §3.3.10。
