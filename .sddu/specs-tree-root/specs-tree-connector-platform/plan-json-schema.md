...
> 💡 以下可视化对比中，`type: "connector"`、`labelCn`、`connectorVersionId` 等均为**本项目的自定义注册类型和业务字段**（非 React Flow 内置），React Flow 框架仅提供 `input`/`default`/`output`/`group` 四种内置类型（见 §7.2.2）。本对比**仅展示 `node.data` 嵌套 vs 平铺的结构差异**，不暗示 React Flow 包含任何业务节点类型。
...
React Flow 的 `node.type` 同时承担两个职责（见 §7.2.1）：
...
| 节点业务类型 | `node.type`（注意：type 为项目自定义注册类型，见 §7.2.1） | `node.getType()` |
...
> 💡 **关于 `type` 值**：以下示例中的 `"type": "trigger"`、`"type": "connector"`、`"type": "data_processor"`、`"type": "exit"` 是**本项目的自定义注册类型**（见 §7.2.1），不是 React Flow 内置类型。React Flow 框架本身不定义这些字符串——它们只在我们的 `nodeTypes` 注册表中有意义。存储格式使用这些值，是因为它们同时兼顾了 React Flow 渲染映射和后端运行时业务分类的双重职责。
