...
**连接器平台**是 open-app 体系下能力开放平台的**自然演进**，作为独立 Feature 以**松耦合**方式与能力开放平台协作。它将能力开放平台已开放的 API/事件/回调作为触发器和动作源，通过**连接器封装**和**集成流编排**，将原本需要大量人工编码的跨系统集成场景转化为**低代码/零代码配置**，释放能力开放平台的真正价值。
...
> 💡 **关系解读**：
> 1. **演进关系**：连接器平台是能力开放平台 MVP 的自然演进，不是替代，而是互补
> 2. **松耦合**：连接器平台是独立 Feature，不内嵌于能力开放平台，可对接任意系统
...
| 集成方向 | 触发方式 | 典型场景 |
|---------|---------|---------|
| **XX平台→外部系统** | 事件触发 | IM消息→ERP创建工单、审批通过→CRM更新 |
| **外部系统→XX平台** | Webhook触发 | ERP数据变更→XX平台同步、外部事件→XX平台动作 |
...
| **松耦合** | 连接器平台不绑定能力开放平台，可对接任意系统的 API |
