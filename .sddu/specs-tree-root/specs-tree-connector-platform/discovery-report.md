...
| **业务人员（低代码用户）** | 不写代码但需要配置跨系统业务流程的运营/HR/客服人员 | **零代码**：通过可视化/表单编排配置连接流，无需开发人员介入 |
...
| **触发器** | 连接器的入口——**感知**：知道发生了什么（入站） | 钉钉的"触发器"、Zapier 的"Trigger" |
| **动作** | 连接器的出口——**执行**：去做什么（出站） | 钉钉的"动作"、Zapier 的"Action" |
...
| 版本 | 范围 | 核心价值 |
|------|------|---------|
| **MVP** | 预置连接器 + 线性编排 + 平台托管运行 + 运行监控 + 基础数据映射 | 验证"零代码配置连接流"核心价值 |
