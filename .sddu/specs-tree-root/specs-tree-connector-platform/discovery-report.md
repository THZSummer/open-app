...
**连接器平台**是开放平台的组成部分，提供与 API、事件、回调**同级并列**的第四种开放形式。四种开放形式共同服务于同一目标：连接 XX 通讯平台内部与企业内其它三方平台，促成对内对外的连接。连接器是**可选项/锦上添花**——没有连接器，消费方也能通过 API/事件/回调达成目标；有了连接器，可以将原本需要大量人工编码的跨系统集成场景转化为**低代码/零代码配置**，使原有的开放更加便捷。
...
| **不做后果** | 可接受但不够便捷 — 没有连接器也能通过 API/事件/回调达成目标，但三方平台接入仍需大量人工编码，效率无法进一步提升 |
| **现状** | 能力开放平台 MVP 已就绪（API/事件/回调/权限/审批），基本开放能力已具备，但跨系统的集成编排场景仍需人工编码 |
| **目标** | 构建连接器平台，作为 API/事件/回调同级并列的第四种开放形式，将人工编码的集成场景转化为低代码/零代码配置，使原有开放更便捷 |
...
| 影响维度 | 后果 |
|---------|------|
| **业务影响** | 无实质影响 — 三方平台仍可通过 API/事件/回调达成目标，只是需要更多人工编码 |
| **效率影响** | 跨系统集成的场景仍需人工编码编排，无法通过低代码方式配置 |
| **体验影响** | 缺少便捷的集成流编排能力，业务人员无法自主配置跨系统流程 |
...
| 启示 | 来源 | 我们的策略 |
|------|------|-----------|
| **能力开放 > 连接器封装** | 飞书调研 | API/事件/回调是基础，连接器是同级并列的可选项，使开放更便捷 |
