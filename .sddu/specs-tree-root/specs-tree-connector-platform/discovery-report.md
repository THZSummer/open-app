...
连接器平台的概念体系按"代码流程"类比组织为三层：**定义层**（函数签名/API 定义）、**编排层**（函数体/语句）、**执行层**（运行时的实际调用）。
...
graph TB
    subgraph Def["📄 定义层（静态）"]
        ...
        T["触发器<br/>流程启动条件定义<br/>手动·定时·Webhook·事件"]
    end

    subgraph Flow["🔀 编排层（动态·可执行）"]
        direction TB
        EN["流入口节点<br/>（唯一，选择触发器类型）"]
        LN["流逻辑节点（多个·有序）"]
        LN1["├─ 连接器节点<br/>调用外部系统"]
    ...
    style Def fill:#f3e5f5,stroke:#7b1fa2
...
| 概念 | 层级 | 代码类比 | 本质 | 定义 | 包含 |
|------|------|---------|------|------|------|
| **连接器** | 定义层 | 函数签名 / API 文档 | **外部系统的端点定义** | 封装调用一个外部系统的完整技术信息；一个连接器对应一个外部系统的**单一功能点**（如"发送消息"为一个连接器，"查询用户"为另一个连接器），不包含任何触发或编排逻辑 | 协议（HTTP/RPC/DB）、地址（URL/连接串）、认证方式（OAuth/API Key/Basic）、输入参数 Schema、输出返回值 Schema、超时时间、重试策略 |
| **触发器** | 定义层 | main 函数的触发条件 | **流程的启动条件** | 定义连接流在什么条件下开始执行；独立于任何连接器存在，是一个流有且仅有一个的起点 | 手动触发（用户点击）、定时触发（Cron 表达式）、Webhook 触发（HTTP 请求回调）、事件触发（平台事件订阅） |
| **连接流** | 编排层 | 一个完整的函数 | **可执行的业务流程** | 由三个部分有向连接而成：一个流入口节点 + 多个流逻辑节点 + 可选的流出口节点 | 节点类型 + 节点间连线（顺序/条件/循环）+ 节点间参数映射规则 |
| ├─ 流入口节点 | 编排层 | main() 的函数签名 | **流程的唯一起点** | 引用一个触发器类型，并配置其具体参数（如 Cron 表达式、Webhook URL） | 引用触发器 + 入参配置 |
...
| │ ├─ 控制节点 | 流逻辑子类 | if/for/switch | **变更执行路径** | 改变流程执行走向：分支（if/else/switch）、循环（for/while）、并行（fork/join） | 分支条件定义、循环次数/条件、并行分支数 |
| │ ├─ 数据处理节点 | 流逻辑子类 | map/filter/reduce | **数据加工** | 在节点间对数据进行格式转换和运算 | 字段映射、内置函数（字符串/日期/数学/数组）、自定义脚本（JS/Python） |
...
> 💡 **为什么这样设计**：用代码流程类比是为了消除概念模糊。连接器 = API 文档（不执行），触发器 = 触发条件（入口），流逻辑节点 = 函数体语句（执行体），运行时 = 虚拟机（落地执行）。三层分离，职责单一，无重叠。

**表格二：各竞品核心概念对照**

| 我们的概念 | 我们的做法 | Zapier | Make | 钉钉 | 集简云 |
|-----------|----------|--------|------|------|--------|
| **连接器**（端点定义） | 单一功能，纯出站，不内置触发器 | App 的 Action / Search | App 的 Action / Search | 连接器的"动作"部分 | 动作 |
| **触发器**（启动条件） | 独立概念，不属于连接器 | App 内置的 Trigger + 独立 Schedule | App 内置的 Trigger + 独立调度 | 连接器内置触发器 + 独立定时/手动 | 触发器 + 独立定时 |
| **连接流**（业务流程） | 流入口 + 流逻辑节点 + 流出口 | Zap | Scenario | 连接流 | 数据流程 |
| **连接器节点**（调用外部系统） | 引用连接器，传入参数取回结果 | Zap 中的 Action 步骤 | Scenario 中的 Action 模块 | 连接流中的动作 | 数据流程中的动作 |
| **控制节点**（逻辑分支） | 独立节点类型 | Paths（有限） | Router + Iterator + Aggregator | 条件判断（简单） | 条件判断 + 循环 |
| **数据处理节点**（数据加工） | 独立节点类型 | Formatter | 300+ 内置函数 | 字段映射 | 字段映射 + 转换 |
| **运行时**（执行引擎） | 平台托管 | 平台托管 | 平台托管 | 平台托管 | 平台托管 |

> 💡 **与竞品的关键差异**：Zapier/钉钉将 Trigger 内嵌在连接器内部（连接器=入站+出站），我们将触发器完全独立。原因是：**触发器本质是"启动条件"而非"系统接入"**。手动/定时/事件触发与具体系统的接入方式无关，独立后概念更纯粹；且当同一个外部系统需要被多个流以不同方式触发时，无需重复封装连接器。
...
| v3.1 | 2026-05-19 | 核心概念重构（3.2节）：① 建立"定义层→编排层→执行层"三层架构模型，消除概念重叠；② 连接器重构为单一功能点（纯出站端点定义），不再内嵌触发器/动作/查找三种能力；③ 触发器独立为流程启动条件概念，不属于任何连接器；④ 编排层抽象为流逻辑节点体系（连接器节点/控制节点/数据处理节点/流入口节点/流出口节点），以代码流程类比消除编排黑盒；⑤ 新增竞品对照表（Zapier/Make/钉钉/集简云逐项对比）；⑥ 新增三层概念拓扑图 | AI Assistant |
