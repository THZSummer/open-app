...
> 💡 以下规范全面对齐能力开放平台（CAP-OPEN-001 `plan.md §4.2 表设计规则`），连接器平台子域作为 openplatform_v2 体系一部分严格遵循。
...
CREATE TABLE `openplatform_v2_cp_connector_t` (
  ...
  `tags`              varchar(500)  DEFAULT NULL              COMMENT '标签（逗号分隔，选填）',
...
> **变更说明**（相对 v2.0）：① 表名加 `openplatform_v2_cp_` 前缀 + `_t` 后缀；② 删除 `connector_id varchar(32)`（直接用 BIGINT `id` 作业务标识）；③ `name` → `name_cn`/`name_en` 双语；④ `description varchar(2000)` → `description_cn`/`description_en` VARCHAR(1000) 双语；⑤ 新增 `tags`；⑥ `connector_type`/`status` 从 varchar 改为 TINYINT(10)；⑦ 审计字段 `created_at/updated_at/created_by/updated_by` → `create_time/last_update_time/create_by/last_update_by`；⑧ 删除 `is_deleted`（MVP 物理删除）。
...
| `OAUTH2_CLIENT` | OAuth2 Client Credentials | `access_token`（调用方完成 OAuth2 授权流后传入） |
| `BASIC_AUTH` | HTTP Basic Auth | `username`, `password` |
| `API_KEY` | API Key (header/query) | `key_value`（位置由 schema 的 `carrier`/`field_name` 声明） |
...
CREATE TABLE `openplatform_v2_cp_flow_t` (
  ...
  `description_en`                  varchar(1000) DEFAULT NULL              COMMENT '英文描述（选填）',
  `tags`                            varchar(500)  DEFAULT NULL              COMMENT '标签（逗号分隔，选填）',
  `owner_group`                     varchar(100)  DEFAULT NULL              COMMENT '归属组（选填）',
...
> **变更说明**（相对 v2.0）：① 表名前缀+后缀对齐；② 删除 `flow_id varchar(32)`；③ `name` → `name_cn`/`name_en` 双语；④ `description varchar(2000)` → `description_cn`/`description_en` VARCHAR(1000) 双语；⑤ 新增 `tags`/`owner_group`；⑥ `status enabled/disabled` → `lifecycle_status` TINYINT (0=stopped, 1=running)；⑦ **新增 `current_published_version_id` 指针**（v2.4，HTTP 触发查找路径，借鉴 PA flowversions + 钉钉模式）；⑧ 删除 `is_deleted`；⑨ 索引 `idx_current_published_version_id` 用于 HTTP 触发查找。
...
CREATE TABLE `openplatform_v2_cp_flow_version_t` (
  ...
  `version_description_cn`   varchar(1000) DEFAULT NULL              COMMENT '版本变更说明（中文）',
  `version_description_en`   varchar(1000) DEFAULT NULL              COMMENT '版本变更说明（英文）',
  `basic_info_snapshot`      mediumtext    DEFAULT NULL              COMMENT 'TEXT 存 JSON：发布时连接流基本信息快照 {name_cn,name_en,description_cn,description_en,tags,owner_group}',
