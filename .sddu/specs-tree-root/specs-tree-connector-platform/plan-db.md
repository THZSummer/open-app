...
| **v3.0** | **2026-05-26** | **对齐 plan-json-schema.md v5.5**：① 新增 §0.6 JSON 结构定义源规范，明确 JSON 内部结构以 plan-json-schema.md 为权威定义；② §3.2 / §3.4 删除内嵌 JSON 示例（~110 行），替换为交叉引用；③ §3.2 / §3.4 DDL 下方新增 `📐` 交叉引用行；④ 认证枚举段精简为 JSON 字符串 ↔ TINYINT 映射表 + 交叉引用；⑤ 变更说明更新字段引用。**SQL DDL 零变更**。 | SDDU Plan Agent |
