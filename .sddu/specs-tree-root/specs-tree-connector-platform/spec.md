...
| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-01 | 作为 **业务模块负责人**，我想要 **将本模块的 API/事件/回调封装为连接器（纯出站端点定义）**，以便 **三方平台可通过连接器消费我模块的能力** | P0 | 业务模块负责人可创建连接器，定义调用方式和认证配置，提交发布审批 |
...
| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-04 | 作为 **三方平台人员**，我想要 **将我的系统（ERP/CRM/OA 等）封装为连接器并注册到平台**，以便 **连接流可引用我系统的 API 能力** | P0 | 三方平台人员可创建连接器，定义调用方式和认证配置（封装自身系统的 API），提交发布审批 |
...
| US-07 | 作为 **三方平台人员**，我想要 **浏览和使用已上架的连接器**，以便 **减少将 XX 平台能力集成到自己系统的编码工作量** | P0 | 消费方可浏览连接器目录，查看连接器的调用方式、认证类型和参数 Schema |
...
> 💡 **定位**：连接器是对单个外部系统的**纯出站端点定义**（类比 import 的模块/库），定义单个功能点的完整调用信息（调用方式、认证、入参/出参 Schema），**不含触发器**。连接器是可复用的原子单元，一个连接器可被多个连接流引用。触发器归流平台编排层管理，作为流入口节点独立于连接器存在。
...
| FR-002 | 连接器编辑 | 连接器提供方编辑连接器的基本信息、调用方式和认证配置 | • 支持修改基本信息、调用方式、入参/出参 Schema<br/>• 已发布连接器编辑后生成新版本草稿，不影响已引用该连接器的运行中连接流<br/>• 修改记录留痕 |
| FR-003 | 连接器发布 | 连接器提供方提交连接器发布申请，经审批后上架 | • 提交发布前校验：调用方式和认证配置完整，入参/出参 Schema 完整<br/>• 提交发布后自动进入审批流程（复用能力开放平台审批引擎）<br/>• 审批通过后连接器状态变为「已发布」，可被连接流引用<br/>• 已发布连接器的新版本发布需重新审批 |
...
| FR-006a | 连接器使用统计 | 连接器提供方查看本模块连接器的使用情况 | • 展示连接器被引用的连接流数量和列表<br/>• 展示连接器的调用次数统计<br/>• 支持按时间范围查询 |

| FR-009 | 连接器调用方式定义 | 定义连接器调用外部系统的具体方式 | • 连接器定义调用方式类型：API 调用（调用外部系统 API）/ 事件发布（向事件总线发布事件）/ 回调通知（向外部系统发送回调）<br/>• 一个连接器仅对应一种调用方式（单一功能点原则）<br/>• 定义调用方式的入参 Schema 和出参 Schema<br/>• API 调用：配置关联的能力开放平台 API（路径/方法/Scope）<br/>• 事件发布：配置关联的事件 Topic<br/>• 回调通知：配置回调通知的目标地址格式和认证方式 |
...
> 💡 **定位**：连接流是消费方直接使用的对象，由三个部分有向连接而成：**流入口节点**（触发器 — 定义启动条件）→ **流逻辑节点**（MVP 含连接器节点）→ **流出口节点**（可选 — 定义返回值）。通过编排多个节点实现跨系统业务流程。
...
| FR-015 | 可视化拖拽编排 | 通过拖拽式可视化编排器编排连接流 | • 提供画布式编排界面，左侧为连接器面板（可拖拽的连接器节点）<br/>• 画布预置三个区域：流入口节点区（顶部）→ 逻辑节点编排区（中部）→ 流出口节点区（底部）<br/>• 从连接器面板拖拽连接器节点到编排区，通过连线表示数据流转方向<br/>• 画布支持缩放、平移<br/>• 节点支持选中、删除、复制<br/>• 连线支持删除 |
...
| FR-017 | 线性编排 | 支持流入口节点→连接器节点→流出口节点的线性编排 | • 第一个固定为流入口节点（事件/Webhook/定时/手动）<br/>• 中间节点为连接器节点，按顺序串联<br/>• 最后固定为流出口节点（可选，无出口为 fire-and-forget）<br/>• 每个连接器节点可选择已发布的连接器<br/>• 支持添加、删除、调整连接器节点顺序 |
| FR-018 | 连接器节点配置 | 配置每个连接器节点的参数 | • 为连接器节点配置输入参数（支持引用上游节点的输出）<br/>• 为连接器节点配置认证凭证<br/>• 配置节点级别的错误处理策略（重试次数/重试间隔） |

#### 3.2.2 数据转换

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-019 | 字段映射配置 | 配置源字段到目标字段的映射 | • 在连接器节点配置界面，提供字段映射编辑器<br/>• 支持从上游节点输出 Schema 中选择源字段<br/>• 支持映射到当前连接器节点的输入参数<br/>• 支持一对一映射（源字段→目标字段）<br/>• 映射配置可视化展示（源字段名→目标字段名） |
| FR-020 | 常量值配置 | 配置连接器节点的固定参数值 | • 支持为连接器节点的输入参数配置常量值<br/>• 常量值与字段映射互斥（同一参数只能二选一） |
...
| 节点类型 | 代码类比 | 描述 | MVP 范围 |
|---------|---------|------|---------|
| **连接器节点** | 调用外部函数 | 引用某个已发布连接器，传入输入参数获取输出返回值 | ✅ MVP |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-021 | 连接流编辑 | 消费方编辑已创建的连接流 | • 支持修改连接流编排、流入口节点配置、连接器节点配置、数据映射<br/>• 已启用连接流编辑后生成新版本草稿，不影响当前运行版本<br/>• 编辑后需重新提交部署审批 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-036 | 节点级错误处理 | 单个连接器节点执行失败时的处理 | • 支持配置重试策略（重试次数、重试间隔）<br/>• 重试次数耗尽后，标记该节点为「失败」<br/>• 节点失败后，连接流整体标记为「执行失败」 |
...
| 数据实体 | 关键字段 | 说明 |
|---------|---------|------|
| **Connector** | id, name, code_name, type(preset/custom), icon, description, status, version, call_type(api_call/event_publish/callback), input_schema, output_schema, auth_config | 连接器主体（纯出站端点定义，定义单个功能点的调用方式、入参/出参 Schema、认证） |
...
| **FlowNode** | id, flow_id, node_type(entry/connector/exit), connector_id, config, position | 连接流节点（入口节点/连接器节点/出口节点） |
...
| 需求挖掘报告需求编号 | 对应本规范 FR |
|---------------------|--------------|
| MH-01 平台连接器 | FR-001 ~ FR-006, FR-006a, FR-009, FR-012, FR-013 |
