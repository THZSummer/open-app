...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-005 | 连接配置查看 | 查看连接器的连接配置（默认查询最新版本） | • 默认展示最新已发布版本的连接配置<br/>• 无已发布版本时展示最新草稿版本<br/>• **无任何版本时提示"暂无配置"**<br/>• 展示内容：协议类型、协议地址、认证方式、入参 Schema、出参 Schema、超时设置、限流设置 |
| FR-006 | 连接配置编辑 | 编辑连接配置（仅允许编辑最新未发布的版本） | • 编辑目标始终为「最新未发布的草稿版本」<br/>• **无草稿版本且无任何已发布版本时，创建首个草稿版本（v1）**<br/>• 无草稿版本但有已发布版本时，基于最新已发布版本创建草稿，**自动分配新版本号**<br/>• 草稿版本可重复编辑，每次保存覆盖当前草稿<br/>• 已发布版本不可编辑（只读）<br/>• **可编辑内容**：协议类型、协议地址、**认证方式及认证凭证**、入参定义、出参定义、超时设置、限流设置等<br/>• 认证凭证加密存储，界面脱敏显示 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-016 | 连接流配置查看 | 查看连接流的编排配置（默认查询最新版本） | • 默认展示最新已发布版本的完整编排配置<br/>• 无已发布版本时展示最新草稿版本<br/>• **无任何版本时提示"暂无配置"**<br/>• 展示内容：流入口节点配置、连接器节点/数据处理节点列表及参数、节点间数据映射、流出口节点配置 |
| FR-017 | 连接流配置编辑 | 编辑连接流的编排内容（仅允许编辑最新未发布的版本） | • 编辑目标始终为「最新未发布的草稿版本」<br/>• **无草稿版本且无任何已发布版本时，创建首个草稿版本（v1）**<br/>• 无草稿版本但有已发布版本时，基于最新已发布版本创建草稿，自动分配新版本号<br/>• 草稿版本可重复编辑，每次保存覆盖当前草稿<br/>• 已发布版本不可编辑（只读）<br/>• **可编辑内容**：① 流入口节点：触发方式（HTTP/手动）及配置、触发数据 Schema<br/>② 连接器节点：选择已发布连接器的版本，配置输入参数<br/>③ 数据处理节点：配置字段映射、内置函数转换、条件过滤<br/>④ 节点间数据映射<br/>⑤ 流出口节点：返回字段列表定义<br/>• 提供拖拽式可视化编排画布：画布三区域（入口区→编排区→出口区），支持拖拽、缩放、平移、删除 |
