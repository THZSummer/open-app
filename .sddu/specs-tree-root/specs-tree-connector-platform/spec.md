...
**创建日期**: 2026-05-18  
**最后更新**: 2026-05-18  
**需求挖掘报告**: discovery-report.md (v2.2)
...
| # | 目标 | 衡量标准 |
|---|------|---------|
| G1 | 提供连接器封装能力 | 支持对 XX 平台能力（API/事件/回调）的标准化封装，形成可复用的连接器；连接器包含触发器、动作、认证配置、参数定义 |
| G2 | 提供连接器生命周期管理 | 支持连接器的注册、编辑、发布、下架、版本管理；发布时复用能力开放平台审批流程 |
| G3 | 提供连接流可视化编排 | 支持通过可视化拖拽编排器创建连接流，串联触发器→动作链；支持事件/Webhook/定时/手动四种触发方式 |
...
| NG6 | 支持自定义连接器发布 | Could Have，V2 阶段 |
...
### 2.1 连接器提供方（XX 平台内部业务模块负责人）

> 💡 **定位**：XX 通讯平台内部各业务模块（IM、云盘、审批等）的负责人，负责将自身模块能力封装为连接器并上架到连接器平台，使三方平台消费方可以使用。只有内外的连接器都先上架到连接器平台，才能完成内外的连接产生新的业务。

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-01 | 作为 **业务模块负责人**，我想要 **将本模块的 API/事件/回调封装为连接器**，以便 **三方平台可通过连接器消费我模块的能力** | P0 | 业务模块负责人可创建连接器，定义触发器和动作，提交发布审批 |
...
### 2.2 三方平台消费方（开发者 / 业务人员）

> 💡 **定位**：企业内三方平台的人员，可能是开发者（偏技术操作），也可能是纯业务人员（偏配置操作），统一作为消费方使用连接器平台。连接流的核心用户旅程：发现连接器 → 编排连接流 → 测试 → 部署审批 → 运行监控。

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-04 | 作为 **三方平台消费方**，我想要 **浏览和使用已上架的连接器**，以便 **减少将 XX 平台能力集成到自己系统的编码工作量** | P0 | 消费方可浏览连接器目录，查看连接器包含的触发器和动作 |
| US-05 | 作为 **三方平台消费方**，我想要 **通过可视化拖拽编排连接流**，以便 **无需硬编码即可串联多个能力完成业务流程** | P0 | 消费方可拖拽连接器节点到画布，配置连线关系和数据映射，形成完整连接流 |
| US-06 | 作为 **三方平台消费方**，我想要 **配置连接器的认证信息**，以便 **安全地调用外部系统的 API** | P0 | 消费方可为连接器配置 AKSK/OAuth 等认证凭证，运行时自动使用 |
| US-07 | 作为 **三方平台消费方**，我想要 **手动触发连接流测试运行**，以便 **验证配置是否正确后再正式启用** | P0 | 消费方可点击"测试运行"按钮，查看测试执行结果 |
| US-08 | 作为 **三方平台消费方**，我想要 **启用/停用连接流**，以便 **按需控制业务流程的运行** | P0 | 消费方可一键启停连接流，停用后不再自动触发 |
| US-09 | 作为 **三方平台消费方**，我想要 **查看连接流的详细执行日志**，以便 **排查数据流转和执行异常问题** | P0 | 消费方可查看每个步骤的输入/输出数据、执行时间、错误信息 |
| US-10 | 作为 **三方平台消费方**，我想要 **收到连接流运行异常告警**，以便 **及时发现和处理问题** | P1 | 连接流运行失败时，消费方收到告警通知 |

### 2.3 平台运营方

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-11 | 作为 **平台运营人员**，我想要 **审核连接器发布申请**，以便 **确保上架连接器的合规性** | P0 | 运营人员可查看连接器发布申请，执行审批操作 |
| US-12 | 作为 **平台运营人员**，我想要 **审核连接流部署申请**，以便 **确保连接流的合规运行** | P0 | 运营人员可查看连接流部署申请，执行审批操作 |
| US-13 | 作为 **平台运营人员**，我想要 **监控连接器平台的整体运行状态**，以便 **及时发现系统异常** | P1 | 运营人员可查看平台级运行仪表盘，包括活跃连接流数、执行成功率等 |
...
> 💡 **定位**：连接器是对单个外部系统的标准化封装，包含触发器、动作、认证配置和参数定义。连接器是可复用的原子单元，一个连接器可被多个连接流引用。

#### 3.1.1 连接器注册与管理

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | 连接器创建 | 连接器提供方创建新连接器，定义基本信息 | • **基本信息**：连接器名称、英文名称、图标、描述、版本号<br/>• **连接器类型**：预置连接器（XX平台能力封装）/ 自定义连接器（外部系统封装）<br/>• 创建后状态为「草稿」 |
| FR-002 | 连接器编辑 | 连接器提供方编辑连接器的基本信息、触发器、动作 | • 支持修改基本信息、添加/编辑/删除触发器和动作<br/>• 已发布连接器编辑后生成新版本草稿，不影响已引用该连接器的运行中连接流<br/>• 修改记录留痕 |
| FR-003 | 连接器发布 | 连接器提供方提交连接器发布申请，经审批后上架 | • 提交发布后自动进入审批流程（复用能力开放平台审批引擎）<br/>• 审批通过后连接器状态变为「已发布」，可被连接流引用<br/>• 已发布连接器的新版本发布需重新审批 |
...
| FR-006 | 连接器列表查看 | 消费方浏览可用连接器目录 | • 列表展示：连接器名称、图标、描述、类型、状态、版本<br/>• 支持按类型（预置/自定义）、状态过滤<br/>• 支持搜索 |
| FR-006a | 连接器使用统计 | 连接器提供方查看本模块连接器的使用情况 | • 展示连接器被引用的连接流数量和列表<br/>• 展示连接器动作的调用次数统计<br/>• 支持按时间范围查询 |

#### 3.1.2 触发器定义

> 💡 **定位**：触发器是连接器"感知"外部事件的能力（入站方向）。触发器触发时执行的是**连接流**，而非连接器本身。连接器触发器（事件/Webhook）属于连接器，流触发器（定时/手动）属于流平台。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-007 | 事件触发器定义 | 定义基于事件订阅的触发器 | • 配置关联的能力开放平台事件（Topic/Scope）<br/>• 定义事件触发器的输出参数 Schema（事件数据结构）<br/>• 一个连接器可定义多个事件触发器 |
| FR-008 | Webhook 触发器定义 | 定义基于 Webhook 回调的触发器 | • 配置 Webhook 请求的参数格式（Header/Body Schema）<br/>• 定义 Webhook 触发器的输出参数 Schema<br/>• 一个连接器可定义多个 Webhook 触发器 |
...
> 💡 **定位**：连接流 = 流触发器 + 连接器 + 编排逻辑。连接流是消费方直接使用的对象，通过编排多个连接器实现跨系统业务流程。

#### 3.2.1 连接流创建与编排

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-014 | 连接流创建 | 消费方创建新连接流 | • 填写连接流名称、描述<br/>• 选择触发方式（事件/Webhook/定时/手动）<br/>• 创建后进入编排画布 |
| FR-015 | 可视化拖拽编排 | 通过拖拽式可视化编排器编排连接流 | • 提供画布式编排界面，左侧为连接器面板（可拖拽的触发器/动作节点）<br/>• 从连接器面板拖拽节点到画布，通过连线表示数据流转方向<br/>• 画布支持缩放、平移、对齐辅助线<br/>• 节点支持选中、删除、复制<br/>• 连线支持删除 |
| FR-016 | 流触发器配置 | 配置连接流的触发方式 | • **事件触发**：选择连接器的事件触发器，订阅能力开放平台事件<br/>• **Webhook 触发**：系统自动生成该连接流专属的 Webhook URL<br/>• **定时触发**：配置 Cron 表达式，支持可视化配置（每天/每周/每月）<br/>• **手动触发**：无需额外配置，通过界面"立即执行"触发 |
| FR-017 | 线性编排 | 支持触发器→动作1→动作2→…的线性编排 | • 第一个节点必须是触发器（事件/Webhook/定时/手动）<br/>• 后续节点为连接器动作，按顺序串联<br/>• 每个动作节点可选择已发布的连接器动作<br/>• 支持添加、删除、调整动作步骤顺序 |
| FR-018 | 动作节点配置 | 配置每个动作节点的参数 | • 为动作节点配置输入参数（支持引用上游节点的输出）<br/>• 为动作节点配置连接器认证凭证<br/>• 配置节点级别的错误处理策略（重试次数/重试间隔） |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-021 | 连接流编辑 | 消费方编辑已创建的连接流 | • 支持修改连接流编排、触发器配置、动作配置、数据映射<br/>• 已启用连接流编辑后生成新版本草稿，不影响当前运行版本<br/>• 编辑后需重新提交部署审批 |
...
| 数据实体 | 关键字段 | 说明 |
|---------|---------|------|
| **Connector** | id, name, code_name, type(preset/custom), icon, description, status, version | 连接器主体 |
| **ConnectorTrigger** | id, connector_id, type(event/webhook), config, output_schema | 连接器触发器 |
...
| **Flow** | id, name, description, trigger_type, trigger_config, status, version, creator_id | 连接流主体 |
| **FlowNode** | id, flow_id, node_type(trigger/action), connector_id, action_id, config, position_x, position_y | 连接流节点 |
...
| 连接器详情 | /connectors/{id} | 连接器详情，含触发器/动作/认证信息 |
...
| 事件订阅 | 连接器平台 → 能力开放平台 | 连接器触发器订阅事件总线，接收事件推送 |
...
| # | 问题 | 影响范围 | 建议决策时间 |
|---|------|---------|-------------|
| OQ-001 | **MVP 预置连接器范围**：具体需要封装哪些 XX 平台能力（IM/云盘/审批/…） | 影响连接器开发和测试范围 | Plan 阶段开始前 |
...
| 版本 | 范围 | 核心价值 |
|------|------|---------|
| **MVP** | 预置连接器 + 可视化拖拽线性编排 + 平台托管运行 + 运行监控 + 基础数据映射 | 验证"零代码配置连接流"核心价值 |
...
| 需求挖掘报告需求编号 | 对应本规范 FR |
|---------------------|--------------|
| MH-01 预置连接器 | FR-001 ~ FR-013, FR-006a |
| MH-02 连接器生命周期管理 | FR-001 ~ FR-006, FR-006a, FR-043 |
| MH-03 连接流线性编排 | FR-014 ~ FR-020 |
