...
| FR-002 | 连接器编辑 | 编辑连接器基本信息 | • 支持修改名称、图标、描述、类型<br/>• **编辑基本信息时同步更新到最新版本的 basic_info_fields**，保证使用侧始终看到最新的基本信息<br/>• **特殊：若所有版本均已发布（无草稿版本），编辑基本信息时自动创建一个新的草稿版本**，基于最新已发布版本复制连接配置，仅更新 basic_info_fields，**自动分配新版本号** |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-005 | 连接配置查看 | 查看连接器的连接配置（默认查询最新版本） | • 默认展示最新已发布版本的连接配置<br/>• 无已发布版本时展示最新草稿版本<br/>• 展示内容：协议类型、协议地址、认证方式、入参 Schema、出参 Schema、超时设置、限流设置 |
| FR-006 | 连接配置编辑 | 编辑连接配置（仅允许编辑最新未发布的版本） | • 编辑目标始终为「最新未发布的草稿版本」<br/>• 无草稿版本时，基于最新已发布版本创建草稿，**自动分配新版本号**<br/>• 草稿版本可重复编辑，每次保存覆盖当前草稿<br/>• 已发布版本不可编辑（只读）<br/>• **可编辑内容**：协议类型、协议地址、**认证方式及认证凭证**、入参定义、出参定义、超时设置、限流设置等<br/>• 认证凭证加密存储，界面脱敏显示 |
| FR-007 | 版本切换 | 切换查看历史版本的连接配置 | • 支持查看版本列表（版本号、发布时间、状态、变更说明）<br/>• 支持切换到任意历史版本查看其连接配置（只读）<br/>• 版本切换仅为查看操作，不改变当前编辑目标 |
| FR-008 | 版本发布 | 将草稿版本发布为已发布版本 | • 数据已在编辑阶段（FR-002/FR-006）写入版本表，版本号已在创建草稿时分配<br/>• **发布动作仅更新状态**：草稿→已发布<br/>• 发布前校验：连接配置完整（协议地址、认证方式、参数 Schema 等）<br/>• 校验通过后进入审批流程<br/>• 审批通过后新版可用<br/>• 已引用旧版本的运行中连接流不受影响 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-009 | 连接流创建 | 创建连接流基本信息 | • **名称**：连接流名称<br/>• **描述**：连接流功能描述<br/>• 创建后连接流基本信息和第一个版本（草稿状态）同时生成，进入编排画布 |
| FR-010 | 连接流编辑 | 编辑连接流基本信息 | • 支持修改名称、描述<br/>• **编辑基本信息时同步更新到最新版本的 basic_info_fields**<br/>• **特殊：若所有版本均已发布（无草稿版本），编辑基本信息时自动创建新草稿版本**，基于最新已发布版本复制连接配置，仅更新 basic_info_fields，自动分配新版本号 |
...
| FR-019 | 版本发布 | 将草稿版本发布为已发布版本 | • 数据已在编辑阶段（FR-010/FR-017）写入版本表，版本号已在创建草稿时分配<br/>• **发布动作仅更新状态**：草稿→已发布<br/>• 发布前校验：编排配置完整（至少有一个节点）<br/>• 校验通过后进入审批流程<br/>• 审批通过后新版可用<br/>• 已引用旧版本的运行中连接流不受影响 |
