...
| FR-013 | 连接流部署 | 将连接流部署到运行时环境 | • 部署时选择已发布版本，**默认选中最新已发布版本**<br/>• 部署后连接流进入**运行中**状态<br/>• 部署需先有已发布版本<br/>• **部署无需审批** |
| FR-014 | 连接流启动 | 启动已停止的连接流 | • 启动后连接流进入**运行中**状态，开始响应触发<br/>• **启动无需审批**<br/>• 已启动的连接流才响应外部触发 |
| FR-015 | 连接流停止 | 停止正在运行的连接流 | • 停止后连接流进入**已停止**状态，不再响应触发<br/>• **停止无需审批**<br/>• 已停止连接流可随时重新启动 |
