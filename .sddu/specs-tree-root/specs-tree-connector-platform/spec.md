...
> 💡 **定位**：面向内部业务模块人员和三方平台人员，提供将外部系统封装为连接器的能力。连接器是对单个外部系统的**纯出站端点定义**（类比 import 的模块/库），定义单个功能点的完整调用信息（调用方式、认证、入参/出参 Schema），**不含触发器**。连接器是可复用的原子单元，一个连接器可被多个连接流引用。
...
> 💡 **定位**：连接器定义对外部系统的调用方式和认证方式。每个连接器对应单一功能点，仅一种调用方式。
...
> 💡 **定位**：面向三方平台人员，提供连接流的创建、编排、测试和全生命周期管理。连接流由**流入口节点**（触发器）→ **流逻辑节点**（MVP 含连接器节点）→ **流出口节点**（可选）三部分有向连接而成。
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-014 | 连接流创建 | 消费方创建新连接流 | • 填写连接流名称、描述<br/>• 选择流入口节点类型（事件/Webhook/定时/手动）<br/>• 创建后进入编排画布，画布初始包含流入口节点（顶部）和流出口节点（底部） |
| FR-015 | 可视化拖拽编排 | 通过拖拽式可视化编排器编排连接流 | • 提供画布式编排界面，左侧为连接器面板（可拖拽的连接器节点）<br/>• 画布三区域：流入口节点区 → 逻辑节点编排区 → 流出口节点区<br/>• 从连接器面板拖拽连接器节点到编排区，通过连线表示数据流转<br/>• 画布支持缩放、平移，节点支持选中、删除、复制 |
| FR-016 | 流入口节点配置 | 配置连接流的触发方式 | • **事件触发**：选择能力开放平台事件（Topic），定义触发数据 Schema<br/>• **Webhook 触发**：系统自动生成该连接流专属的 Webhook URL，支持签名验证<br/>• **定时触发**：配置 Cron 表达式（每天/每周/每月）<br/>• **手动触发**：通过界面"立即执行"触发 |
| FR-017 | 线性编排 | 支持入口→连接器→出口的线性编排 | • 第一个固定为流入口节点，中间为连接器节点，最后为流出口节点（可选）<br/>• 每个连接器节点可选择已发布的连接器<br/>• 支持添加、删除、调整连接器节点顺序 |
| FR-018 | 连接器节点配置 | 配置每个连接器节点的参数 | • 配置输入参数（支持引用上游节点输出）<br/>• 配置认证凭证<br/>• 配置重试策略（重试次数/间隔） |
| FR-019 | 字段映射配置 | 配置源字段到目标字段的映射 | • 在连接器节点配置界面提供字段映射编辑器<br/>• 支持从上游节点输出 Schema 中选择源字段<br/>• 支持一对一映射（源字段→目标字段），可视化展示 |
| FR-020 | 常量值配置 | 配置连接器节点的固定参数值 | • 支持配置常量值；与字段映射互斥（同一参数二选一） |
| FR-019a | 流出口节点定义 | 定义连接流的返回值 | • 支持配置返回字段列表，每个字段可引用上游节点输出或常量值<br/>• 支持选择返回值格式（JSON/原始数据）<br/>• 无出口定义时为 fire-and-forget |

#### 3.2.2 连接流生命周期

> 💡 **定位**：连接流的编辑、审批、启停、版本、删除、测试等全生命周期操作。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-021 | 连接流编辑 | 消费方编辑已创建的连接流 | • 支持修改编排、入口节点配置、连接器节点配置、数据映射<br/>• 已启用连接流编辑后生成新版本草稿，不影响运行版本<br/>• 编辑后需重新提交部署审批 |
| FR-022 | 连接流部署审批 | 连接流启用前需经审批 | • 提交部署后自动进入审批流程（复用审批引擎）<br/>• 审批通过后可启用，不通过需修改后重新提交 |
| FR-023 | 连接流启用/停用 | 消费方控制连接流的运行状态 | • 启用：开始监听触发器/按计划执行<br/>• 停用：停止运行，不再响应触发<br/>• 已停用连接流可随时重新启用 |
| FR-024 | 连接流版本管理 | 支持连接流多版本并存 | • 查看版本历史；每次编辑生成新版本草稿<br/>• 支持回滚到历史版本 |
| FR-025 | 连接流删除 | 消费方删除不再需要的连接流 | • 仅「已停用」状态可删除；删除后不可恢复，关联历史保留 |
| FR-026 | 连接流列表查看 | 消费方查看自己的连接流列表 | • 展示名称、触发方式、状态、最后运行时间<br/>• 支持按状态过滤、搜索；支持快捷启停/编辑/删除 |
| FR-027 | 手动触发测试 | 消费方手动触发连接流测试运行 | • 支持"测试运行"按钮，手动触发连接流执行一次<br/>• 测试运行不影响正式运行；结果独立展示，可查看每步执行详情 |
| FR-028 | 测试数据模拟 | 消费方提供模拟触发数据用于测试 | • 支持为触发器输入模拟数据（JSON 格式）<br/>• 系统提供基于触发器 Schema 的数据模板
