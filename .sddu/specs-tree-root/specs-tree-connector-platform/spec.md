...
| FR-015 | 连接流启用/停用 | 控制连接流的运行状态 | • 启用：开始监听触发器/按计划执行（需先有已发布版本；**启用无需审批**）<br/>• 停用：停止运行，不再响应触发（**停用无需审批**）<br/>• 已停用连接流可随时重新启用 |
...
| FR-029 | 节点级错误处理 | 单个连接器节点执行失败时的处理 | • 连接器节点执行失败时，标记该节点为「失败」<br/>• 节点失败后，连接流整体标记为「执行失败」<br/>• 失败上下文数据保留，供排查 |
...
| ID | 需求 | 描述 |
|----|------|------|
| NFR-018 | 执行容错 | 单个连接器节点失败不丢失上下文数据 |
...
| **事件 Scope 权限申请** | 连接器平台 → 能力开放平台 | 连接流入口节点若使用事件触发，需在能力开放平台申请对应事件的 Scope 权限，获得授权后才能订阅该事件 |
| **Scope 鉴权** | 连接器平台 → 能力开放平台 | 运行时连接器调用 API / 连接流触发事件时，通过能力开放平台进行 Scope 鉴权验证订阅关系 |
...
| EC-004 | 事件触发时，同一事件驱动大量连接流并发执行 | 执行队列缓冲，超出处理能力时排队等待 |
...
| **V1** | 完整编排（分支/循环/子流程）+ 代码模式互转 + 函数库+脚本 + 连接器 SDK + 非 HTTP 协议连接器（MySQL/Redis 等）+ 连接器上架/下架 + 资源配额与隔离 + 失败重试 | 满足复杂场景，开放开发者生态 |
...
| MH-06 平台托管运行时 | FR-022 ~ FR-027, FR-029 ~ FR-030 |
