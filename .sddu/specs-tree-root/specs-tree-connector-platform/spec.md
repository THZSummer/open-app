...
| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-01 | 作为 **三方平台开发者**，我想要 **浏览和使用预置连接器**，以便 **减少将 XX 平台能力集成到自己系统的编码工作量** | P0 | 开发者可浏览预置连接器目录，查看连接器包含的触发器和动作 |
| US-02 | 作为 **三方平台开发者**，我想要 **通过可视化拖拽编排连接流**，以便 **无需硬编码即可串联多个能力完成业务流程** | P0 | 开发者可拖拽连接器节点到画布，配置连线关系和数据映射，形成完整连接流 |
| US-03 | 作为 **三方平台开发者**，我想要 **配置连接器的认证信息**，以便 **安全地调用外部系统的 API** | P0 | 开发者可为连接器配置 AKSK/OAuth 等认证凭证，运行时自动使用 |
| US-04 | 作为 **三方平台开发者**，我想要 **查看连接流的详细执行日志**，以便 **排查数据流转和执行异常问题** | P0 | 开发者可查看每个步骤的输入/输出数据、执行时间、错误信息 |

### 2.2 业务人员（低代码用户）

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-05 | 作为 **业务人员**，我想要 **通过可视化拖拽创建连接流**，以便 **无需开发人员介入即可配置跨系统业务流程** | P0 | 业务人员可通过拖拽式编排器独立完成连接流的创建和配置 |
| US-06 | 作为 **业务人员**，我想要 **手动触发连接流测试运行**，以便 **验证配置是否正确后再正式启用** | P0 | 业务人员可点击"测试运行"按钮，查看测试执行结果 |
| US-07 | 作为 **业务人员**，我想要 **启用/停用连接流**，以便 **按需控制业务流程的运行** | P0 | 业务人员可一键启停连接流，停用后不再自动触发 |
| US-08 | 作为 **业务人员**，我想要 **收到连接流运行异常告警**，以便 **及时发现和处理问题** | P1 | 连接流运行失败时，业务人员收到告警通知 |

### 2.3 平台运营方

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-09 | 作为 **平台运营人员**，我想要 **审核连接器发布申请**，以便 **确保上架连接器的合规性** | P0 | 运营人员可查看连接器发布申请，执行审批操作 |
| US-10 | 作为 **平台运营人员**，我想要 **审核连接流部署申请**，以便 **确保连接流的合规运行** | P0 | 运营人员可查看连接流部署申请，执行审批操作 |
| US-11 | 作为 **平台运营人员**，我想要 **监控连接器平台的整体运行状态**，以便 **及时发现系统异常** | P1 | 运营人员可查看平台级运行仪表盘，包括活跃连接流数、执行成功率等 |
...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | 连接器创建 | 平台开发者创建新连接器，定义基本信息 | • **基本信息**：连接器名称、英文名称、图标、描述、版本号<br/>• **连接器类型**：预置连接器（XX平台能力封装）/ 自定义连接器（外部系统封装）<br/>• 创建后状态为「草稿」 |
| FR-002 | 连接器编辑 | 平台开发者编辑连接器的基本信息、触发器、动作 | • 支持修改基本信息、添加/编辑/删除触发器和动作<br/>• 已发布连接器编辑后生成新版本草稿，不影响已引用该连接器的运行中连接流<br/>• 修改记录留痕 |
| FR-003 | 连接器发布 | 平台开发者提交连接器发布申请，经审批后上架 | • 提交发布后自动进入审批流程（复用能力开放平台审批引擎）<br/>• 审批通过后连接器状态变为「已发布」，可被连接流引用<br/>• 已发布连接器的新版本发布需重新审批 |
| FR-004 | 连接器下架 | 平台开发者下架已发布的连接器 | • 下架前校验：无运行中的连接流引用该连接器时才允许下架<br/>• 有引用时提示影响范围，禁止下架或要求先停用关联连接流<br/>• 下架后该连接器不可被新连接流引用，已引用的不受影响 |
...
| NFR-009 | 权限控制 | 连接器发布/下架仅限平台开发者；连接流操作仅限创建者及管理员；Scope 权限复用能力开放平台模型 |
...
| 需求挖掘报告需求编号 | 对应本规范 FR |
|---------------------|--------------|
| MH-01 预置连接器 | FR-001 ~ FR-013 |
| MH-02 连接器生命周期管理 | FR-001 ~ FR-006, FR-043 |
