...
| ID | 需求描述 | 验收标准 | 优先级 |
|----|---------|---------|--------|
| FR-001 | **用户角色查询**：嵌入能力方查询用户在指定应用中的角色 | • 输入：应用标识（支持两种：open-app 应用ID / 外部应用编码 hisAppId）、用户账号<br/>• 系统自动识别传入的应用标识类型并解析为对应应用<br/>• 输出：用户在该应用中的角色编码列表（integer[]）<br/>• 角色编码：`0`(开发者) / `1`(拥有者) / `2`(管理员)，对应现有 `MemberTypeEnum`<br/>• 请求需携带内部服务凭证，凭证由平台管理员预配置，无效或缺失时拒绝访问<br/>• 开发阶段：Mock 数据 + 内部凭证可配置绕过；联调阶段：对接现有成员管理系统 + 正式凭证 | P0 |
...
| 用户账号 | 用户登录账号（输入） |
| 角色列表 | integer[] | 用户在该应用中的角色编码集合（输出），可选值：`0`(开发者) / `1`(拥有者) / `2`(管理员) |
...
| 3 | 角色编码是否与现有 `MemberTypeEnum` 一致？ | 数据模型 | 现有 `MemberTypeEnum` 为 `0=Developer`/`1=Owner`/`2=Admin`，直接复用即可 | ⏳ 待确认 |
