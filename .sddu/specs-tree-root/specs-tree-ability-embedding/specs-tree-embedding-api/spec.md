...
嵌入能力方的配置页面通过微前端嵌入 wecodesite（开放面）后，嵌入能力方需要对操作者进行权限校验。例如：只有应用的管理员才能配置群置顶内容。嵌入能力方需要根据**应用ID**和**用户账号**，查询该用户在应用中的**角色**。
...
sequenceDiagram
    ...
    OpenFace->>EmbedBackend: 调用能力后端服务<br/>(携带应用ID + 用户账号)
    EmbedBackend->>APIServer: 内部调用用户角色查询<br/>(携带内部凭证 + 应用ID + 用户账号)
...
| # | 目标描述 |
|---|---------|
| G-001 | 嵌入能力方可查询用户在应用中的角色（输入应用ID + 用户账号，返回角色列表） |
...
| ID | 需求描述 | 验收标准 | 优先级 |
|----|---------|---------|--------|
| FR-001 | **用户角色查询**：嵌入能力方查询用户在指定应用中的角色 | • 输入：应用ID、用户账号<br/>• 输出：用户在该应用中的角色列表<br/>• 角色枚举：拥有者、管理员、普通成员<br/>• 开发阶段返回 Mock 数据，联调阶段对接现有成员管理系统 | P0 |
| FR-002 | **内部凭证鉴权**：内部接口需校验调用方身份 | • 请求携带内部服务凭证<br/>• 凭证由平台管理员预配置，支持多服务方独立凭证<br/>• 凭证无效或缺失时拒绝访问<br/>• 开发阶段可配置绕过 | P0 |
...
| 业务操作 | 说明 | 鉴权 |
|---------|------|------|
| 用户角色查询 | 输入应用ID + 用户账号，返回该用户的角色列表 | 内部凭证 |
...
| v1.1 | 聚焦核心场景：简化为单个用户角色查询接口；去除应用验证/批量查询等非本期接口 | 2026-07-13 | SDDU Spec Agent |
