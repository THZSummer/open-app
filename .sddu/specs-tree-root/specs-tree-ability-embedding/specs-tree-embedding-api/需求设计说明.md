...
| 认证方式 | 内部凭证（`X-Internal-Token` 请求头），由平台管理员预配置 |
...
| roles | integer[] | 用户在应用中的角色编码列表，可选值：`0`(开发者) / `1`(拥有者) / `2`(管理员) |

**错误响应**：

| code | 说明 |
|:---:|------|
| 400 | 参数错误（如 appId 和 hisAppId 均为空） |
| 401 | 内部凭证无效或缺失 |
| 404 | appId 和 hisAppId 均未匹配到有效应用 |

**性能指标**：TPS ≥ 50，P99 < 200ms
...
// === 场景一：按平台 appId 查询（成功）===
// 请求体
{ "appId": "1234567890123456789", "hisAppId": null, "userAccount": "zhangsan@xxx.com" }
// 响应体 200
{ "code": "200", "data": { "appId": "1234567890123456789", "roles": [1, 2] } }

// === 场景二：按 hisAppId 查询（成功）===
// 请求体
{ "appId": null, "hisAppId": "EAMAP_APP_001", "userAccount": "lisi@xxx.com" }
// 响应体 200
{ "code": "200", "data": { "appId": "9876543210987654321", "roles": [0] } }
...
// === 场景五：用户无角色 ===
// 响应体 200（空角色列表）
{ "code": "200", "data": { "appId": "1234567890123456789", "roles": [] } }
```
