...
| FR-004 | **删除能力类型**：平台管理员删除 ability 类型 | • 删除前检查是否有应用订阅了该能力类型<br/>• 有关联订阅 > 禁止删除，提示"该能力类型已被 XX 个应用订阅，无法删除"<br/>• 无关联订阅 > 允许删除 | P1 |
...
> 💡 其余字段（中文名、英文名、中文描述、英文描述、能力类型编码、排序号、状态、图标、示意图等）保持现有结构不变。

**abilityType 编码规则**：
- 1-7：保留给现有预置能力类型（群置顶、群通知、链接增强、点对点通知、we码、应用入群通知、助手广场卡片）
