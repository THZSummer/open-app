...
|-----|--------|------|---------|--------|------|-----|------|
| `/ability/admin/list` | GET | 查询能力列表 | 查 | 管理面 | 管理员角色 | ≥50 | P99<500ms |
| `/ability/admin` | POST | 创建能力 | 增 | 管理面 | 管理员角色 | ≥20 | P99<1s |
| `/ability/admin/{id}` | PUT | 更新能力 | 改 | 管理面 | 管理员角色 | ≥20 | P99<1s |
| `/ability/admin/{id}` | DELETE | 删除能力 | 删 | 管理面 | 管理员角色 | ≥10 | P99<1s |

**输入参数**（详见 plan.md 3.3 节）：

- `#1 能力列表`: GET, query params: curPage, pageSize, keyword, sortField, sortOrder
- `#2 创建能力`: POST, body: abilityType(int), nameCn, nameEn, descCn, descEn, iconUrl, diagramUrl, orderNum, frontendEntryUrl, hidden
- `#3 更新能力`: PUT /{id}, body: nameCn, nameEn, descCn, descEn, iconUrl, diagramUrl, orderNum, frontendEntryUrl, hidden
- `#4 删除能力`: DELETE /{id}
...
| 接口 | 成功响应 data | 错误码 |
|------|-------------|--------|
| #1 列表 | 分页 VO 数组 | 403(无权限) |
| #2 创建 | { abilityType, nameCn, createTime } | 400(参数), 409(编码冲突) |
| #3 更新 | { id, updateTime } | 404(不存在) |
| #4 删除 | { id } | 404(不存在), 409(有订阅) |
...
sequenceDiagram
    ...
    Admin->>Web: 打开能力目录页面
    Web->>AdminCtrl: GET /ability/admin/list?curPage=1
...
sequenceDiagram
    ...
    Admin->>Web: 填写创建表单
    Web->>AdminCtrl: POST /ability/admin { abilityType, nameCn, ... }
    AdminCtrl->>AdminCtrl: 校验 abilityType 唯一性
    AdminCtrl->>FileSvc: 上传图标文件
    FileSvc-->>AdminCtrl: 文件 URL
    AdminCtrl->>DB: 写入 ability_t + ability_p_t
    DB-->>AdminCtrl: 成功
    AdminCtrl-->>Web: 返回创建成功
    Web-->>Admin: 提示"创建成功"
```
