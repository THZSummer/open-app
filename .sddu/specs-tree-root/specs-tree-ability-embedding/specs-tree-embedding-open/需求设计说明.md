...
| 变更类型 | 均为**修改**——接口路径、方法不变，内部逻辑/返回字段变化 |

> ⚠️ 三个接口均已有前端调用方，必须保持**向后兼容**：新增字段为 optional，不删除/修改现有字段。
...
| orderNum | int | 排序号 | — |
| frontendEntryUrl | string | 前端入口URL（有则返回，无则 null） | **新增** |

**性能指标**：TPS ≥ 100，P99 < 200ms
...
| 变更项 | 旧 | 新 |
|--------|---|----|
| 校验逻辑 | `AbilityTypeEnum.isValidCode()` 枚举校验 | 查 DB `ability_t` 存在且 `status=1` |
| 重复检查 | 检查 `app_ability_relation_t` 是否已订阅 | **不变** |
| 关联插入 | 写入 `app_ability_relation_t` | **不变** |
| 自动桥接 | 空实现 | 打日志（FR-003） |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|:--:|------|
| abilityType | int | ✅ | 能力编码（预设 1-7 或自定义 ≥100） |
...
sequenceDiagram
    ...
    WECODE->>OPeN: GET /ability/subscribed?appId=X
    OPeN->>DB: 查询已订阅列表（不再硬编码排除 type=6）
    DB-->>OPeN: 订阅列表（含 frontendEntryUrl）
    OPeN-->>WECODE: 响应
```
